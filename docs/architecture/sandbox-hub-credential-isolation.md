...
authorized by the existing run-bound agent capability.
...
same run-scoped gcode runtime and path policy.
...
1. A managed agent can obtain only a credential for its own run and project.
...
6. Managed execution fails before provider launch when credential issuance,
   sandbox preflight, or runtime materialization fails.
7. Every terminal agent transition attempts immediate revocation; PostgreSQL
   expiry and daemon-start reconciliation provide independent backstops.
...
runtime config (`src/gobby/config/app.py:518-604`), and
...
- **Disabled or unsupported sandbox:** fully affected by the host user's
  filesystem and loopback authority. AGY currently refuses managed transport
...
The daemon creates a login role for each managed run. A non-login capability
role defines gcode's table and function privileges. PostgreSQL row-level
security maps `current_user` to the run's project and rejects cross-project
reads and writes. The login uses `NOSUPERUSER`, `NOCREATEDB`, `NOCREATEROLE`,
`NOREPLICATION`, and `NOBYPASSRLS`, with `VALID UNTIL` set to the run deadline.
...
project, issue time, and expiry (`src/gobby/utils/local_token.py:51-133`). The
endpoint returns only the gcode service bundle authorized for that run. It
never exposes a general secret lookup primitive.
...
- Direct-PostgreSQL performance: preserved for search, symbol retrieval,
  indexing, and projection metadata. Only credential issuance and typed
  service-config resolution cross the daemon.
...
- Implementation cost: requires a privilege inventory, RLS policies, a role
  manager, lifecycle reconciliation, and gcode config-source changes.
...
Add a PostgreSQL-owned registry that records login role name, agent run ID,
session ID, project ID, issued time, expiry, revoked time, and credential
generation. The plaintext password exists only during issuance and runtime
materialization; the registry stores no password or DSN.

Use one stable `NOLOGIN` capability role for agent gcode operations and one
ephemeral `LOGIN` role per run. The capability role receives an explicit SQL
inventory derived from every gcode query at HEAD:
...
RLS policies resolve the project from `current_user` through the registry.
Security-definer helpers use a fixed `search_path`, validate the active role,
and return only the bound project. Gcode commands whose semantics are global
must fail clearly under an agent role. Standalone/operator gcode retains its
existing administrative path outside managed agents.
...
contract. A generation number supports rotation without reusing credentials.
...
bundle for the caller's run and project. The daemon resolves only the existing
gcode service keys required for FalkorDB, Qdrant, embeddings, and code-vector
configuration. Authorization derives identity from the verified agent token;
request parameters cannot select another run or project. Responses exclude
secret names, wrapped DEKs, KEK material, unrelated configuration, and the
...
inventory may include the current runtime bootstrap, current policy/tmp
directory, current hook spool, required logs, project state, installed
read-only binaries, and provider config paths already modeled by their
dedicated helpers. It excludes both root credentials and shared mutable tool
installations.
...
Daemon-side tool-chat execution receives an agent runtime context. Gcode CLI
subprocesses receive the scoped runtime home. Built-in filesystem tools enforce
the same sensitive-root deny set before touching a path. Arbitrary raw
subprocess execution is unavailable to a managed web-chat tool policy.
...
3. Materialize the runtime bootstrap with mode `0600` and the secret-free
   service-bundle endpoint.
4. Preflight database scope and the selected sandbox backend.
5. Launch the provider only after every step succeeds.

Rotation:

1. Create a new role and generation before the old role expires.
2. Atomically replace the per-run bootstrap.
3. Allow a short, configured connection-drain interval.
...
1. Agent exit is a terminal transition. `complete`, `fail`, `timeout`, `cancel`,
   explicit termination, spawn
   rollback, and watchdog cleanup enqueue an idempotent revocation.
...
- Typed service-bundle failure disables secret-dependent gcode features with a
  precise error; it does not fall back to local KEK access.
- Daemon loss leaves existing direct PostgreSQL work usable until the role
  expires. New service-bundle resolution and rotation fail closed.
...
- Add a migration and baseline schema for agent DB principal bindings, the
  non-login gcode capability role, grants, RLS policies, expiry metadata, and
  audit events.
...
- Direct SQL adversary tests cannot bypass project scope through crafted IDs,
  `search_path`, transactions, prepared statements, or role changes.
...
- Tests cover success plus partial-create rollback, repeated revocation,
  ...
  connection, and orphan role recovery.
- No terminal run retains a login-capable PostgreSQL role after reconciliation.
- The password and full DSN exist only in bounded issuance/runtime buffers and
  the mode-`0600` per-run bootstrap.
...
- Performance tests compare direct scoped-role search and index throughput to
  the current direct path and enforce the implementation epic's agreed
  regression budget.

### WP4 — KEK removal and typed service bundle

Scope:

- Remove `.secret_kek` from runtime asset linking and delete stale links and
  copies from every managed runtime root.
- Add an authenticated, run/project-bound daemon service-bundle endpoint with
  an explicit response schema and configuration-key allowlist.
- Update `crates/gcode/src/config/services.rs` and gcore configuration plumbing
  so managed mode consumes the bundle and rejects local KEK/secret expansion.
...
- Search and indexing work without KEK access; secret-dependent embeddings and
  projections work only through the typed bundle.
...
- Add provider-native capability declarations and effective preflights for
  Claude, Codex, Qwen, and Grok. Reject any backend that cannot prove the
...
- Authorize every agent-capable daemon endpoint from verified run/project
  claims and limit the service-bundle endpoint to its typed response.
...
- End-to-end tests cover issue, use, rotate, resume, normal exit, crash,
  timeout, forced revoke, expiry, daemon restart, and provider-native refusal.
...
1. Root bootstrap and KEK paths are unreadable and unwritable in every
   supported managed execution path.
...
5. Role expiry and every agent terminal path remove usable database authority.

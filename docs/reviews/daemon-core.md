...
- **Status (resolved):** Fixed by the flock-based PID claim (`src/gobby/runner_pid_file.py`, commit `ffea840af`) and the #18218 singleton hardening: `main()` claims the lock before subsystem init and exits 0 on contention (no launchd hot-loop), the lock file records the owner PID, `claim_pid_file` refuses to overwrite a live owner's record, `gobby start` consults `probe_daemon_lock` instead of trusting pidfile+cmdline, and `gobby stop` reports any lock survivor. The description below is preserved as the original finding.

...
- **worktrees/git `_locking.py` is dead code and provides no concurrency control** (`_locking.py:1-88`, `manager.py:72-80`) — wraps `git worktree lock/unlock` (anti-prune, not mutual exclusion); zero non-test callers. The real worktree-mutation guard is the dispatch-layer `task_dispatch_mutex`. File a refactor task to remove or document.
- **worktrees/git `_merge.py` is dead in production** — the only live `merge_branch` is the inline impl in `_sync.py`; `_merge.merge_branch` runs only in tests. Duplicate maintenance surface carrying the two latent merge bugs above.
- **`delete_worktree` returns stale `output=result.stdout` from the failed remove after rmtree fallback succeeds** (`_lifecycle.py:210-217,244-249`) — misleading diagnostics; use the prune result's output.
- **`get_worktree_status` returns `None` (dropping branch/commit) on an `int()` parse failure of ahead/behind** (`_status.py:90-99`) — a transient parse hiccup collapses the whole status; wrap only the `int()` conversions, defaulting ahead/behind to 0.
- **merge resolver: tier-2 cannot resolve a file whose last conflict is at EOF without a trailing newline** (`resolver.py:112` `_CONFLICT_BLOCK_RE` requires trailing `\n`; count mismatch at `:676-692`) — silent permanent tier-2 failure (escalates to full-file tier, no data loss). Make the regex tolerate `(?:\n|\Z)`, or derive splice positions from the same parser.
- **merge resolver: `clean_ai_source_response` rejects valid resolutions containing a bare `=======` line** (RST/markdown/asciidoc heading underline) (`resolver.py:119,171-173`) — false-negative making a class of files un-AI-resolvable. Require a full conflict triad (`<<<<<<<` AND `>>>>>>>`) rather than rejecting on a standalone `=======`.
- **merge resolver: no diff3 (`|||||||`) test coverage, and `_CONFLICT_MARKER_LINE_RE` omits `|||||||`** (`conflict_parser.py` handles diff3 extraction; `resolver.py:119`) — an LLM response leaking a `||||||| base` line passes the marker guard (compounds the apply-path Blocker). Add `|||||||` to the regex and diff3 round-trip tests.
- **merge resolver: `resolve_file`/`resolve` drop a file that legitimately resolves to empty content** (`resolver.py:350-356,372-378`, `if r.get("content")`) — a whole-file-deleted resolution is omitted from the content map while listed in `resolved_files` with `success=True`, so the MCP layer escalates it to human review. Key on `"content" in r`, not truthiness.
- **merge resolver: `auto_resolve_trivial_conflicts` suffix match can over-match nested `.gobby/` paths** (`resolver.py:61`, `f.endswith(".gobby/tasks.jsonl")`) — a vendored `vendor/x/.gobby/tasks.jsonl` would be auto-resolved by blindly taking `--theirs`. Match the exact repo-root-relative path.
- **clones `get_clone_status` catches bare `Exception` and returns None** (`clones/git.py:545-547`) — violates the no-broad-except convention; narrow to `(subprocess.SubprocessError, OSError)`. (Returning None here would actually be safer than the fabricated all-clean status — see the Blocker.)
- **clones `merge_branch` restores to literal "HEAD" when starting detached** (`clones/git.py:695-700,805-811`) — same detached-HEAD bug as the worktree twin; harmless in practice (clones are on a branch). Capture the SHA via `rev-parse HEAD` as fallback.
- **clones `sync_clone` push-failure message omits stdout fallback** (`clones/git.py:407-411`) — push diagnostics often land on stdout; use `push_result.stderr or push_result.stdout` to match the pull branch.
- **events: timed-out `wait()` leaves the registry entry resident** (`completion_registry.py:119-123`; caller `pipeline_executor.py:790` has no `finally`/cleanup) — per-completion in-memory growth on timeout; defers to the existing leak fix in `agents.md:76-80`.
- **events: `wake()` returns `None` while `dispatch_live_wake()` returns a rich result dict** (`wake.py:111-132` vs `:134-146`) — high-level `wake()` callers can't tell whether the live nudge was delivered, debounced, or failed; observability gap, not a correctness bug (durable ISM is the source of truth).
- **scheduler: `running_timeout_seconds` config description is now false** (`config/cron.py:22-25`) — claims "Mark cron run records failed after this many seconds in running state"; no code does this. Update the description or restore the behavior.
- **scheduler: `_execute_dispatcher` folds distinct stop reasons (`cap_reached`/`reason`) into a string and always reports `completed`** (`executor.py:310-350`) — the dispatcher heartbeat can't register a degraded/failed tick; same family as the completed-on-no-raise Blocker.

## Systemic patterns

- **Errors-as-in-band-success at every action seam.** Cron handlers return error/degraded strings (`linear_sync`, dispatcher), pipelines return `status=failed` without raising, `merge_apply` commits marker-laden content, `resolve()` returns `GIT_AUTO` on dropped conflicts, `get_clone_status` converts a failed `git status` into an all-clean result. The executor's "non-exception == completed" rule and the merge gate's "`--diff-filter=U` clean == done" check are only as strong as the weakest adapter's discipline. This is the same family flagged in `llm-prompts.md` — not subsystem-specific.
...
- **Dead/unwired safety layers shipped as if active.** The entire autonomous stuck/progress-detection layer (no recorder, no `is_stuck` caller), `worktrees/git/_locking.py` and `_merge.py`, the scheduler's `_planned_restart_*` and `fail_stale_running_runs`. Latent bugs accumulate in code that looks load-bearing.
...
- **Heuristics standing in for semantic comparison.** Autonomous tool-loop keys on arg names not values; test/build classification uses bare substrings and defaults to high-value success; merge marker validation rejects bare `=======` while missing `|||||||`. Each biases toward false negatives (looks productive) or false positives (varied work looks looped).

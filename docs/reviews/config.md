...
- **Where:** `config/code_index.py:21-26,84-87` (`maintenance_interval_seconds`/`max_file_size_bytes`/`summary_batch_size`/`sync_worker_interval_seconds` lack lower bounds while siblings have `ge=1`); `config/tasks.py:148,152,131` (expansion `timeout`/`research_timeout`/`research_max_steps` lack the `>= 0` validator `WorkflowConfig.timeout` has); operator-supplied URLs (`mcp_proxy/models.py:122`, `config/voice.py:15`, `config/local.py:15`, `config/ai.py:17`, `config/extensions.py:51`) validated only non-empty, no scheme/host check.

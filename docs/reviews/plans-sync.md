...
- **Confidence:** high.
- **Resolution:** Resolved by #16295: active help and contract docs now describe only `db|matrix-file`; this finding remains as historical review evidence.
...
7. **Doc/code contract drift (historical finding).** At review time the canonical deferral YAML example didn't parse; `--task-tree db|jsonl|path` was actually `db|matrix-file`; the documented symbol-ref format couldn't be resolved by the sweep; CLAUDE.md's `$N` placeholder rule was stale against the psycopg `%s` reality. The task-tree mode mismatch was resolved by #16295.

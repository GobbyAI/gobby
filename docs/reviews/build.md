...
4. **`explain_dispatch` re-implements dispatcher truth** (the gate set and the active-agent count) instead of reusing `list_automation_candidates`/`count_active_agents`/`dispatch_rules.evaluate` — the root of the holistic-gate Blocker and a latent drift source.

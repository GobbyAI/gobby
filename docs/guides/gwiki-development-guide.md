...
| Synthesis provider discovery | `GET /api/providers/models` | No body. | `{ "providers": [ { "provider", "available", "models": [ { "canonical_model", "reasoning", "context_length", "max_output_tokens", "routes", "provenance", ... } ], "refresh": { "generation", "sources" }, ... } ] }`. Routes are keyed by `standard`/`fast` and may report `usage_multiplier`, `throughput_multiplier`, and activation descriptors; sources report `pending`, `ok`, `stale`, or `error`. | Skip daemon-assisted synthesis and return source/manually-authored wiki content. Stale or errored refresh sources still permit last-good models. |
| Synthesis stream | `WS /ws` message `chat_message` | JSON frame: `type: "chat_message"`, `conversation_id?`, `content?`, `content_blocks?`, `model?`, `provider?`, `project_id?`, `request_id?`, `reasoning_effort?`, `speed_mode?: "standard" | "fast"`, `attachments?`, `tts_enabled?`. `speed_mode` applies to one send. | Frames include `connection_established`, `session_info`, `chat_stream` chunks with `done`, `chat_thinking`, `tool_status`, and `chat_error`. Final completion metadata may include `speed: { requested, effective, status, reason }` plus usage, context window, session ref, and SDK session id. | Degrade synthesis, keep source collection/indexing results, and do not assume a provider-native completion schema. |
...
| Agent dispatch | `POST /api/agents/spawn` | JSON `AgentSpawnRequest`: `task_id`, `agent_name?`, `prompt?`, `web_chat?`, `isolation?`, `provider?`, `model?`, `reasoning_effort?`, `reasoning_required?`, `speed_mode?: "standard" | "fast"`, `workflow?`, `branch_name?`, `base_branch?`, `timeout?`. | `AgentSpawnResponse`: `success`, optional `run_id`, `child_session_id`, `conversation_id`, `isolation`, `branch_name`, `pid`, `message`, `reasoning`, `speed: { requested, effective, status, reason }`, or `error`. | Return dispatch degradation metadata. `fast_unavailable` fails before dispatch. `gwiki` must not spawn or manage internal subprocesses. |
...
- Provider capability collectors and bundled empty-store seeds are daemon-owned.
  `gwiki` consumes the matrix envelope and its provenance/refresh health; it does
  not discover or persist provider model facts itself.

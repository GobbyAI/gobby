...
| Grok | snake_case hooks in `GROK_EVENT_MAP`; ACP `0.2.51` init reports `loadSession`, `cancelRewind`, `_meta.x.ai/fs_notify`, and `availableCommands` (`compact`, `context`, `session-info`) | Observe stdout is ignored; PreToolUse uses deny reason/`updatedInput`; Stop and SubagentStop use `block` + `additionalContext` | `pre_tool_use` permission control (`permission_decision`, `auto_approve`, modified input); live ACP model discovery reads `_meta.modelState.availableModels` | Not supported | Static fallback defaults to `grok-composer-2.5-fast` (200k ctx) with `grok-build` (512k ctx) secondary; reasoning efforts are `low`, `medium`, `high`, `xhigh`, `max` |

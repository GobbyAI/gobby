...
| `_gobby_feedback_epoch_reviewed` | unset | bool | Persisted epoch ack for the bundled Gobby-experience survey; set true when the gate fires or `gobby-sessions:feedback` succeeds, and re-armed only by a context reset (`reset-gobby-session-feedback-on-context-reset`), never by a task closure. Not a template default |

...
| code_index.community_label.enabled | DaemonConfig schema via /api/config/schema; save via /api/config/values | (none — daemon-config-only) | cli-only | keep | (none) | edited in the runtime config file; settings UI is intentionally out of scope for this epic |
| code_index.community_label.batch_size | DaemonConfig schema via /api/config/schema; save via /api/config/values | (none — daemon-config-only) | cli-only | keep | (none) | maximum 20; settings UI is intentionally out of scope for this epic |
| code_index.community_label.profile | DaemonConfig schema via /api/config/schema; save via /api/config/values | (none — daemon-config-only) | cli-only | keep | (none) | edited in the runtime config file; settings UI is intentionally out of scope for this epic |
| code_index.community_label.candidates | DaemonConfig schema via /api/config/schema; save via /api/config/values | (none — daemon-config-only) | cli-only | keep | (none) | array items=string; settings UI is intentionally out of scope for this epic |
| code_index.community_label.max_concurrency | DaemonConfig schema via /api/config/schema; save via /api/config/values | (none — daemon-config-only) | cli-only | keep | (none) | edited in the runtime config file; settings UI is intentionally out of scope for this epic |
| code_index.community_label.max_tokens | DaemonConfig schema via /api/config/schema; save via /api/config/values | (none — daemon-config-only) | cli-only | keep | (none) | edited in the runtime config file; settings UI is intentionally out of scope for this epic |
| code_index.community_label.decisions_api_base | DaemonConfig schema via /api/config/schema; save via /api/config/values | (none — daemon-config-only) | cli-only | keep | (none) | optional Decisions API base URL; settings UI is intentionally out of scope for this epic |
| code_index.community_label.decisions_api_key | DaemonConfig schema via /api/config/schema; save via /api/config/values | (none — daemon-config-only) | cli-only | keep | (none) | secret reference; settings UI is intentionally out of scope for this epic |
| code_index.community_label.decisions_model | DaemonConfig schema via /api/config/schema; save via /api/config/values | (none — daemon-config-only) | cli-only | keep | (none) | edited in the runtime config file; settings UI is intentionally out of scope for this epic |
| code_index.community_label.decisions_min_confidence | DaemonConfig schema via /api/config/schema; save via /api/config/values | (none — daemon-config-only) | cli-only | keep | (none) | range 0–1; settings UI is intentionally out of scope for this epic |
| code_index.community_label.decisions_timeout_seconds | DaemonConfig schema via /api/config/schema; save via /api/config/values | (none — daemon-config-only) | cli-only | keep | (none) | edited in the runtime config file; settings UI is intentionally out of scope for this epic |

...
| auth.username | DaemonConfig schema via /api/config/schema; save via /api/config/values; read by src/gobby/storage/auth.py | (none — dead draft editor removed in task-19645 R5) | dead-backend | drop | (none) | registered live key; credential management stays CLI-only until #19650 lands an auth surface |

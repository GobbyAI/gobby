...
| Grok | 1.0.13 | `Stop(reason=end_turn)` and other non-user terminal reasons | `pending_interaction` kinds, correlated by exact interaction and `promptId` | Current `StopCancelled(reason=user_interrupt,cancelledBy=user)` |

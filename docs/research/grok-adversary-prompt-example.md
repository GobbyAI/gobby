...
-- [ ] Commit only the leaf’s files as `[gobby-#<leaf>] <type>: <summary>`. `close_task` with the SHA (`preview=false`). Do not merge or delete the worktree. Append a Progress Log entry. Call `gobby-sessions:compact_self` (or accept a close-triggered compaction as the boundary).
...
+- [x] Commit only the leaf’s files as `[gobby-#<leaf>] <type>: <summary>`. `close_task` with the SHA (`preview=false`). Do not merge or delete the worktree. Append a Progress Log entry. Call `gobby-sessions:compact_self` (or accept a close-triggered compaction as the boundary).

...
consumers in the owning deliverable's Consumers unchanged inventory described below.
...
| `consumer-coverage` | An exact symbol Target has owned call or import consumers in the code index. | Declare every same-repository consumer, including tests, in edit Targets or the owning deliverable’s Consumers unchanged inventory. Vendor, `node_modules`, and generated files are excluded. Missing consumers warn for narrative drafts and error for manifest-bearing plans or expansion validation. |
...
A deliverable can record verified consumers that require no edits in a separate
contiguous inventory before its acceptance block:

```markdown
Consumers unchanged:
- `src/caller.py` — no-edit-reason: The existing call already satisfies the new contract.
```

Each entry requires an exact repository-relative existing file path and a nonempty
`no-edit-reason`. Blank lines terminate the block. Absolute paths, traversal,
symbol scopes, globs, missing files, and conflicts with edit Targets anywhere in
the plan fail validation. Validation requires a project root to verify these files.
These entries satisfy consumer coverage only for exact symbol Targets in their own
deliverable. They never assign edit ownership or participate in shared-edit ordering
or production-size growth. Review snapshots and expanded task descriptions retain
the inventory and reasons. Consumers absent from both edit Targets and the owning
deliverable's unchanged inventory still produce coverage findings.
...
recheck. Narrative drafts without manifests remain reviewable with advisory
consumer coverage. A failed check returns structured diagnostics and creates no
evidence row; correct edit Targets or the owning unchanged-consumer inventory
before preparing a new round.

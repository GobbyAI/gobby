...
`gwiki_documents` and `gwiki_chunks`. Apply hub schema with `gdaemon apply`
so the `pg_search` extension and BM25 indexes exist before wiki commands run.

AI configuration is shared with the other Gobby CLI crates through
`gobby_core` and always routes through the daemon grant. The capability
matrix and outage behavior live in
[docs/guides/ai-configuration.md](../../docs/guides/ai-configuration.md).

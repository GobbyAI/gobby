...
fn schema_identity_json_reports_exact_contract() {
    let output = common::gwiki_command()
        ...
        .output()
        .expect("run gwiki schema-identity");

    assert!(
        output.status.success(),
        "gwiki schema-identity failed\nstdout:\n{}\nstderr:\n{}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );

    let actual: Value =
        serde_json::from_slice(&output.stdout).expect("schema identity stdout json");
...
fn parity_contract_tracks_code_grounding_and_dependency_classification() {
    ...
    assert_eq!(contract["contract_version"], 19);
    assert!(
        contract["commands"]
            .as_array()
            .expect("commands array")

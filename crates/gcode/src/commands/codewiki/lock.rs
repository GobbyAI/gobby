...
///
/// Two concurrent `gcode codewiki` runs against the same out_dir corrupt the
...
fn held_by_message(out_dir: &Path, lock_path: &Path) -> String {
    let holder = std::fs::read_to_string(lock_path)
        ...
        .unwrap_or_default();
    format!(
        "another gcode codewiki run is already writing {}{holder}; \
...
mod tests {
    ...
    fn second_acquire_on_same_out_dir_reports_the_holder() {
        ...
        assert!(
            message.contains("another gcode codewiki run is already writing"),
            "{message}"
        );
        assert!(

-- Converge existing databases on the baseline@420 tool-chat overlay binding.
--
-- #20664 taught `issue_tool_principal` to derive `code_overlay_project_id` from
-- the isolation workspace registered for the issuing session, and granted
-- `gobby_agent_issuer` the `agent_session_id` column it reads to find it. Both
-- landed as edits to `baseline.sql` carried by the `baseline_refresh` module
-- that the baseline@419 flatten removed, so they reached fresh installs and no
-- database that was already migrated. Those keep issuing tool-chat principals
-- whose binding stores a NULL overlay, and any `gcode` index of an isolation
-- workspace under such a principal fails the `code_indexed_projects` insert
-- policy, which requires
-- `id = COALESCE(current_code_overlay_project_id(), current_project_id())`.
-- Native Ask hits this on every run: it registers its sealed snapshot worktree
-- for the calling session and then indexes that worktree.
--
-- The statements below are exactly what baseline@420 already expresses, so a
-- fresh database is unchanged and re-running this migration is a no-op.

GRANT SELECT(agent_session_id) ON TABLE worktrees TO gobby_agent_issuer;
GRANT SELECT(agent_session_id) ON TABLE clones TO gobby_agent_issuer;

CREATE OR REPLACE FUNCTION gobby_agent_auth.issue_tool_principal(p_execution_id uuid, p_session_id uuid, p_machine_id uuid, p_expires_at timestamp with time zone, p_password text) RETURNS TABLE(role_name name, credential_generation integer)

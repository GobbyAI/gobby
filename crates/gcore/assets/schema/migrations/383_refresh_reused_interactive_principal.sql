-- Refresh reused interactive principals so handshake grants cannot outlive the role.
-- Expired non-revoked bindings occupy uq_interactive_principal_active and used to
-- make issue_or_reuse insert a duplicate. Reuse now matches that unique predicate
-- and extends VALID UNTIL / expires_at to the requested expiry.

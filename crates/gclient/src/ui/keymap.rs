...
/// its `workspace` noun becomes Gobby's `terminal` (a roster row) and its
...
pub const BINDINGS: &[BindingSpec] = &[
    ...
    spec("new_terminal", "Open a new terminal", &["prefix+shift+n"]),
    spec(
        "rename_terminal",
    ...
    spec("navigate_up", "Select the previous roster row", &["up"]),
    spec("navigate_down", "Select the next roster row", &["down"]),
    ...
    indexed("switch_terminal", "Switch to terminal 1-9", &[]),
        ...
        impl Action {
            ...
            pub fn name(self) -> &'static str {
                match self {
                    ...
                    Action::SwitchTerminal(_) => "switch_terminal",
                    $(Action::$variant => $name,)*
                }
            }
            ...
            pub fn from_name(name: &str, index: Option<u8>) -> Option<Action> {
                ...
                match name {
                    ...
                    "switch_terminal" => index.map(Action::SwitchTerminal),
...
action_names! {
    ...
    "rename_terminal" => RenameTerminal, "close_terminal" => CloseTerminal,
...
mod tests {
    ...
    fn indexed_override_takes_single_chords_in_order() {
        let keymap =
            Keymap::from_toml("[bindings]\nswitch_terminal = [\"prefix+f5\", \"prefix+f6\"]\n")
                .unwrap();
        let chords = &keymap.binding("switch_terminal").unwrap().chords;
        assert_eq!(chords[0].action, Action::SwitchTerminal(1));
        assert_eq!(chords[1].action, Action::SwitchTerminal(2));

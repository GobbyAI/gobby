...
//! The machines section: one row per machine the sidebar knows, this one
//! first, each a state dot (the most urgent of its agents), the machine's
//! name and a mark: `local` on this machine, `all` on every row while the
//! filter admits every machine. The highlighted rows are the machines the
//! sessions and agents sections list; a click on a row moves the filter
//! there (`Hit::Machine`).

use std::sync::OnceLock;

use super::agents::agent_state;
use super::{render_header, render_section_rows, SidebarHits, ALL_MACHINES};
...
/// The first section's header: the title row and one blank.
pub const MACHINES_HEADER_ROWS: u16 = 2;
/// Mark of this machine's row while the filter is local.
const LOCAL_LABEL: &str = "local";

/// This machine's row label: the host name without its domain, probed
/// once per process; `None` where the platform gives none.
pub fn local_hostname() -> Option<&'static str> {
    static HOSTNAME: OnceLock<Option<String>> = OnceLock::new();
    HOSTNAME.get_or_init(probe_hostname).as_deref()
}
...
/// The rows: this machine first, then the others in model order. Each
/// carries the most urgent state of the agents running on it, its name
/// (the host name here, the short id elsewhere), and the `local`/`all` mark
/// of the filter; the rows the filter admits are `active`.
...
pub(super) fn render_machines<W: WorkspaceView>(
    frame: &mut Frame,
    area: Rect,
    ws: &W,
    chrome: &Chrome,
    ...
    if render_header(frame, area, SidebarSection::Machines, &chrome.palette).is_none() {
        return;
    }
    let rows = machine_rows(ws, chrome);
    let (drawn, lane) = render_section_rows(frame, area, SidebarSection::Machines, &rows, chrome);
    hits.machines = drawn;
    hits.scrollbars[SidebarSection::Machines.index()] = lane;
}

...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=GTERM_BUILD_CHANNEL");
    ...
    let status = match command.current_dir(&vendored_dir).status() {
        Ok(status) => status,
        Err(err) if err.kind() == ErrorKind::NotFound => {
            panic!(
                "zig not found (required Zig 0.15). Install Zig 0.15 and ensure `{zig}` is on PATH, or set ZIG to the zig binary."
            );
        }

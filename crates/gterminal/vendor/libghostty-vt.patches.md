...
patch: deleted (`vendor/patches/libghostty-vt/0001-default-grapheme-cluster-mode.patch`)

herdr issue: https://github.com/herdrdev/herdr/issues/243

removed because: the patch set `default_modes.grapheme_cluster` inside
`ghostty_terminal_new` so DEC private mode 2027 survived RIS. The C API now
exposes the same control: `ghostty_terminal_set` with
`GHOSTTY_TERMINAL_OPT_MODE_DEFAULT` and a
`GhosttyTerminalModeConfig { mode: 2027, value: true }` sets both the current
value and the value a full reset restores. `Terminal::new` does that
(`crates/gterminal/src/ghostty/terminal_api.rs`), so the vendored source is
unmodified here.

evidence: with the patch reverted out of the vendored tree, all four
grapheme regressions pass:

```sh
cargo nextest run -p gobby-terminal --features vt-engine --lib grapheme_cluster_mode
# 4 tests run: 4 passed
#   ghostty::tests::grapheme_cluster_mode_is_default_and_survives_full_reset
#   pane::terminal::grapheme_cluster_mode_tests::grapheme_cluster_mode_is_default_and_survives_full_reset
#   pane::terminal::tests::grapheme_cluster_mode_renders_flag_emoji_in_single_wide_cell
#   pane::terminal::tests::grapheme_cluster_mode_renders_zwj_family_in_single_wide_cell
```

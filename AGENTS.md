...
2. **NEVER create or leave monoliths.** Keep code files under 1,000 lines. You *MUST* search for an existing refactor task or create it if one does not already exist in gobby-tasks. Place it under the parent #12730 for the gobby project if working in gobby. Create a new epic only if no suitable open or active epic exists. Leave these tasks for another agent to pick up.

...
- **Size guard.** Hand-maintained production files stay under 1,000 lines and the hook
  blocks threshold-crossing writes. Measured 2026-09-21: `src/gobby/terminals/workspace_ops.py`
  964, `src/gobby/storage/workspaces.py` 967, `crates/gclient/src/app/live_loop/actions.rs`
  979, `crates/gclient/src/app/live_loop/menu.rs` 970, `crates/gclient/src/app/live_loop.rs`
  902, `crates/gclient/src/app/mod.rs` 927, `crates/gclient/src/daemon/live.rs` 985,
  `crates/gclient/src/daemon/mod.rs` 987. Deliverable 1.2 carries genuine splits; 2.1 lives in new
  `command` modules and touches `startup.rs` (688 lines) by two symbols and
  `daemon/workspace.rs` (374 lines) by one enum variant and two optional fields. gclient
  enforces the ceiling in `crates/gclient/tests/source_size.rs`.
...
### 1.1 Migration 446: pane role and tab runbook columns [category: code]
`kind: deliverable`

Targets:
- `crates/gcore/assets/schema/migrations/446_add_runbook_roles.sql`
...
Add migration 446: `ALTER TABLE workspace_panes ADD COLUMN role text` and
...
- Precedent commit `541320efc2` (migration 444) shows the carrier procedure; there is no
  single regenerate command. Order: (1) append an `EmbeddedMigration { version: 446,
  ...
  `GOLDEN_LATEST_CHECKSUM`, `GOLDEN_ASSETS_ROOT_HASH`, and `latest_version: 446` in
...
- Latest existing migration is `445_drop_task_validation_system_prompt.sql`; refs became
  zero-based in 442.
...
- 1.1.1 - Migration 446 adds nullable `role` on `workspace_panes` and nullable `runbook`
  ...
  accepts 1024. file: `crates/gcore/assets/schema/migrations/446_add_runbook_roles.sql`.
  test: `crates/gcore/tests/schema_contract.rs::migration_446_enforces_role_and_runbook_byte_limits`.
- 1.1.2 - `MIGRATIONS` embeds version 446 with its checksum and the catalog manifest
  lists both columns. symbol: `MIGRATIONS`. file: `crates/gcore/assets/schema/catalog.manifest.json`.
- 1.1.3 - The schema identity contract tests pass with `latest_version` 446. test:
...
`src/gobby/storage/workspace_layout.py` and update importers, so the 967-line module
...
(1.3 failure mode). Split: move the pane I/O surface (`PaneWrite`, `PaneOutputWait`,
...
Research context:
- Entry points: `WorkspaceOps.tab_create` (354-384), `pane_split` (442-472),
  `pane_rename` (523-530), `_fill` (794-844, returns the `set_pane_terminal` row, which
  already carries `role`), `_emit` (692-709); `WorkspaceManager.create_tab` (598-636),
  `add_pane` (720-733), `rename_pane`, `list_panes` (568-578), `get_pane_for_terminal`
  (886-891); `TerminalManager.bind_session` writes `terminals.session_id`
  (`src/gobby/storage/terminals.py` 663-721). Line numbers are 2026-09-21 navigation
  hints only.
...
- `swap_panes` (`workspaces.py` ~750-761) remaps layout leaves only; `move_pane`
  (~764-803) updates `tab_id` and `ref`; `remove_pane` (~735) deletes the row.
...
`daemon_event` in `daemon/live_reader.rs` turns an unknown kind into a protocol error
that `run_connection` answers by dropping the socket (`live_reader.rs` ~230). 1.2 adds
...
Research context:
- `startup::run` (`crates/gclient/src/startup.rs` 671-688) parses args, probes health,
  and starts the TUI; `resolve_probe_env` (447-465) holds the URL and token defaults to
  reuse. `LiveDaemon::connect` (`daemon/live.rs` 190), `Daemon::workspace_op`
  (`daemon/mod.rs` 458), `op_request` (`daemon/workspace.rs` 339-345), and
  `workspace_op_live` (`daemon/live_workspace.rs` 32-39) are the whole send path;
  `attach_request` (`workspace.rs` 328-336) serves `list`. `WorkspaceOp` already has
  every variant the verbs need (`workspace.rs` 172-324). Line numbers are 2026-09-22
...
- Daemon side: `WORKSPACE_OPS` in `src/gobby/servers/websocket/workspace_ws.py`
  ...
  `src/gobby/terminals/workspace_ops.py`: `tab_create` 354, `pane_split` 442,
  `pane_resize` 514, `pane_rename` 523, `pane_close` 532, `pane_send_text` 553,
  `pane_read` 599, `pane_wait_for_output` 611, `workspace_set_focus_hints` 309.

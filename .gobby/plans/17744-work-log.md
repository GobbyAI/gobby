...
### Round 2 — validator gaps + a second bug found and fixed (#17821)

The close validator rejected round 1 on two evidence gaps: no focused route-seam test for the uninitialized-personal scenario, and no UI evidence that the wiki tab renders empty/ready. Closing those gaps surfaced a real second bug.

**Route-seam test** (`tests/servers/routes/test_wiki_routes.py`): new `test_personal_scope_routes_resolve_uninitialized_workspace` — starting from a gobby home with no personal folder, provisions via `ensure_personal_project`, then asserts `/api/wiki/status`, `/api/wiki/pages`, `/api/wiki/graph` with `project=PERSONAL_PROJECT_ID` all return 200 `ok:true`, that the gateway (patched `GwikiGateway`) received the personal root as its `--project` value, and that the root carries `.gobby/project.json` with the canonical id — the exact on-disk contract at the subprocess boundary where the invalid_scope fired. Proved red against the pre-fix `projects.py` (checked out `33cd6289e^`, 1 failed; restored, 26 passed). Also fixed 4 pre-existing mypy errors in this file while in there (`wiki_routes.tempfile` indirect-module access, untyped closure, `FailingUpload` cast).

**Second bug — gwiki burned one vault fallback dir per call**: the headless UI check still showed the offline banner after the identity fix. Root cause: `resolve_vault_dir` treats an existing non-vault directory as a collision (protecting user dirs), and the vault claim marker is `_gwiki/scope.json` — but `health`, `graph`, `page write`, `compile`, and `librarian` all write derived artifacts (meta/health/, outputs/, pages, checkpoints, reports) into the resolved vault dir **without claiming it**, unlike `collect`/`index`/`session_sync` which call `vault::initialize` first. Sequence on the live machine: each UI poll's health call resolved a fresh candidate, wrote `meta/health/`, and thereby poisoned it — `wiki`, `gobby-wiki`, `gobby-wiki-001`, `gobby-wiki-002` all burned within minutes; after 999 numbered fallbacks the scope hard-fails again. Fix: `vault::initialize(&scope)?` (idempotent) added to all five vault-writing command entries in `crates/gwiki/src/commands/{health,graph,page,compile,librarian}.rs`. `audit` and `ask` were audited and are read-safe on empty vaults (audit's lifecycle promotion only rewrites existing pages; ask is pure retrieval). Red-first Rust test `health_claims_the_vault_so_repeat_runs_resolve_the_same_root` in `commands/health.rs` pins the production sequence: two health runs against a fresh project root must claim `wiki/` (scope.json present) and must not create `gobby-wiki`. Coverage rationale: the claim is the identical one-liner in all five entries; the health test pins the pattern at the offender the UI polls; graph/compile/librarian additionally require PostgreSQL/LLM services, so their claim behavior is covered by the shared `vault::initialize` unit tests plus the health regression.

**Verification (round 2)**: `cargo test -p gobby-wiki --lib` 826/826 (health test red → green), `cargo clippy -p gobby-wiki` clean, `cargo fmt -p gobby-wiki -- --check` clean; rebuilt release and reinstalled `~/.gobby/bin/gwiki`; deleted the four poisoned dirs from `~/.gobby/personal` (each contained only generated `meta/health/latest.{json,md}`). Live: two consecutive `gwiki health --project ~/.gobby/personal` runs both resolve `wiki/`, `_gwiki/scope.json` carries `project:00000000-…-060887`, no `gobby-wiki*` dirs created; the vault scaffolded the standard `gwiki init` shape (knowledge/, code/, INDEX pages, ai-readme). **UI evidence**: headless playwright-core run (scratchpad `wiki-personal.mjs`, screenshot `personal-wiki-empty.png`) with wiki fetches pinned to the personal project id — offline banner count 0, degraded banner count 0, wiki-mode radiogroup present, `invalid_scope` absent from the DOM, zero console errors; the tab renders the browse empty state ("Select a page from the tree, or press ⌘K…"). Python: `test_wiki_routes.py` 26/26, mypy/ruff/format clean, test-quality audit 0 new issues.

...
- `start_stage`: `ready → in_progress`. Increments `work_attempt_count` (sole work-attempt increment site). Legal on all policies.
...
- `mark_task_review_rejected` (`reject_review`): `needs_review → ready`. Increments `review_round_count` (sole review-round increment site). **Rejected** on `policy=none`. Stage loops until escalation cap (`max_review_rounds`) hit.
...
- `fail_stage`: work-failure path; legal ONLY when current state is `in_progress` (calling from `ready`, `needs_review`, `review_approved`, or `done` raises `IllegalStageTransitionError` — review rejection MUST go through `reject_review`, the sole site that increments `review_round_count`). Transitions `in_progress → ready` WITHOUT changing either counter (the just-failed attempt is already counted by the `start_stage` that started it; the next retry's `start_stage` is the next increment), or escalates the task when `work_attempt_count >= effective_max_work_attempts` (effective cap resolved as `state_row.max_work_attempts ?? registry_row.default_max_work_attempts`). Legal on all three review policies so long as the source state is `in_progress`.
...
3. For each existing task: write `task_stage_states` rows by mapping current `(lifecycle, status)` tuple onto the manifest derived from `task_type` defaults plus existing `stage-:<name>` skip labels. Mapping table (every observed pair audited against a SQL census before migration runs; unmapped tuples fail loudly):
   ...
   Migrate `review_round_count` from the legacy review-round labels (these labels historically tracked review iterations, not work attempts): `planning-round:N` (numeric suffix) → `planning.review_round_count`; `qa-attempts:N` (numeric suffix) → `development.review_round_count` (development subsumes the legacy `code_review_qa` rounds). `work_attempt_count` starts at 0 for every backfilled row — no legacy column or label tracks work attempts directly; the dispatcher only needs the counter to be monotonic going forward, and starting at 0 is correct for any row that has not yet entered a fresh `start_stage` post-cutover. Migrate per-stage caps from all five legacy `task_artifacts` cap columns onto the per-stage cap columns: `max_expansion_attempts → expansion.max_work_attempts`; `max_qa_rounds → development.max_review_rounds`; `max_merge_attempts → merge.max_work_attempts`; `max_holistic_rounds → holistic_qa.max_review_rounds`; `max_review_rounds → pr.max_review_rounds`. NULL legacy values stay NULL on the per-stage row (inherit registry defaults at evaluation time). Stages with no legacy cap column (`planning`, `test_arch`, and the four discovery stages) get NULL caps on every backfilled row; effective caps fall through to the registry defaults (`default_max_work_attempts=3`, `default_max_review_rounds=5`). There is no `max_planning_rounds` column on `task_artifacts` — earlier drafts referenced it speculatively.
...
7. Drop `lifecycle`, `lifecycle_stage`, and active `status` semantics in the same epic once callers are ported. Drop all five legacy `task_artifacts` cap columns (`max_expansion_attempts`, `max_qa_rounds`, `max_merge_attempts`, `max_holistic_rounds`, `max_review_rounds`) since per-stage caps on `task_stage_states` replace them. Any short-lived local migration helpers are deleted before closure.
...
- **Migration tests**: census every observed `(lifecycle, status, labels)` tuple in a fixture DB, run migration, assert per-task manifest matches expected mapping table, including `pr` / `merge` rows and delivery artifact references; assert `review_round_count` populated from the legacy review-round labels (`planning-round:N → planning.review_round_count`, `qa-attempts:N → development.review_round_count`); assert `work_attempt_count` starts at 0 for every backfilled row; assert per-stage caps populated from legacy artifact columns.

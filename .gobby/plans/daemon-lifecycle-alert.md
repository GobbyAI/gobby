...
`gobby-agents:send_message` supports explicit same-project fanout through
`target="project"`; `target="global"` is reserved for one-daemon coordination
across projects. The implementation reuses that API and adds rule-engine evidence
checks instead of introducing another broadcast tool.
...
- Scope coordination to the caller's project and exclude the sender and system
  session.
- Treat any peer in `interrupted`, `awaiting_input`, `awaiting_approval`, or
  `awaiting_handoff` as a hard veto, regardless of alert state.
...
### 1.1 Use explicit project broadcast coverage [category: code]
`kind: deliverable`
...
Use targetless `target="project"` for every live session in the sender's project:
`active`, `paused`, `interrupted`, `awaiting_input`, `awaiting_approval`, and
`awaiting_handoff`. Preserve sender exclusion, system-session exclusion, stable
ordering, transactional fanout, selector metadata, and the current handling for
an empty recipient set. Message text never wakes a recipient; `wake=true` requests
immediate processing, while protected sessions retain queued mail without daemon input.

Update mailbox coverage for every live state plus expired,
...
- 1.1.1 - `target="project"` durably fans out to every same-project live session
  while retaining all existing exclusions. file:
  `src/gobby/sessions/mailbox.py`.
...
same-project live peers, the protected-state subset, and the latest qualifying
alert for each lifecycle action.
...
- be a `target="project"` broadcast sent within the preceding 5 minutes;
- have persisted recipients covering every current live peer;
...
3. Hard block when any peer is in a protected lifecycle state.
...
The coordination rules apply only to non-spawned agents. When no live peer exists,
one lifecycle action is allowed without an alert.
...
persistence is sufficient; `wake=false` is the safe default and ACKs remain optional. The
...
- 1.3.1 - Bundled rules enforce the declared priority order and render
  ...
  `src/gobby/install/shared/workflows/rules/daemon-safety/daemon-lifecycle-alert.yaml`.
- 1.3.2 - Any interrupted, waiting, or handoff peer blocks lifecycle commands
  even when a valid alert exists. test: `tests/workflows/test_daemon_safety_rules.py`.
...
coordinated operator restart when no session is protected. After activation,

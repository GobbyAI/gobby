...
- There is no shipped compatibility surface. New code never reads or writes `projects.repo_path`. The column stays in frozen baseline 375 and on live schema 411 as unread source data until P6's `413_drop_projects_repo_path.sql` copies it into `project_checkouts` and drops it. No dual-read, dual-write, SQL `COALESCE`, or reusable legacy-mapping interface. No automatic predecessor-receipt refresh.
- Live schema head is 411 (`411_terminals.sql`). `BASELINE_VERSION` stays 375. Section 1.1 ships numbered migration `412_project_checkouts.sql` and does not add `CREATE TABLE` to `baseline.sql`. Do not apply 412 to the live hub until P6.
- `#18902` (gcode/gwiki daemon-native) closed 2026-08-16. This plan still installs the C1 grant shape in 1.1; do not wait on `#18902`.
- `#17678` (scoped code facts / graph views) closed 2026-08-20. D1 is framing, not an open deferral.
- Do not restart the daily daemon on a mid-epic commit. Work stays in worktrees until P6 completes.
- `hook_manager.py` is 907 lines. Do not add registration logic there. Keep `HookManager._ensure_project_in_db` as a delegate.
...
- `projects.repo_path` remains in gcode’s grant list until 6.1. 1.1 adds `deleted_at` SELECT and `project_checkouts` machine-scoped grants; 6.1 removes `repo_path` and rewrites `resolve_tool_session`.
...
- Grant shape installed in 1.1 (`#18902` is closed; C1 is not waiting on it):
  - `projects`: `SELECT (id, name, deleted_at)` plus the existing `repo_path` grant until 6.1
  ...
  - `gobby_agent_auth.resolve_tool_session(p_session_id UUID) RETURNS (session_id, project_id, machine_id, root_path)` `LEFT JOIN`s each eligible session to `project_checkouts` on `sessions.machine_id` + `sessions.project_id` only, keeps `COALESCE(session.status, 'active') NOT IN ('expired', 'deleted')`, and permits a null `root_path` when no primary checkout exists. Expired or deleted sessions still return no row. No `projects.repo_path` fallback. The function rewrite itself stays in 6.1.

## D1: Repository content versus index-view identity
`kind: framing`

`#17678` closed 2026-08-20. Canonical repository content identity separate from index-view identity is not an open deferral of this plan. This plan only keeps `code_indexed_project_states.root_path` as view metadata, requires primary-view agreement with the registered checkout, and deletes only the rebound `(machine_id, project_id)` active project/file states. The historical `#19651 → #18902 → #17678 → #19664` chain is recorded; `#17678` is not in `#19651`'s dependency closure.
...
### 1.1 Add project_checkouts via 412_project_checkouts.sql [category: code]
`kind: deliverable`

Targets:
- `crates/gcore/assets/schema/migrations/412_project_checkouts.sql`
- `crates/gcore/assets/schema/baseline.sql::*` — scope-reason: frozen at 375; 1.1 must not add CREATE TABLE here
- `crates/gcore/src/grant/bundle.rs::*` — scope-reason: derived carrier after latest-schema identity 412
- `crates/gdaemon/tests/cli_contract.rs::*` — scope-reason: derived carrier after latest-schema identity 412
...
- `crates/gcore/src/schema/assets.rs::*` — scope-reason: regenerate latest-schema identity constants for 412
...
- `src/gobby/storage/schema_expected_identity.json::*` — scope-reason: synchronize Python expected identity to latest 412
...
Do not edit `crates/gcore/assets/schema/baseline.sql`. Keep `BASELINE_VERSION` 375 frozen. Add this table in numbered migration `412_project_checkouts.sql`, with PK, unique root, cascading FKs, FORCE RLS, policies, and grants. Fresh lineages execute 412 after the baseline, so every statement that can meet an existing object is guarded:
...
Keep `BASELINE_VERSION` 375 frozen. Numbered migration 412 is the only place `project_checkouts` is created. Regenerate catalog checksum plus expected identity so latest-schema identity is 412.

**Acceptance:**

- 1.1.1 - Numbered migration `412_project_checkouts.sql` creates `project_checkouts` with the stated columns, timestamp defaults, PK, unique root, cascading FKs, and forced RLS. `baseline.sql` is unchanged. file: `crates/gcore/assets/schema/migrations/412_project_checkouts.sql`.
...
- `src/gobby/hooks/project_checkout_ingress.py` — split: cwd-marker register/validate leaves hook_manager.py as a one-line delegate
...
- `tests/cli/test_cli_build.py::*` — scope-reason: fail closed without a checkout
...
- `src/gobby/agents/lifecycle_checkout.py` — split: checkout root resolution leaves lifecycle_monitor.py under the ceiling
...
- `src/gobby/mcp_proxy/tools/tasks/_lifecycle_close_checkout.py` — split: checkout-aware close path leaves _lifecycle_close.py under the ceiling
...
- `src/gobby/servers/websocket/chat/_session_checkout.py` — split: checkout-backed project_path leaves _session.py under the ceiling
...
- `tests/mcp_proxy/tools/test_mcp_proxy_tools_task_worktree_lifecycle_decoupling.py::*` — scope-reason: replace repo_path-only project mocks with session-machine checkout or resolver fakes
...
- `tests/servers/websocket/chat/test_servers_websocket_chat_session.py::*` — scope-reason: session.project_path is the resolved checkout
...
- `tests/mcp_proxy/tools/sessions/test_mcp_proxy_tools_sessions_registration.py::*` — scope-reason: session-message project_path uses checkout, not repo_path
...
Replace `Project.repo_path` mocks in `test_task_lifecycle_coverage.py`, `test_tasks_lifecycle_coverage.py`, and `test_mcp_proxy_tools_task_worktree_lifecycle_decoupling.py` with session-machine checkout or resolver fakes. Lifecycle-monitor, project-context middleware, websocket session, spawn-factory, and session-message tests exercise checkout success plus each family's relevant overlay or missing-checkout branch.
...
### 3.3 Cut over files, source control, plans, and workflows [category: code] (depends: 1.3, 2.2)
`kind: deliverable`
...
- `src/gobby/servers/routes/source_control_checkout.py` — split: checkout-rooted git cwd leaves source_control.py under the ceiling
...
- `src/gobby/workflows/hooks_checkout.py` — split: overlay-aware dirty-file roots leave hooks.py under the ceiling
...
- `tests/servers/routes/test_servers_routes_sessions_routes.py::TestGetCommitCount`
- `src/gobby/plans/coverage.py::*` — scope-reason: coverage repo_root is require_root
...
- `tests/plans/test_covers.py::*` — scope-reason: coverage uses require_root
- `tests/cli/test_cli_sessions.py::*` — scope-reason: session cwd fails closed without a checkout
...
- `tests/cli/test_cli_plans.py::*` — scope-reason: plan CLI roots use checkout and fail closed without one
...
- 3.3.6 - Plans, wiki, scheduler, dream, workflows, and session-changes resolve a local checkout and fail closed or skip sentinels on each family's relevant missing-checkout branch. test: `tests/cli/test_cli_plans.py`. test: `tests/plans/test_handoff_manifest_service.py`. test: `tests/plans/test_review_evidence.py`. test: `tests/wiki/test_scope_resolution.py`. test: `tests/scheduler/test_cron_executor.py`. test: `tests/memory/test_dream.py`. test: `tests/workflows/test_hooks.py`. test: `tests/servers/test_session_changes.py`.
- 3.3.7 - Session commit-count uses the session machine checkout resolver and covers local success, missing checkout, missing machine, and foreign-machine refusal before git runs. test: `tests/servers/routes/test_servers_routes_sessions_routes.py::TestGetCommitCount`.
...
- `src/gobby/runner_lifecycle_checkout.py` — split: missing-checkout wiki-job cleanup leaves runner_lifecycle_subsystems.py under the ceiling
...
- `tests/storage/test_project_checkouts.py`
- `tests/code_index/test_code_index_storage.py::*` — scope-reason: Python primary upsert refuses a stale root; overlay-view writes remain
...
- 4.2.4 - The Python project-state upsert refuses a stale primary root and still writes overlay-view rows without a checkout. test: `tests/code_index/test_code_index_storage.py`.
...
- 4.2.6 - A paused primary seed or stats writer holding `FOR SHARE` cannot write old-root state after a different-root rebind takes `FOR UPDATE`; overlay mode still writes without a checkout lock. test: `crates/gcode/src/index/indexer/tests/api_contract.rs`. test: `tests/code_index/test_code_index_storage.py`.
...
- 4.2.9 - The Python file-state upsert and delete refuse a stale primary root, hold `FOR SHARE` through the write, and still write overlay-view rows without a checkout. test: `tests/code_index/test_code_index_storage.py`.
...
- `crates/gcore/assets/schema/migrations/413_drop_projects_repo_path.sql`
- `crates/gcore/src/grant/bundle.rs::*` — scope-reason: derived carrier after dropping projects.repo_path
- `crates/gdaemon/tests/cli_contract.rs::*` — scope-reason: derived carrier after dropping projects.repo_path
...
- `crates/gcore/src/schema/runner.rs::*` — scope-reason: recognize live schema 411 as the project-checkout predecessor
...
- `tests/storage/test_storage_account_identity_cutover.py::*` — scope-reason: add project-checkout-cutover to CAMPAIGNS and the baseline CHECK-constraint set
...
- `tests/cli/test_cli_import.py::*` — scope-reason: stop inserting projects.repo_path
...
- `tests/mcp_proxy/tools/workflows/test_mcp_proxy_tools_workflows_import.py::*` — scope-reason: stop inserting projects.repo_path
- `tests/memory/test_memory_manager_1.py::*` — scope-reason: stop inserting projects.repo_path
...
- `tests/storage/test_storage_task_affected_files.py::*` — scope-reason: stop inserting projects.repo_path
...
Predecessor is live schema 411 (`411_terminals.sql`), not a 375-only receipt. Before any candidate insert and without refreshing the baseline receipt: apply numbered migration 412 (or no-op if 412 is already present), then verify the complete table, constraint, policy, and grant shape (PK, unique root, FKs, forced RLS, daemon-runtime access, migration-owner access, the 1.1 project-and-machine capability SELECT policy, and the lock-only UPDATE grant). Do not refresh the 375 baseline receipt. Do not add `CREATE TABLE` to `baseline.sql`. Drop `projects.repo_path` in `413_drop_projects_repo_path.sql` after checkout rows are persisted. Update the production-Python DDL inventory in `tests/storage/test_schema_contract.py` with the exact post-implementation operation counts for `src/gobby/storage/project_checkout_cutover.py`. Keep exact equality so unexpected `CREATE`, `ALTER`, or `DROP` still fails.
...
- 6.1.7 - Campaign registry and baseline CHECK constraints include `project-checkout-cutover`; `account_identity_cutover.py` `_TARGET_CAMPAIGNS` and known-constraint recognition preserve that value, and constraint replacement retains the expanded set. test: `tests/storage/test_storage_account_identity_cutover.py`.
...
- title: Cut over agents, sessions, and MCP tool roots
  ...
  validation_criteria: '3.2.1: Task-path MCP ordinary filesystem work fails closed
    ...
    test: `tests/servers/test_project_context_middleware.py`. test: `tests/servers/websocket/chat/test_servers_websocket_chat_session.py`.
    test: `tests/mcp_proxy/tools/spawn_agent/test_factory.py`. test: `tests/mcp_proxy/tools/sessions/test_mcp_proxy_tools_sessions_registration.py`.
    ...
    test: `tests/mcp_proxy/tools/test_tasks_lifecycle_coverage.py`. test: `tests/mcp_proxy/tools/test_mcp_proxy_tools_task_worktree_lifecycle_decoupling.py`.
...
- title: Cut over files, source control, plans, and workflows
  ...
  validation_criteria: '3.3.1: Files routes resolve the calling machine checkout and
    ...
    branch. test: `tests/cli/test_cli_plans.py`. test: `tests/plans/test_handoff_manifest_service.py`.
    ...
    before git runs. test: `tests/servers/routes/test_servers_routes_sessions_routes.py::TestGetCommitCount`.
...
- title: Invalidate only the rebound machine index view
  ...
  validation_criteria: "4.2.1: Different-root rebind removes only the selected machine\u2019\
    ...
    \ writes overlay-view rows without a checkout. test: `tests/code_index/test_code_index_storage.py`.\n\
    ...
    \ test: `tests/code_index/test_code_index_storage.py`.\n4.2.7: Destination-side discovery\
...
- title: Run the project-checkout-cutover campaign
  ...
  validation_criteria: "6.1.1: Successful populated migration, covered-row rerun with\
    ...
    \ constraint replacement retains the expanded set. test: `tests/storage/test_storage_account_identity_cutover.py`.\n\

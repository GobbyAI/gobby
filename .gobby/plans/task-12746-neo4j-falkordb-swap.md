...
- `set_config_batch(entries)` (R35-F1 — live tool at `src/gobby/mcp_proxy/tools/config.py:184` whose body calls `config_store.set_many(flat_updates, source="mcp")`) — the batch path MUST honor the same secret contract as `set_config`. Two acceptable shapes; pick the SPLIT shape:
    - **Split secret vs non-secret entries:** before the `set_many(...)` call, partition `flat_updates` into `secret_updates` (where `is_secret_key_name(key)` is True or the entry's explicit `is_secret` flag is True) and `plain_updates`. For each entry in `secret_updates`, run `validate_falkordb_password(str(value))` if the key is `databases.falkordb.requirepass`; on failure, abort the WHOLE batch (do not partially persist) and return the validator's error. Persist `secret_updates` via `ConfigStore.set_secret(...)` per-key and `plain_updates` via the existing `set_many(...)` path. If any persisted secret key matches the restart-required set (`databases.falkordb.requirepass`), include `requires_restart: True` and the restart hint in the response.

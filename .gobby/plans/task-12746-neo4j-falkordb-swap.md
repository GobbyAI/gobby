...
- **`POST /api/config/import` (`import_config`):** apply the same partition + validate + transactional dual-write rule. Both call sites (`config_store.set_many(request.config_store, source="import")` at line 672 AND `config_store.set_many(diff, source="import")` at line 689 — the two branches differentiate whole-replace vs diff-import) MUST honor the secret contract. Include `requires_restart: True` whenever the imported set persists `databases.falkordb.requirepass`.

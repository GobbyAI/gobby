...
3. **`src/gobby/code_index/context.py`:** if the typing imports reference `Neo4jClient`, update to `FalkorClient`. Do not change construction logic — there is none in this file. Add an `async def close(self) -> None` method on `CodeIndexContext` that awaits `self._graph.close()` when `self._graph is not None` (idempotent — safe when graph is already None or already closed). § 4.3's shutdown ordering (R23-F1) calls this before `LocalDatabase.close()`. Also add `def clear_graph(self) -> None` that sets `self._graph = None` so the health-check failure path (R23-F1) can null the reference without going through the constructor.
...
a. **Health-check failure path:** the periodic FalkorDB health check (currently named for Neo4j; rename per the sweep above) MUST, on unhealthy return, clear BOTH client references — set `runner.memory_manager._falkor_client = None` AND call `runner.code_indexer.context.clear_graph()` (the new method added in § 2.2) so `CodeIndexContext.graph` is None and the `available` property returns False. This makes `/api/memories/...` and `/api/code-index/...` routes fall back to the "graph unavailable" path simultaneously, matching the user-visible single-backend semantics — partial degradation (memory works, code-index doesn't, or vice versa) is a confusing operator UX and a frequent source of false bug reports.
b. **Shutdown ordering:** in the runner's shutdown sequence, await `runner.code_indexer.context.close()` (the new method added in § 2.2) BEFORE calling `LocalDatabase.close()`. The MemoryManager already has its own `close()` plumbing; ensure both close paths fire even if one raises (use `try/except/finally` around the pair so one client's close failure does not strand the other connection open).

**Tests (R21-F1 + R22-F4 + R23-F2) — owned by this task:**

- `tests/test_runner_lifecycle.py` (R23-F2: lines 138, 210 import or fixture-patch Neo4j surfaces today). Update the fixture-level `Neo4jClient` patches to `FalkorClient`; add a focused case where the FalkorDB health check returns unhealthy and assert BOTH `runner.memory_manager._falkor_client is None` AND `runner.code_indexer.context.graph is None` after the next health-check tick — verifies the dual-client clear from R23-F1.a.
- `tests/test_runner_shutdown.py` (R23-F2: lines 322, 367) — replace Neo4j-named teardown assertions with FalkorDB equivalents; add a case where `runner.code_indexer.context.close()` is invoked and the underlying CodeGraph FalkorClient connection close fires exactly once. Verify shutdown ordering: code-index close runs BEFORE `LocalDatabase.close()` (R23-F1.b).

...
| 4 | `uv run pytest tests/memory/ tests/cli/ tests/code_index/ tests/config/ tests/utils/ tests/servers/routes/ --cov=gobby --cov-fail-under=80 --cov-report=term-missing` | Exit 0 (R22-F4: scope expanded from `tests/cli/installers/` to all of `tests/cli/`, plus `tests/config/`, `tests/utils/`, and `tests/servers/routes/` — these are the directories whose tests import `Neo4jConfig`, `_neo4j_client`, `is_neo4j_installed`, the deprecation handlers, and the admin payload assertions; the test ownership distribution under § 1.1, § 4.1, § 8.1 covers each. The `--cov-fail-under=80` flag enforces the threshold; without it `pytest` only reports coverage when explicitly requested per this repo's `pyproject.toml`.) |

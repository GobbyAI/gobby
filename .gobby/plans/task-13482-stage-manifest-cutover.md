...
These five legacy columns are migrated into the new per-stage cap columns during the Phase 2.2 backfill (acceptance 2.2.4) and dropped in Phase 5.3 cleanup (acceptance 5.3.7). After migration 234 commits, runtime code paths read effective caps exclusively via `state_row.max_<X> ?? registry_row.default_max_<X>` — no runtime reader touches `task_artifacts` cap columns. The columns survive on the schema until Phase 5.3 drops them so the historical migration helper can replay against pre-cutover databases.

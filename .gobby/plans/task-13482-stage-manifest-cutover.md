...
Target: `src/gobby/mcp_proxy/tools/tasks/_stages.py` (new — exposes both `create_stage_read_registry` and `create_stage_ops_registry` factories), `src/gobby/mcp_proxy/tools/tasks/_factory.py` (registers the stage read registry on the `gobby-tasks` server), `src/gobby/mcp_proxy/tools/tasks/_ops_factory.py` (registers the stage ops registry on the `gobby-tasks-ops` server), `.gobby/plans/task-13482-lifecycle-status-kanban.md` (strategy plan — the §2.3 worker owns the round-24 F1 edit to its `### MCP / HTTP / CLI surfaces` section per §2.3.6c so the strategy-plan tool listing splits across the two server headers and matches §2.3.6b(d)'s assertion). Server split (round-22 F1, matching the already-shipped #13731 contract at commit `f8c323b8e`): the three READ tools (`get_task_stages`, `list_stages_registry`, `get_task_type_defaults`) live on `gobby-tasks`; the eight MUTATING tools (`start_stage`, `complete_stage`, `fail_stage`, `add_stage`, `remove_stage`, `record_pr_verdict`, `record_pr_opened`, `record_merge_result`) live on `gobby-tasks-ops`. This mirrors the existing repo convention that mutating ops tools (e.g., `create_task` on tasks-ops, `reindex_tasks` on tasks-ops) live on `gobby-tasks-ops` while read tools (e.g., `get_task`, `list_tasks`) live on `gobby-tasks`. The three review-state-axis tools (`mark_task_needs_review`, `mark_task_review_approved`, `mark_task_review_rejected`) are a separate, pre-existing surface and remain on `gobby-tasks` unchanged (see §2.3.6a regression below); they predate the ops split convention and §2.6 rewires only their behavior, not their server placement.
...
- 2.3.6c — **(round-24 F1)** Strategy-plan MCP-surface split edit. The §2.3 deliverable explicitly owns rewriting the strategy plan's `### MCP / HTTP / CLI surfaces` section at `.gobby/plans/task-13482-lifecycle-status-kanban.md` so the tool listing splits across two server headers matching the §2.3 contract: a `New gobby-tasks MCP tools (read)` header listing exactly the three READ tools (`get_task_stages`, `list_stages_registry`, `get_task_type_defaults`), and a `New gobby-tasks-ops MCP tools (mutating)` header listing exactly the eight MUTATING tools (`start_stage`, `complete_stage`, `fail_stage`, `add_stage`, `remove_stage`, `record_pr_verdict`, `record_pr_opened`, `record_merge_result`). At round-24 baseline the strategy plan lists all eleven tools under a single `New gobby-tasks MCP tools:` header at lines 226-236; the worker MUST rewrite that subsection in place (and add a one-sentence prose lead-in noting the split mirrors the existing repo convention — read tools on `gobby-tasks`, mutating ops tools on `gobby-tasks-ops`) before the §2.3.6b(d) and §2.3.6b(e) regressions can pass. The strategy-plan path is on §2.3's target inventory for this purpose. The §2.3.6c-specific test `tests/plans/test_strategy_plan_mcp_split_headers.py::test_strategy_plan_has_separate_read_and_ops_headers` parses the strategy plan markdown, locates the `### MCP / HTTP / CLI surfaces` section, and asserts (i) two distinct subheaders matching the read/ops contract are present, (ii) the three read tools are listed under the read header and ZERO of the eight mutating tools appear under it, (iii) the eight mutating tools are listed under the ops header and ZERO of the three read tools appear under it; the broader cross-surface agreement is asserted by §2.3.6b(d) and §2.3.6b(e). file: `.gobby/plans/task-13482-lifecycle-status-kanban.md`. test: `tests/plans/test_strategy_plan_mcp_split_headers.py::test_strategy_plan_has_separate_read_and_ops_headers`, `tests/plans/test_strategy_plan_mcp_split_headers.py::test_read_header_lists_only_read_tools`, `tests/plans/test_strategy_plan_mcp_split_headers.py::test_ops_header_lists_only_mutating_tools`.

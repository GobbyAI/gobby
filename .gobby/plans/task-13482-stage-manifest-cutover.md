...
Target: `src/gobby/mcp_proxy/tools/tasks/_stages.py` (new), registered via `_factory.py` on the `gobby-tasks` server. Stage lifecycle tools — read AND mutating — live on `gobby-tasks` (NOT `gobby-tasks-ops`, which carries plan/expansion/build operations only), matching the strategy plan's MCP / HTTP / CLI surface and the existing convention that registers task lifecycle tools (including the three review tools) on `gobby-tasks`. `_ops_factory.py` is NOT touched by this deliverable.
...
| `start_stage(task_id, stage_name, notes?)` | `gobby-tasks` | Transition `ready → in_progress`. Increments `work_attempt_count`. |
| `complete_stage(task_id, stage_name, commit_sha?, artifact_updates?, validation_override_reason?)` | `gobby-tasks` | Transition to `done` per the row's `review_policy`: direct from `in_progress` for `policy ∈ {none, optional}`; from `review_approved` for `policy ∈ {required, optional}`; direct from `in_progress` on `policy=required` is REJECTED unless `validation_override_reason` is supplied. |
| `fail_stage(task_id, stage_name, reason, needs_human?)` | `gobby-tasks` | Work-failure path: transitions `in_progress → ready` (no counter change), OR escalates when `work_attempt_count >= effective_max_work_attempts`. Legal source state is `in_progress` only — calling from `ready`, `needs_review`, `review_approved`, or `done` raises `IllegalStageTransitionError`. Review rejection MUST use `reject_review`. |
| `add_stage(task_id, stage_name, position)` | `gobby-tasks` | Insert a row mid-manifest; mirrors `review_policy` + `reviewer_agent` from registry at insert. |
| `remove_stage(task_id, stage_name)` | `gobby-tasks` | Delete a row from manifest. |
| `record_pr_verdict(task_id, verdict, findings, report_ref?)` | `gobby-tasks` | Store `structured_pr_verdict` + `pr_review_report` on task_artifacts; calls `approve_review(task_id, 'pr')` for `approved` (transitions `pr.needs_review → pr.review_approved`; dispatcher's `pr_advance_rule` later runs `complete_stage`), or `reject_review(task_id, 'pr', reason=findings)` for `rejected`/`needs_changes` (transitions `pr.needs_review → pr.ready`; increments `pr.review_round_count`). Raises `IllegalStageTransitionError` if `pr` is not in `needs_review`. |
| `record_pr_opened(task_id, pr_url, github_pr_number?)` | `gobby-tasks` | Persist `pr_url` and `github_pr_number` artifacts; does not change `pr` stage state. Idempotent: re-recording the same `pr_url` is a no-op. |
| `record_merge_result(task_id, merge_sha?, report_ref?, failure_reason?)` | `gobby-tasks` | Persist merge outcome and advance/fail merge stage. Phase 2.3 registers the tool with stub semantics (delegates to a NotImplementedError until 4.2 wires the success/failure paths and cascade-close); 4.2 fills the body. |
...
- 2.3.2 — Tool registration adds the eleven new stage tools to the `gobby-tasks` registry only; `gobby-tasks-ops` is NOT touched. file: `src/gobby/mcp_proxy/tools/tasks/_factory.py`. test: `tests/mcp_proxy/tools/tasks/test_stage_tools_registered.py::test_tools_visible_on_gobby_tasks`, `tests/mcp_proxy/tools/tasks/test_stage_tools_registered.py::test_tools_absent_from_gobby_tasks_ops`.
...
- 2.3.6 — `record_merge_result` is registered on `gobby-tasks` with the documented signature and `inputSchema` accepting `task_id` (required), `merge_sha`, `report_ref`, `failure_reason` (all optional). The stub handler raises `NotImplementedError("wired in Phase 4.2")`; tool listing on `gobby-tasks` includes the tool name and schema. Phase 4.2 (acceptances 4.2.2 and 4.2.3) replaces the stub with the success/failure paths and cascade-close. file: `src/gobby/mcp_proxy/tools/tasks/_stages.py`. test: `tests/mcp_proxy/tools/tasks/test_record_merge_result_stub.py::test_registered_with_stub`.
...
- 2.3.6b — **Stage-lifecycle-tool server-placement regression.** The eight mutating stage lifecycle tools introduced by this deliverable (`start_stage`, `complete_stage`, `fail_stage`, `add_stage`, `remove_stage`, `record_pr_verdict`, `record_pr_opened`, `record_merge_result`) AND the three read tools (`get_task_stages`, `list_stages_registry`, `get_task_type_defaults`) are registered on the `gobby-tasks` server only; `gobby-tasks-ops` does NOT expose any of them. The test queries the `gobby-tasks` and `gobby-tasks-ops` registries and asserts: (a) `gobby-tasks` exposes all eleven stage tools by name with the documented `inputSchema`, (b) `gobby-tasks-ops` exposes zero of them, (c) every bundled agent allowed-tool reference in `src/gobby/install/shared/workflows/agents/*.yaml` that names a stage lifecycle tool uses the `gobby-tasks:` prefix and zero references use `gobby-tasks-ops:` for any of the eleven names, (d) the strategy plan's `### MCP / HTTP / CLI surfaces` section names each tool under "New `gobby-tasks` MCP tools" (i.e., the implementation plan's tool table and the strategy plan's tool surface section agree on the server). file: `src/gobby/mcp_proxy/tools/tasks/_factory.py`, `src/gobby/mcp_proxy/tools/tasks/_ops_factory.py`. test: `tests/mcp_proxy/tools/tasks/test_stage_tools_server_placement.py::test_stage_tools_on_gobby_tasks_only`, `tests/mcp_proxy/tools/tasks/test_stage_tools_server_placement.py::test_stage_tools_absent_from_gobby_tasks_ops`, `tests/mcp_proxy/tools/tasks/test_stage_tools_server_placement.py::test_bundled_agents_reference_gobby_tasks_stage_tools`, `tests/mcp_proxy/tools/tasks/test_stage_tools_server_placement.py::test_strategy_plan_and_implementation_plan_agree_on_server`.
...
**Test-architect rewrite — full workflow scope, not just call-site swap.** `test-architect.yaml` is rewritten end-to-end so the workflow remains executable under the new contract. The pre-rewrite YAML's `design` step has an `on_mcp_success` hook keyed on `gobby-tasks:mark_task_review_approved` that flips `handoff_ready=true` and gates the transition to `terminate`; if only the agent's instructions are flipped from `mark_task_review_approved` to `complete_stage` while the success hook stays keyed on the old tool name, a successful `complete_stage` call leaves `handoff_ready=false` and the workflow can never reach `terminate`. Every `test-architect.yaml` change required by this deliverable, in one rewrite:
  (a) **Instructions/prose:** the `instructions:` block (drafting prose) and the `design` step's `status_message:` are rewritten to describe completing the stage with `gobby-tasks:complete_stage` (NOT `mark_task_review_approved`). The CRITICAL RULES line "Do NOT call `mark_task_review_rejected`" is dropped — the rule is irrelevant under the new contract because review tools are illegal on `test_arch.policy=none` and the workflow cannot path through them. The "If blocked on missing context, escalate the stage task" guidance is preserved.
  (b) **`on_mcp_success` hooks:** the `mark_task_review_approved` success hook is removed and replaced with a `complete_stage` success hook that sets `handoff_ready=true` (server: `gobby-tasks` per §2.3.6b). The existing `escalate_task` success hook stays.

...
- New `gwiki pages [--prefix <p>]`: query `gwiki_documents` for the resolved scope selecting `path, title, frontmatter, updated_at` (verify exact timestamp column name in `crates/gwiki/src/setup.rs` during implementation); extract `tags` from frontmatter JSONB. Additionally walk the vault's outputs directory for markdown files on the filesystem (unindexed by design) into a separate array. Payload:

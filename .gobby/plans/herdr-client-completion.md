...
3. Hangers: #21191 folds in as P1 (still open, no commits). Its single-owner
   transition is coordinator-owned and happens post-expansion, pre-dispatch: resolve
   the generated 1.1 leaf, close #21191 as its duplicate with a backlink to that leaf,
   and verify exactly one open task owns the raw-input obligation before any dispatch —
   the Task Mapping section records the transition. #21207 and #21209 are
...
- `tests/servers/test_tmux_mixin.py::TestTerminalInputRouting`
...
- 1.1.8 - Retiring the run-id legacy input branch rewrites or removes its existing TestTerminalInputRouting expectation while preserving the detached-tmux non-UUID regression against the surviving attachment-scoped handler. test: `tests/servers/test_tmux_mixin.py::TestTerminalInputRouting`.
...
2. **`direct` block.** `terminal_attach_result` gains `direct`: `null` for proxy
   ...
   landed `PROXY_ATTACH_FAILURE_REASONS` reply path (#21207/#21258). A returned
   `AttachLocator` is validated before serialization, because its fields are optional
   and a backend-incoherent instance would otherwise serialize as `success: true` with
   an unusable `direct` block: the row's backend must match the locator's shape — a
   native row requires nonempty `frame_host_epoch`, `host_socket`, and
   `host_terminal_id` with no pane, a tmux row additionally requires one complete pane
   locator (`socket_path`, `pane_id`, `server_pid`, `server_start_time` all present) —
   and a malformed, partial, or wrong-backend result maps to the existing
   `locator_invalid` failure with the lease finalized, exactly like a non-locator
   return.
...
- 1.3.3 - A direct attach on a native row answers a `direct` block with the host epoch, frame socket path, and host terminal id; a direct attach on a tmux row additionally carries the pane locator; a proxy attach answers `direct: null`; a locator failure answers `success: false, code: "locator_failed"`; and a locator whose fields are incomplete or backend-incoherent — a native-row locator missing the epoch, socket, or host terminal id or carrying a pane, or a tmux-row locator with a partial pane — answers `success: false, code: "locator_invalid"` with the lease finalized, never `success: true` with an unusable `direct` block. test: `tests/servers/test_native_web_proxy.py::test_direct_attach_result_carries_locator`.
...
- `src/gobby/servers/websocket/server.py::*` — scope-reason: `WebSocketServer.start` and `WebSocketServer.stop` are the server lifecycle seams that start the registry publication worker on the running loop before any admission and fence, cancel, and await it during cleanup
...
pages carry `snapshot: null`. `encode_page`'s byte-cap truncation branch is repaired,
not inherited, because its current behavior is broken on all three edges this deliverable
now feeds: it emits `str(selected[-1]["id"])` — a bare id where `parse_list_cursor`
demands `<created_at>|<terminal_id>` — against rows that carry `terminal_id` and no
`id` key at all, and it turns an oversized first row into an empty page with no cursor,
silently ending traversal with rows lost. The repaired contract: truncation budgets the
complete WS/REST envelope (items, `next_cursor`, and the first page's `snapshot` block
together), a truncated page's `next_cursor` is the canonical
`<created_at>|<terminal_id>` cursor of the last *included* raw row so the next request
resumes exactly after it, and a single row that cannot fit inside the budget alone is a
typed failure on both transports rather than an empty page — every nonterminal page
returns a parser-valid cursor and either advances or fails explicitly.
...
no entry at all. Admission itself races capacity against the close notification: a
submitter blocked on the full queue re-checks the closed latch atomically before
inserting, so space freed by a concurrent close can never admit an entry behind the
fence, and close wakes every pre-admission waiter with the typed publication error
rather than leaving one blocked on capacity forever.

The sole consumer has an explicit lifecycle, reusing the `SocketRelay`-style
start/shutdown pattern inside `TerminalLeaseRegistry` and driven from the WebSocket
server's lifecycle seams — `WebSocketServer.start` and `WebSocketServer.stop` are the
carriers: `start` starts the worker on the running loop before any submission is
admitted, and `stop` closes it — close fences new submissions, deterministically drains
or fails every already-admitted completion, cancels and awaits the sole worker, and
leaves no task behind. The worker is explicitly restartable, because the registry is
process-wide and survives a server bounce while `WebSocketServer` supports a later
`start()` after `stop()`: the close fence is scoped to one worker run, and the next
`start` lifts it and launches a fresh worker task on the then-running loop — the same
registry keeps its `daemon_epoch` and its monotonic committed high-water, so a second
server run admits, numbers, and publishes exactly like the first instead of rejecting
every submission against a terminally closed worker. If the worker itself
faults, queued and future submitters are failed with a typed publication error and the
registry performs the same bounded relay cleanup; no admitted waiter is left pending
forever and the committed high-water never silently stalls behind an abandoned entry.
...
- 1.4.7 - The publication worker's lifecycle is explicit: `WebSocketServer.start` starts it before any submission is admitted and `WebSocketServer.stop` cancels and awaits it with no task left behind; a cleanup issued while the queue is full drains or fails every admitted completion deterministically; a submitter cancelled after admission does not strand the worker or the drain; and an injected worker exception fails queued and future submitters with a typed publication error while the registry performs bounded relay cleanup. test: `tests/servers/test_terminal_list_watermark.py::test_publication_worker_lifecycle_settles_all_waiters`.
- 1.4.8 - With 256 entries admitted and submitter 257 blocked on capacity, a concurrent close wakes that pre-admission waiter with the typed publication error and admits nothing behind the fence — the atomic closed-latch recheck rejects insertion even when the close itself freed queue space; and after a full `WebSocketServer.stop` a second `WebSocketServer.start` restarts the worker in place on the same registry — `daemon_epoch` unchanged, committed high-water continuing monotonically — so the second run admits, publishes, and cleanly shuts down. test: `tests/servers/test_terminal_list_watermark.py::test_preadmission_close_race_and_worker_restart`.
- 1.4.9 - `encode_page` truncation preserves forward progress on both transports: a page landing exactly at the byte budget includes its final row with no truncation, one row past the budget yields a truncated page whose `next_cursor` is the canonical `<created_at>|<terminal_id>` cursor of the last included raw row and whose next request returns the excluded row first, and a single row that cannot fit inside the budget alone produces a typed failure rather than an empty page — no branch emits a bare-id cursor, indexes a missing `id` key, or ends traversal with rows lost. test: `tests/servers/test_terminal_list_watermark.py::test_byte_cap_truncation_preserves_forward_progress`.
...
pub trait Daemon: Send + Sync {
    ...
    async fn close(&self, deadline: Instant) -> Result<(), DaemonError>;    // idempotent, terminal; owns the shutdown deadline (3.3): first poll latches + aborts before any await
}
pub enum DaemonError { Unauthorized, NotFound, Unavailable { retry_after: Option<Duration> }, Protocol { detail: String }, ControlRequestInFlight, ControlScopeIndeterminate, Timeout }
```

`ControlScopeIndeterminate` begins once a control send enters the `write_started`
indeterminate region (2.1.14's tombstone), persists for that attachment, and clears
only when the attachment is finalized or retired; 2.1.14 and 3.2.12 consume exactly
this variant.
...
and hold no accumulator of their own. The reassembler carries the existing
`terminalWsFragments` lifetime contract: 16 MiB per assembly, 64 MiB aggregate per
generation/socket, at most one incomplete `message_seq` per live attachment, and a
fixed non-sliding 5 s deadline per assembly. Retained payload plus bounded metadata is
charged against those budgets before a slice is appended, and the accounting is
released on every terminal path — completion, sequence error, timeout, finalization,
lag, cancellation, close, and generation teardown. A lagging receiver gets `DaemonEvent::Lagged`,
...
- 2.1.16 - Under a paused clock with multiple live attachments, the fragment reassembler enforces its lifetime bounds: a second incomplete `message_seq` on an attachment with one already open is refused, an assembly exceeding 16 MiB and an aggregate exceeding 64 MiB per generation are each rejected before append, an incomplete assembly whose fixed 5 s deadline expires is discarded with its accounting released, and after saturation-then-release every budget returns to zero and a fresh fragment sequence reassembles normally. test: `crates/gclient/tests/daemon_live.rs::fragment_reassembly_bounds_hold_under_saturation`.
...
before any delta. That receiver also preserves finalization in wire order: `ProxyHub`'s
already-running pump can see immediate EOF, protocol failure, or terminal exit and
publish `terminal_attachment_finalized` before `terminal_attach_result` arrives, so a
finalization for the returned `attachment_id` that is buffered before — or observed
during — `ProxyFrameSource` construction tombstones that id and surfaces the
finalization's code and reason as the source's typed finalized failure instead of
installing the source; the pane settles Detached and read-only, and no viewport,
control request, or detach is issued for the retired id. Fragments never reach it: the LiveDaemon reader (2.1) is the client's
...
- 2.2.10 - Cancelling the source-owned writer after a partial length prefix and after a partial payload retires the connection in both schedules, closes both socket halves, surfaces FrameError::Cancelled at the source boundary, and permits no later read or write. test: `crates/gterminal/tests/wire_codec_async.rs::async_writer_cancellation_retires_connection`.
...
- `theme.rs`: load the `impeccable` skill first. Map every herdr palette name to the
  ...
  state colours differs in relative luminance enough to survive a grayscale render;
  grayscale rank alone is not sufficient for state — per `.impeccable.md`, every
  state-bearing indicator (info, warning, destructive, success, control) also carries a
  non-color icon or positional cue that stays distinguishable desaturated; the
...
- 3.1.10 - Every rendered info, warning, destructive, success, and control-state indicator includes a non-color icon or positional cue and remains distinguishable in desaturated dark and light representative renders. test: `crates/gclient/tests/theme.rs::tokens_match_design_contract_and_survive_monochrome`.
...
**Live resize** (herdr parity: v0.8.0 consumes `SIGWINCH` through a signal flag plus a
coalescing poll and propagates client geometry to the controlled PTY —
`client/mod.rs` resize watcher → `ClientMessage::Resize` → `PaneResize` → PTY). The
imported `raw_input` reader drops resize events, so the loop selects over its own
coalesced resize source: a `SIGWINCH`-driven branch that reads the current terminal size
when it fires, coalesces bursts into the latest observation, and ignores transient
0×0 sizes. On a real change the loop resizes the Ratatui surface, recomputes every pane
rectangle, and sends `notify(terminal_set_viewport{attachment_id, rows, cols})` through
each live direct and proxy source. Geometry policy, recorded explicitly: a **controlled
native pane** — the client holds the write lease for the attachment — additionally
sends the existing lease-gated `terminal_resize` verb so the host PTY follows the
client's pane rectangle, which is the herdr behavior; a native pane observed without
control is viewport-only; and **tmux pane geometry is host-owned** — the client never
issues `terminal_resize` for a tmux row, implicitly or otherwise, and letterboxing
(`views/grid.rs`) remains the only accommodation for a differing owner geometry.
...
coalesces every intent naming the same observed generation into one episode. Coalescing
also rolls forward: when an intent arrives naming a *newer* generation than the active
episode observed — G1's episode is mid-flight and its replacement G2 has already
dropped before `Workspace` finished the roster handshake — the supervisor advances the
active episode's observation to that newer generation for its next attempt and
coalesces the new waiters into the same episode, preserving the current attempt count
and delay schedule. One disconnected episode keeps one budget until install and replay
succeed or the budget exhausts; a rolled-forward generation never restarts the count,
never spends attempts reconnecting to a generation already known dead, and never leaves
the newer intent undispositioned. A
...
`shutdown(workspace, daemon, deadline)` phase and the local terminal restore. The
invocation wiring is owned here for the same reason: `run_loop.rs` is this deliverable's
carrier, so this deliverable wires every non-panic loop exit to enter 3.3's
`shutdown(workspace, daemon, deadline)` phase exactly once, with the same absolute
deadline, immediately after the latch sets — 3.3 defines what that phase does without
editing the loop.
...
- 3.2.13 - Under a paused clock, with a G1 episode mid-flight whose reconnect produced G2 and G2 dropping before `Workspace` completes the roster handshake, the supervisor rolls the active episode's observation forward to G2: the next attempt targets G2, the attempt count and delay schedule continue uninterrupted from where the episode stood, every G1 and G2 waiter settles from the same single episode result, no second budget starts, and no attempt is issued against the dead G1 socket after the rollover. test: `crates/gclient/tests/client_loop.rs::reconnect_episode_rolls_generation_forward`.
- 3.2.14 - A resize delivered while direct and proxy panes are both live re-renders the chrome at the new dimensions, recomputes both pane rectangles, and sends `terminal_set_viewport` with the new geometry through both sources; a burst of resize signals coalesces to one application of the latest size and a transient 0×0 observation changes nothing; the pane holding a write lease on a native row additionally issues exactly one `terminal_resize`, while an uncontrolled native pane and a tmux pane issue none — the tmux pane letterboxes instead. test: `crates/gclient/tests/client_loop.rs::live_resize_propagates_geometry_by_policy`.
...
attachment, await results, and call `Daemon::close(deadline)` (the trait member 2.1 adds
for exactly this step) with the same absolute deadline — unconditionally, even when the
detach phase has already exhausted the budget — then drop the runtime so the guard's
local restore happens last. The 2 s deadline is one **outer** bound over that whole
sequence, not a bound on the detach phase alone, and its fallback lives inside the close
contract rather than in a second cleanup subsystem: only `close` owns `LiveDaemon`'s
private reconnect, reader, and sink handles, so its first poll terminal-latches the
daemon, takes and synchronously aborts those handles, and fails and clears every
outstanding waiter before any await; its subsequent awaits only join already-owned work
within the remaining time. Dropping the `close` future after that first poll therefore
cannot leave a live reader, a live sink, or a later generation-ready publication behind,
and a deadline that expired before `close` was polled still gets the synchronous abort
because the call is unconditional. On expiry the phase proceeds directly to the
synchronous terminal restore; a panic skips the async phase by design and the
...
call exactly once — the invocation itself lives in `run_loop.rs` and is wired by 3.2
alongside the latch it owns, since only that deliverable can edit the loop carrier —
and quit, `SIGINT`, `SIGTERM`, `SIGHUP`, and reconnect-budget
...
- 4.3.8 - Resizing the real gclient PTY after startup (via the driver's window-size change delivering `SIGWINCH`) re-renders the workspace at the new dimensions with the pane content following — the screen read after the resize reflects the new geometry rather than the startup 120×40. test: `tests/e2e/test_terminal_client_stack.py::test_gclient_follows_a_live_pty_resize`.
...
predecessor, so this one key is its whole lookup. Finalization then writes the resolved
ref back into this plan: the uniquely created or adopted D1 task ref replaces
`#TBD-created-at-expansion` in D1's typed `deferral.task_ref` before expansion validation
runs, and the coordinator refreshes the plan hash and re-derives the manifest/coverage
state from the updated bytes. `validate_deferral` resolves the literal `task_ref`, so a
plan still carrying the placeholder fails expansion validation with `task_missing` even
after every task-graph postcondition holds.
...
- 5.1.5 - Before expansion validation runs, D1's typed `deferral.task_ref` names the exact stable ref of the created-or-adopted epic — no `#TBD` value survives into validation — and the plan hash and derived manifest/coverage state are refreshed from the written-back bytes, so `validate_deferral` resolves every deferral ref to an open task. behavior: D1 `deferral.task_ref` in this plan.
...
- reviewer_run: 52ca8974-fb2e-4e11-9c89-010b0a1a66a5
- reviewer_session: 2718f10b-93d0-4073-9199-1ffe2257cc28
- verdict: needs_review
- findings:
- R7-F01-terminal-input-stale-test-target / blocking / §1.1 deletes the run-id input branch while tests/servers/test_tmux_mixin.py::TestTerminalInputRouting still asserts it and is absent from Targets
- R7-F02-fragment-reassembly-lifetime-bounds / blocking / the §2.1 fragment reassembler lacks aggregate byte budget, incomplete-sequence cap, and fixed deadline
- R7-F03-proxy-finalized-before-result / blocking / terminal_attachment_finalized can be buffered before terminal_attach_result and install a retired attachment
- R7-F04-lifecycle-publisher-worker-lifecycle / blocking / the §1.4 publication queue's sole consumer has no start, stop, or failure-settlement owner
- R7-F05-daemon-close-deadline-cancellation / blocking / the §3.3 outer deadline can cancel Daemon::close, the only owner of the resources it promises to abort
- R7-F06-daemon-error-enum-parity / blocking / the normative DaemonError declaration omits ControlScopeIndeterminate required by 2.1.14 and §3.2
- R7-F07-hanger-single-owner / blocking / open hanger #21191 and the expansion-generated §1.1 leaf leave two open owners with no reconciliation step
- R7-F08-state-noncolor-cue / blocking / §3.1 permits state indicators distinguished by lightness alone against .impeccable.md's icon-or-position cue mandate
- resolution_notes: All 8 findings accepted by user vote. Typed repairs applied for R7-F01 (add_targets + add_acceptance) and R7-F08 (add_acceptance); prose fixes hand-applied for all eight per the accepted fix texts — fragment lifetime bounds reusing the terminalWsFragments contract, finalized-before-result tombstoning in the pre-attach receiver, SocketRelay-pattern worker lifecycle in TerminalLeaseRegistry, the shutdown deadline folded into Daemon::close, ControlScopeIndeterminate added to the DaemonError declaration, and the coordinator-owned pre-dispatch reconciliation closing #21191 as duplicate of the generated 1.1 leaf.

```json plan-review-round
{"evidence_id":"467d2688-9d28-47b7-a46d-7bfa87d3dca7","plan_hash":"bf5cc4517ba2a01d15bff9cb2e4d48ab4010dbdd06d738cc3404b30ab8085964","round_number":7,"round_result":{"coverage_attestation":{"adjacent_variant_complete":true,"attestation_digest":"25a9c7d702c23954ed5571ab80e7a9101deb3e3d14b42e2fe675dad8d46351b3","cross_lane_interaction_complete":true,"disposition_counts":{"dismissed":0,"emitted_findings":8,"total":8},"evidence_id":"467d2688-9d28-47b7-a46d-7bfa87d3dca7","lanes":[{"candidate_count":3,"lane_id":"requirements_traceability","status":"completed"},{"candidate_count":1,"lane_id":"repository_blast_radius","status":"delegated-verified"},{"candidate_count":4,"lane_id":"runtime_invariants","status":"completed"}],"shadow_manifest_status":{"entry_count":16,"manifest_digest":"742f90eaa0e01e1bb180b6ac3e42ddc1a82910d2658bdec6a18a643b3f3042d3","status":"valid"},"source_digest":"a6820fa75d756ff17b78013bc418560aceeadf288e01064bb127c86415db3e16","version":1},"findings":[{"category":"traceability","check_key":"terminal-input.legacy-test-consumer","description":"Section 1.1 deletes HandlerMixin._handle_terminal_input and its run_id-keyed lookup, while tests/servers/test_tmux_mixin.py::TestTerminalInputRouting still asserts that lookup and is absent from Targets and Guard set H. The generated leaf can therefore leave a stale or failing owned test behind.","finding_id":"R7-F01-terminal-input-stale-test-target","fix":"Target TestTerminalInputRouting, remove or rewrite the obsolete run-id lookup expectation, preserve the detached-tmux non-UUID regression in the planned attachment-scoped seam, and run the affected test class from the leaf's validation/Guard set H.","location":"§1.1 Targets, duplicate-handler retirement, and Guard set H","prevention":"For every deleted symbol or branch, search direct tests and mock/patch strings, then include each owned consumer in Targets and the leaf's focused validation.","principle":"Every owned test that directly asserts a deleted production branch must be targeted and reconciled by the deleting deliverable.","repairs":[{"entries":["`tests/servers/test_tmux_mixin.py::TestTerminalInputRouting`"],"kind":"add_targets","section_id":"1.1"},{"items":[{"artifact":"test: `tests/servers/test_tmux_mixin.py::TestTerminalInputRouting`","prose":"Retiring the run-id legacy input branch rewrites or removes its existing TestTerminalInputRouting expectation while preserving the detached-tmux non-UUID regression against the surviving attachment-scoped handler"}],"kind":"add_acceptance","section_id":"1.1"}],"root_cause":"The consumer sweep followed the surviving handler and new regression seam but missed the existing TestTerminalInputRouting class that asserts the run-id lookup being removed.","section_id":"1.1","severity":"blocking"},{"category":"unhandled-edge","check_key":"ws-fragment.aggregate-lifetime-bounds","description":"The planned map keyed by (attachment_id, message_seq) can retain unlimited incomplete assemblies when final slices never arrive. Per-message size validation does not bound aggregate payload or metadata, and the plan only clears partials on attachment finalization or generation teardown.","finding_id":"R7-F02-fragment-reassembly-lifetime-bounds","fix":"Reuse the existing terminalWsFragments contract in the sole Rust reader: 16 MiB per assembly, 64 MiB per generation/socket, one incomplete message_seq per live attachment, and a fixed non-sliding 5 s deadline. Charge retained payload plus bounded metadata before append; release accounting on completion, sequence error, timeout, finalization, lag, cancellation, close, and generation teardown; add paused-clock multi-attachment saturation coverage.","location":"§2.1 LiveDaemon reader fragment reassembler; consumers in §§2.2 and 3.2","prevention":"For each retained protocol buffer, check per-item bytes, aggregate bytes, concurrent-key count, fixed expiry, and cleanup on success, error, cancellation, lag, close, and generation loss.","principle":"Every incremental decoder needs both per-message and aggregate lifetime bounds, with accounting released on every terminal path.","root_cause":"Round 6 centralized reassembly ownership and bounded reconstructed size, but omitted the existing socket-wide byte budget, fixed timeout, and one-incomplete-sequence-per-attachment rule.","section_id":"2.1","severity":"blocking"},{"category":"unhandled-edge","check_key":"proxy-attach.finalization-before-result","description":"ProxyHub can finalize and publish terminal_attachment_finalized before terminal_attach_result when the newly started pump sees immediate EOF, protocol failure, or terminal exit. The client plan buffers early frames but defines no outcome for an already-buffered finalization, so it can install a retired attachment and issue stale viewport/control work.","finding_id":"R7-F03-proxy-finalized-before-result","fix":"Have the existing pre-attach receiver preserve finalization in wire order. If finalization for the returned attachment_id is buffered before or observed during ProxyFrameSource construction, tombstone that id, return the reason as the source's typed finalized failure, keep the pane Detached/read-only, and issue no viewport, control, or detach for the retired id. Extend the existing three attach schedules with finalization.","location":"§1.3 ProxyHub pump ordering, §2.2 pre-attach receiver, and §3.2 attach state machine","prevention":"For every subscribe-before-request handshake, test state boundary, refusal, and finalization before the reply, between settlement and construction, and immediately after construction.","principle":"A correlated success reply cannot install a resource that an earlier buffered lifecycle event has already finalized.","root_cause":"The pre-result buffering repair covered history and keyframes but did not walk the adjacent terminal_attachment_finalized event emitted by the same already-running pump.","section_id":"2.2","severity":"blocking"},{"category":"unhandled-edge","check_key":"lifecycle-publisher.worker-lifecycle","description":"The ordered lifecycle queue has no defined point that starts its consumer on a running loop, closes it during server cleanup, or settles queued/future submitters if the worker faults. An admitted event can therefore wait forever, committed high-water can stall, and daemon shutdown can abandon the worker.","finding_id":"R7-F04-lifecycle-publisher-worker-lifecycle","fix":"Reuse the existing SocketRelay-style start/shutdown pattern inside TerminalLeaseRegistry and call it from the existing WebSocket server init/cleanup seams. Start before admission; on close fence new submissions, deterministically drain or fail every admitted completion, cancel and await the sole worker, and leave no task behind; on worker exception fail queued/future submitters with a typed publication error and perform bounded relay cleanup. Test clean stop, paused/full-queue stop, post-admission submitter cancellation, and injected worker failure.","location":"§1.4 process-wide ordered publication queue and WebSocket server init/cleanup","prevention":"For every long-lived async worker, name its start owner, stop owner, admission fence, drain/fail policy, exception settlement, and leak test.","principle":"A queue that owns admitted work must have an explicit consumer lifecycle and must settle every admitted waiter on stop or worker failure.","root_cause":"The plan specifies queue admission, backpressure, and publication acknowledgment while leaving the sole consumer's startup, shutdown, and exception owner implicit in a synchronously and lazily constructed registry.","section_id":"1.4","severity":"blocking"},{"category":"unhandled-edge","check_key":"daemon-close.outer-deadline-cancellation","description":"If the 2 s outer deadline expires while Daemon::close awaits a reconnect join, reader shutdown, or sink close—or before close is polled after detach consumed the budget—the caller has no surface that can perform the promised abort/drop. Detached tasks can survive exit and later publish a reader, sink, or ready generation.","finding_id":"R7-F05-daemon-close-deadline-cancellation","fix":"Move the deadline fallback into the existing close contract rather than adding a second cleanup subsystem: pass close the same absolute deadline, call it unconditionally even when detach exhausted the budget, and have its first poll terminal-latch the daemon, take and synchronously abort private handles, and fail/clear waiters before any await; subsequent awaits only join already-owned work within the remaining time. Extend the existing teardown test to expire at each close await and prove no resource or generation-ready publication survives.","location":"§2.1 Daemon::close contract versus §3.3 single outer shutdown deadline","prevention":"At every timeout boundary, identify who still owns each private task, waiter, and socket after the awaited future is cancelled, and test cancellation at each internal await.","principle":"A timeout cannot cancel the only operation that owns the resources the timeout path promises to abort.","root_cause":"The generic shutdown path promises to abort LiveDaemon's private reconnect, reader, and sink resources after the outer deadline, while the trait exposes only an async close future that the same deadline can drop.","section_id":"3.3","severity":"blocking"},{"category":"unhandled-edge","check_key":"daemon-error.control-scope-parity","description":"DaemonError declares ControlRequestInFlight and Timeout but omits ControlScopeIndeterminate, even though 2.1.14 requires that exact distinct variant and 3.2 consumes it to retire the attachment. A section-only implementer receives contradictory API instructions.","finding_id":"R7-F06-daemon-error-enum-parity","fix":"Add ControlScopeIndeterminate to the DaemonError declaration and state beside the interface that it begins once a control send enters the write_started indeterminate region, persists for that attachment, and clears only when the attachment is finalized or retired. Keep 2.1.14 and 3.2.12 aligned to that same variant.","location":"§2.1 normative DaemonError code block versus acceptance 2.1.14 and §3.2 consumer","prevention":"After introducing a typed outcome, search the section's interface declaration, producer paths, acceptance items, and every downstream match arm for exact enum parity.","principle":"A section's normative interface must declare every typed outcome its own acceptance and downstream consumers require.","root_cause":"The control-scope tombstone repair added DaemonError::ControlScopeIndeterminate in prose and acceptance but left the authoritative enum declaration unchanged.","section_id":"2.1","severity":"blocking"},{"category":"missing-requirement","check_key":"task-hanger.single-owner-reconciliation","description":"Live task #21191 remains open with no commits, and expansion will create a separate 1.1 task for the same raw-input obligation. The plan never closes, adopts, or links the hanger, leaving two open owners and a possible double dispatch.","finding_id":"R7-F07-hanger-single-owner","fix":"Add a coordinator-owned post-expansion, pre-dispatch reconciliation: resolve the generated 1.1 leaf, close #21191 as its duplicate with a backlink, and verify exactly one open task owns the raw-input obligation before dispatch. Record the transition in the expansion handoff rather than deferring it to the implementation leaf.","location":"Constraints hanger #21191, §1.1, expansion handoff, and Task Mapping","prevention":"For every open task named as folded into a plan, trace expansion behavior and specify adoption or duplicate closure plus a pre-dispatch exactly-one-open-owner check.","principle":"Folding an open hanger into a manifest leaf requires an explicit single-owner transition before either task can dispatch.","root_cause":"The plan names #21191 as folded into P1 while expansion unconditionally creates a new leaf for 1.1 and no adoption or duplicate-close step reconciles the live task.","section_id":"1.1","severity":"blocking"},{"category":"weak-testability","check_key":"design-state.noncolor-cue","description":"Section 3.1 permits info, warning, destructive, success, and control states to differ only by lightness. The governing .impeccable.md contract requires every state-bearing element to include an icon or positional cue, so distinct grayscale ranks alone are insufficient.","finding_id":"R7-F08-state-noncolor-cue","fix":"Extend the existing theme/chrome render test—without a new framework—to assert a non-color icon or positional cue for every rendered info, warning, destructive, success, and control-state indicator in both dark and light themes, and prove those cues remain distinguishable in desaturated output.","location":"§3.1 theme prose and acceptances 3.1.5/3.1.9","prevention":"For each semantic state in both themes, inventory the rendered indicator and assert its non-color cue in a desaturated representative render.","principle":"Every state-bearing indicator must carry meaning through an icon or positional cue and remain distinguishable without color.","repairs":[{"items":[{"artifact":"test: `crates/gclient/tests/theme.rs::tokens_match_design_contract_and_survive_monochrome`","prose":"Every rendered info, warning, destructive, success, and control-state indicator includes a non-color icon or positional cue and remains distinguishable in desaturated dark and light representative renders"}],"kind":"add_acceptance","section_id":"3.1"}],"root_cause":"The plan translated the design contract into grayscale rank checks and retained a position cue only for keyboard focus, omitting the contract's cue requirement for all other state elements.","section_id":"3.1","severity":"blocking"}],"reviewer_session":"2718f10b-93d0-4073-9199-1ffe2257cc28","round":7,"verdict":"needs_review"},"session_id":"5dcb338e-df86-4030-bb4c-c566efc62d44"}
```

**Round 8** `kind: verification`

- reviewer_run: d9626d4a-a6cb-42fc-8e01-9ad242b68068
- reviewer_session: #11257
- verdict: needs_review
- findings:
- R8-F01-d1-task-ref-finalization / blocking / D1 finalization never writes the resolved task ref over `#TBD-created-at-expansion`, so `validate_deferral` still returns `task_missing`
- R8-F02-proxy-start-target / blocking / §1.3 Targets omit `TerminalWsMixin._start_proxy_attach`, the exact helper that must forward the negotiated encoding into `ProxyHub.start_proxy`
- R8-F03-lifecycle-owner-targets / blocking / §1.4 Targets omit `WebSocketServer.start`/`stop`, the seams that must start and fence the publication worker
- R8-F04-shutdown-run-loop-target / blocking / the `shutdown(workspace, daemon, deadline)` invocation lives in `app/run_loop.rs`, which §3.3 cannot edit under its Targets
- R8-F05-page-byte-cap-forward-progress / blocking / `encode_page` truncation emits a bare id where `parse_list_cursor` requires `<created_at>|<terminal_id>`, indexes an `id` key absent from WS inventory rows, and returns an empty cursorless page for an oversized first row
- R8-F06-direct-locator-validation / blocking / optional `AttachLocator` fields let a backend-incoherent or partial locator serialize as `success: true` with an unusable direct block
- R8-F07-lifecycle-restart / blocking / `WebSocketServer` supports start→stop→start but §1.4 closes the publication worker terminally with no defined later-start behavior
- R8-F08-publication-admission-close-race / blocking / a capacity-blocked pre-admission submitter is in neither covered close class: concurrent close can admit it behind the fence or strand it forever
- R8-F09-reconnect-generation-rollover / blocking / an active G1 reconnect episode has no defined disposition when replacement generation G2 drops mid-handshake
- R8-F10-live-resize-path / blocking / the §3.2 loop has no live-resize branch and the native controlled-pane geometry policy is unrecorded
- R8-F11-writer-cancellation-acceptance / blocking / §2.2 prose promises writer-task retirement on cancellation but acceptance aborts only the reader
- resolution_notes: All 11 findings accepted (11/11 by user vote). Five factual premises were verified against HEAD before the vote: the D1 placeholder at the typed deferral; `_start_proxy_attach` calling `start_proxy` without encoding (terminal_ws.py:748); `WebSocketServer.start`/`stop` absent from all Targets; all three `encode_page` truncation defects live in committed code (ws_protocol.py:524-538); and `stop()` nulling `_server` so a later `start()` re-runs fully. Typed repairs applied via apply_plan_review_repairs for R8-F02, R8-F03, and R8-F11. R8-F04's typed repair was deliberately not auto-applied: consistent with round 1's exit-latch-target-owner decision, the shutdown-invocation wiring moves into §3.2 — which already owns `run_loop.rs`, the exit latch, and every loop-exit path — instead of widening §3.3 into a shared-target edge across the P3 chain; §3.3 keeps `Daemon::close(deadline)` implementation ownership. R8-F07 takes the explicit restartable-worker shape by user vote: the registry worker restarts in place across server stop/start with start-stop-start acceptance. R8-F10 takes the herdr-parity shape by user direction (goal parity+): herdr v0.8.0 runs a SIGWINCH-flag + 100ms coalescing resize watcher and propagates client geometry to the host PTY (client/mod.rs:1638→2516, pane.rs:2596), so a controlled native pane sends lease-gated `terminal_resize` while tmux observer geometry stays host-owned; transient 0×0 sizes are ignored and the Ratatui surface, pane rects, and per-source SetViewport all follow the coalesced branch. Remaining accepted fixes (R8-F01, R8-F05, R8-F06, R8-F08, R8-F09) were hand-applied as prose, contract, and acceptance revisions. R8-F05's committed-code cursor defect folds into §1.4, which reworks exactly that helper — the same disposition pattern recorded for #21191.

```json plan-review-round
{"evidence_id":"beb9e6e9-f6c6-4480-8454-6c1d9f377dd2","plan_hash":"d5f8448330c8850eb978981ad2838987b6863bc5791bfddfcb3a96c308e0b3af","round_number":8,"round_result":{"coverage_attestation":{"adjacent_variant_complete":true,"attestation_digest":"d330482a59bacb8ab5a4f43b9b7055abef4df1ccdca4bfcb6ef10fce73fed4b8","cross_lane_interaction_complete":true,"disposition_counts":{"dismissed":1,"emitted_findings":11,"total":12},"evidence_id":"beb9e6e9-f6c6-4480-8454-6c1d9f377dd2","lanes":[{"candidate_count":1,"lane_id":"requirements_traceability","status":"completed"},{"candidate_count":3,"lane_id":"repository_blast_radius","status":"delegated-verified"},{"candidate_count":8,"lane_id":"runtime_invariants","status":"completed"}],"shadow_manifest_status":{"entry_count":16,"manifest_digest":"65fb5c65032b8282c6e97b58cff86cd82d5733563b48799bb6a59ef5e5efbaaf","status":"valid"},"source_digest":"25fa6bed3cc34bfc7460a2b840d6b02ea8257ba1c8e46584455dfde775d1abff","version":1},"findings":[{"category":"missing-requirement","check_key":"deferral.task-ref-finalization","description":"D1 still declares `task_ref: \"#TBD-created-at-expansion\"`. Section 5.1 can create the real planning task and satisfy provenance, parenting, and dependency checks while `validate_deferral` still resolves the literal placeholder and returns `task_missing`.","finding_id":"R8-F01-d1-task-ref-finalization","fix":"Extend finalization so the uniquely created or resolved D1 task ref replaces the placeholder in D1 before expansion validation; then refresh the plan hash, re-derive the manifest/coverage state, and add acceptance requiring the exact stable ref with no TBD value.","location":"§5.1 D1 finalization algorithm and D1 typed deferral","prevention":"For every placeholder deferral, trace the creation result back into task_ref, refresh the plan hash and manifest, then run expansion validation.","principle":"A deferral entering expansion validation must name the actual task that owns its obligation.","root_cause":"The manual coordinator algorithm mutates the task graph by provenance but never writes the resolved D1 reference back into the plan's typed deferral.","section_id":"5.1","severity":"blocking"},{"category":"traceability","check_key":"targets.proxy-start-encoding-caller","description":"`TerminalWsMixin._start_proxy_attach` currently invokes `ProxyHub.start_proxy` without the negotiated encoding. Section 1.3 requires that encoding to reach `start_proxy` but omits this exact helper from Targets.","finding_id":"R8-F02-proxy-start-target","fix":"Add the helper to §1.3 Targets and state that it accepts or derives the validated encoding and forwards it through the real proxy-start call; retain the existing native proxy-path test.","location":"§1.3 Targets and TerminalWsMixin._handle_terminal_attach → _start_proxy_attach → ProxyHub.start_proxy","prevention":"Run a caller sweep for each exact symbol Target and inventory every intermediate caller that must forward a new argument.","principle":"Every production caller whose signature or data flow changes must appear in the owning deliverable's Targets.","repairs":[{"entries":["`src/gobby/servers/websocket/terminal_ws.py::TerminalWsMixin._start_proxy_attach`"],"kind":"add_targets","section_id":"1.3"}],"root_cause":"The plan targeted the attach handler and ProxyHub.start_proxy while skipping the helper that currently constructs the start_proxy call without encoding.","section_id":"1.3","severity":"blocking"},{"category":"traceability","check_key":"targets.lifecycle-worker-server-carriers","description":"Section 1.4 requires the lifecycle publication worker to start before submissions and be fenced, cancelled, and awaited during server cleanup, yet Targets omit `WebSocketServer.start` and `WebSocketServer.stop`.","finding_id":"R8-F03-lifecycle-owner-targets","fix":"Target both server lifecycle methods; start the registry worker on the running loop before admission and stop it by fencing submissions and awaiting registry shutdown.","location":"§1.4 Targets versus WebSocket server init and cleanup seams","prevention":"For every background worker, target the constructor/start seam, shutdown seam, and their lifecycle tests.","principle":"A long-lived worker's start and stop owners must be explicit, targeted production carriers.","repairs":[{"entries":["`src/gobby/servers/websocket/server.py::WebSocketServer.start`","`src/gobby/servers/websocket/server.py::WebSocketServer.stop`"],"kind":"add_targets","section_id":"1.4"}],"root_cause":"The worker lifecycle was specified against server initialization and cleanup without inventorying the server methods that implement those transitions.","section_id":"1.4","severity":"blocking"},{"category":"traceability","check_key":"targets.shutdown-run-loop-carrier","description":"Every non-panic exit must enter `shutdown(workspace, daemon, deadline)` exactly once. That invocation belongs in `app/run_loop.rs`, but §3.3 cannot edit the file under its current Targets.","finding_id":"R8-F04-shutdown-run-loop-target","fix":"Add `app/run_loop.rs` to §3.3 Targets and wire each non-panic loop exit through the shutdown phase while keeping §3.2's latch and request-suppression ownership unchanged.","location":"§3.3 Targets and §3.2 app/run_loop.rs ownership","prevention":"Trace every new shutdown or recovery function to its production call site and target that caller in the dependent deliverable.","principle":"The deliverable that installs a state transition must target the carrier that invokes it.","repairs":[{"entries":["`crates/gclient/src/app/run_loop.rs`"],"kind":"add_targets","section_id":"3.3"}],"root_cause":"Section 3.2 creates the run loop and exit latch while §3.3 owns the concrete shutdown call, but §3.3 never reopens the loop carrier.","section_id":"3.3","severity":"blocking"},{"category":"unhandled-edge","check_key":"pagination.byte-cap-forward-progress","description":"Current byte-cap truncation emits a bare id although `parse_list_cursor` requires `<created_at>|<terminal_id>`, indexes an `id` field absent from WS inventory rows, and turns an oversized first row into an empty terminal page with no cursor. Section 1.4 adds snapshot bytes to this envelope without defining or testing those branches.","finding_id":"R8-F05-page-byte-cap-forward-progress","fix":"Extend §1.4 so truncation budgets the complete WS/REST envelope, preserves the canonical cursor for the last included raw row, and returns a typed failure when one row cannot fit. Add exact-bound and bound-plus-one tests for both transports and a §2.1 traversal test proving no loop or lost row.","location":"§1.4 encode_page changes and §2.1 roster traversal","prevention":"Test empty, exact-bound, bound-plus-one, oversized-single-row, and multi-page traversal for every paginated envelope.","principle":"Every nonterminal page must return a parser-valid cursor and either advance or fail explicitly.","root_cause":"The plan treats cursor parsing as unchanged and never walks the encoded-byte overflow branch already present in the helper it edits.","section_id":"1.4","severity":"blocking"},{"category":"unhandled-edge","check_key":"direct-attach.locator-completeness","description":"A runtime can return an `AttachLocator` instance with missing native fields, a partial tmux pane, or fields for the wrong backend and still receive `success: true` with an unusable direct block.","finding_id":"R8-F06-direct-locator-validation","fix":"Validate the expected backend and every required nonempty field before serializing `direct`: native requires epoch, host socket, and host terminal id with no pane; tmux additionally requires one complete pane locator. Map malformed or mismatched results to existing `locator_invalid`, finalize the lease, and test malformed native and tmux rows.","location":"§1.3 direct block construction and acceptance 1.3.3","prevention":"For each tagged or backend-specific payload, test missing, empty, partial, and mismatched field combinations before success serialization.","principle":"A successful attach result must contain a backend-coherent locator that its consumer can use.","root_cause":"`AttachLocator` fields are optional, while the plan validates only raised locator failures and assumes every returned object is complete.","section_id":"1.3","severity":"blocking"},{"category":"unhandled-edge","check_key":"lifecycle-worker.restart-transition","description":"`WebSocketServer` supports a later `start()` after `stop()`, but §1.4 closes the registry worker terminally and never says whether the registry is recreated or restartable. The second server run can reject every lifecycle publication or retain a stale worker.","finding_id":"R8-F07-lifecycle-restart","fix":"Define one restart policy in §1.4—recreate the registry at the later start or make its worker explicitly restartable—and extend lifecycle acceptance with start-stop-start proving the second run publishes and shuts down cleanly.","location":"§1.4 publication worker start/cleanup lifecycle","prevention":"For every restartable owner, exercise start, stop, start again and verify fresh admission, publication, and cleanup.","principle":"A supported start-stop-start transition must not retain terminally closed worker state.","root_cause":"The plan copies a terminal `SocketRelay`-style shutdown pattern into a registry object that survives `WebSocketServer.stop`, without defining later-start behavior.","section_id":"1.4","severity":"blocking"},{"category":"unhandled-edge","check_key":"lifecycle-publication.preadmission-close-race","description":"With 256 entries queued, submitter 257 waits for capacity. Concurrent close can free space and let that waiter enqueue behind the fence, or leave it pending forever, because §1.4 defines no atomic closed-latch recheck or wakeup for pre-admission waiters.","finding_id":"R8-F08-publication-admission-close-race","fix":"Make the existing admission operation race capacity against a close notification and recheck the closed latch before insertion; close must wake every pre-admission waiter with the typed publication error. Add the full-queue/submitter-257/concurrent-close schedule to acceptance.","location":"§1.4 bounded queue admission and worker close","prevention":"Race close against an empty queue, full queue, blocked producer, admitted producer, and cancelled producer.","principle":"A close fence must settle every producer and prevent admission after the fence.","root_cause":"The plan specifies ownership after queue insertion and future submissions after close, leaving a capacity waiter blocked before admission in neither class.","section_id":"1.4","severity":"blocking"},{"category":"unhandled-edge","check_key":"reconnect.generation-rollover","description":"If G1 reconnects to G2 and G2 drops before Workspace finishes its roster handshake, the active G1-tagged episode receives a G2 intent with no defined disposition. It can spend no-op G1 attempts, start a second budget, or stall.","finding_id":"R8-F09-reconnect-generation-rollover","fix":"Keep one supervisor and roll the active episode's observation forward to G2 for its next attempt, coalescing all waiters while preserving the current attempt count and delay schedule until install/replay succeeds. Add a paused G2-drops-during-G1-handshake test.","location":"§3.2 ReconnectSupervisor and §2.1 LiveDaemon reconnect cases","prevention":"For each recovery episode, disconnect before connect, after connect, during roster fetch, during replay, and immediately after install.","principle":"One disconnected episode retains one retry budget until roster reconciliation succeeds or the budget exhausts.","root_cause":"Supervisor coalescing is defined only for intents naming the same observed generation and never advances an unfinished episode when its replacement generation drops.","section_id":"3.2","severity":"blocking"},{"category":"missing-requirement","check_key":"run-loop.live-terminal-resize","description":"After startup, resizing the client PTY can leave Ratatui chrome and both direct/proxy viewports at stale dimensions. Requirements context also leaves one policy unanswered: whether a controlled native pane sends lease-gated `terminal_resize` or remains viewport-only; tmux observer geometry must remain host-owned.","finding_id":"R8-F10-live-resize-path","fix":"Add one coalesced live-resize branch to the existing loop that ignores transient 0×0 sizes, resizes the Ratatui surface, recomputes pane rectangles, and sends `SetViewport` through each live direct/proxy source. Record the native controlled-pane PTY policy explicitly, forbid implicit tmux mutation, add direct/proxy loop coverage, and resize the real §4.3 PTY after startup.","location":"§3.2 select loop, §2.2 frame-source viewport, and §4.3 PTY driver","prevention":"For every TUI loop, inventory keyboard, resize, suspend/resume, signal, transport, and timer inputs and drive each through a real PTY.","principle":"A long-running terminal UI must observe live size changes and propagate a decision-complete geometry policy.","root_cause":"The selected raw-input surface drops resize events, the loop names no SIGWINCH or size-poll branch, and all acceptance keeps the PTY at startup geometry.","section_id":"3.2","severity":"blocking"},{"category":"weak-testability","check_key":"framing.writer-cancellation-coverage","description":"A writer cancelled after part of the length prefix or payload can leave a framed stream desynchronized while all named acceptance still passes.","finding_id":"R8-F11-writer-cancellation-acceptance","fix":"Add writer-prefix and writer-payload cancellation schedules proving the source becomes unusable, both socket halves close, `FrameError::Cancelled` surfaces at the surviving source boundary, and no later read or write is attempted. Reuse the existing codec design.","location":"§2.2 cancellation prose and acceptance 2.2.9","prevention":"Mirror every partial-read cancellation schedule on the framed writer and assert the same retirement invariant.","principle":"Every promised cancellation boundary needs an acceptance artifact on both read and write directions.","repairs":[{"items":[{"artifact":"test: `crates/gterminal/tests/wire_codec_async.rs::async_writer_cancellation_retires_connection`","prose":"Cancelling the source-owned writer after a partial length prefix and after a partial payload retires the connection in both schedules, closes both socket halves, surfaces FrameError::Cancelled at the source boundary, and permits no later read or write"}],"kind":"add_acceptance","section_id":"2.2"}],"root_cause":"The prose assigns retirement to source-owned reader and writer tasks, while acceptance aborts only the reader after partial prefix and payload reads.","section_id":"2.2","severity":"blocking"}],"reviewer_session":"#11257","round":8,"verdict":"needs_review"},"session_id":"5dcb338e-df86-4030-bb4c-c566efc62d44"}
```

## Task Mapping
`kind: framing`

Post-expansion, pre-dispatch reconciliation (coordinator-owned): after expansion
creates the 1.1 leaf, close open hanger #21191 as its duplicate with a backlink to

...
server's existing init and cleanup seams: init starts the worker on the running loop
before any submission is admitted, and cleanup closes it — close fences new
submissions, deterministically drains or fails every already-admitted completion,
cancels and awaits the sole worker, and leaves no task behind. If the worker itself

...
**The search floor had the same defect (#20858).** `min_score` is never passed to Qdrant; `build_results` (`_search_results.py:76`) applied it to `similarity`, the same decayed product, so every threshold in the table above was a recency test at the retrieval stage too. At the live corpus median age of 25.9 days (mean 36.6, n=2,705) the decay factor is 0.549, so clearing 0.55 demanded a cosine of 1.002 -- unreachable at any cosine, and 0.835 even with the 1.2 user boost. Measured on the epic's own smoke query, `search_memories(limit=8, min_score=0.55)` returned three candidates out of a merged pool of ~48, all three with `temporal_decay_factor >= 0.84`. The slots it emptied went to keyword-only hits, which the guard's `similarity is not None` term exempts and which 2.3 makes permanently injection-ineligible: rows with `ranking_mode='rrf'` but no `semantic` in `search_via` went from 0 through 08-12 to 23% of returned hits by 08-24, while cosine coverage fell 88% -> 54.6% against a FLAT corpus (2,668 -> 2,705). The search floor now reads the undecayed score through the same `undecay` helper as `selection_min_score`, so both floors share one axis and one implementation. **The value stays 0.55 and every number in the table above is now off-axis for it**; injection volume is still governed by `selection_min_score`, which this does not touch. Phase 4 re-fits both constants together on the stable axis.

**Retrieval no longer erases a knowable cosine (#20858).** The semantic leg fetches the top `candidate_limit * _OVERFETCH_FACTOR` by raw cosine -- 16 at the recall width of 8. A memory another leg surfaced from below that window arrived with `raw_semantic_score` erased and a fabricated graph-synthetic similarity in its place, which 2.3's null-similarity rule then made ineligible. On the smoke query the answering memory ranks in (100, 150] of 2,803 by raw cosine, and its invented 0.5258 stood in for a real 0.6002. Merged candidates the window missed are now scored against the query vector in one extra Qdrant round trip, skipped when the semantic leg itself failed. `graph_synthetic` consequently means what it says -- no vector to score -- and the #17104 expander keeps exactly the memories it was built for. `search_via` still names the leg that found the memory.

...
Expose a citeable Git commit-metadata evidence item containing parent OIDs, the selected comparison parent (first parent, or an explicit empty-tree comparison for a root commit), and canonical changed-path records with status, old/new paths and blob identities. Compute it from exact Git objects, including merge commits and renames; never infer change inventory from live refs or empty default merge diffs. Page the item/records honestly within evidence bounds. Metadata claims cite its stable identity and are validated against the same commit comparison; source-text citations still require exact current-commit ranges. This supports generic commit-change questions without a second mutable source snapshot or answer-key material.

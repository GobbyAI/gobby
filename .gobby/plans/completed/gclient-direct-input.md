...
- `tests/terminals/test_composition_roots.py::*` — scope-reason: gains the assertion that configure_terminals installs the input activity sink on the host manager next to the existing composition checks
...
- Observed wiring: `init_terminal_wiring` (`terminal_wiring.py`) builds the lease
  ...
  only constructor of `TerminalHostManager`. The turn observer is built on the
  WebSocket server (`src/gobby/servers/websocket/server.py:166`,
  `TerminalTurnObserver(session_manager, lifecycle)`) and every consumer reaches it via
  `getattr(owner, "terminal_turn_observer")`; `WebSocketServer.configure_terminals`
  (`server.py:224`) receives the host manager and the write coordinator after
  construction, so it is the one composition point that sees all three.
...
- Events: `InputActivityEvent(terminal_id, host_terminal_id, attachment_id, kind,
  ...
  named for the size ceiling) and give the manager
  `set_input_activity_sink(Callable[[InputActivityEvent], None] | None)` with its
  constructor unchanged; exited events keep calling
...
- `init_terminal_wiring` sets the holder observer (runtime resolved from the registry's
  native runtime). `WebSocketServer.configure_terminals` installs the sink on the host
  manager it already receives: the sink calls
  `self.terminal_turn_observer.record_mediated_input(terminal_id,
  "\x1b" if interrupt == "esc" else "\x03" if interrupt == "ctrl_c" else "", "delivered")`
  then `write_coordinator.observe_operator_input(terminal_id)`.
...
- `src/gobby/app_context.py` — no-edit-reason: TerminalLeaseRegistry keeps its no-argument constructor and the take_control, release_control and finalize signatures; the holder observer is installed only by init_terminal_wiring and ControlResult.host_input_granted defaults to None; TerminalHostManager gains set_input_activity_sink, keeps its constructor, and the event reader split keeps its public methods; WriteCoordinator gains observe_operator_input as an additive method and its constructor and write signatures are unchanged.
- `src/gobby/runner.py` — no-edit-reason: TerminalHostManager gains set_input_activity_sink, keeps its constructor, and the event reader split keeps its public methods.
...
- `src/gobby/runner_init/servers.py` — no-edit-reason: Calls configure_terminals with the same arguments; the sink is built inside it from the host manager and write coordinator already passed.
...
- `tests/mcp_proxy/test_workspaces_registry.py` — no-edit-reason: TerminalLeaseRegistry keeps its no-argument constructor and the take_control, release_control and finalize signatures; the holder observer is installed only by init_terminal_wiring and ControlResult.host_input_granted defaults to None; WriteCoordinator gains observe_operator_input as an additive method and its constructor and write signatures are unchanged; configure_terminals keeps its signature because the sink is built from the host manager and write coordinator it already receives.
...
- `tests/terminals/acceptance/conftest.py` — no-edit-reason: NativeTerminalRuntime gains grant_input and revoke_input as additive methods and every existing method keeps its signature; TerminalHostManager gains set_input_activity_sink, keeps its constructor, and the event reader split keeps its public methods.
...
- `tests/terminals/test_host_shutdown_preservation.py` — no-edit-reason: TerminalHostManager gains set_input_activity_sink, keeps its constructor, and the event reader split keeps its public methods; FakeControlClient gains grant_input and revoke_input handlers while its existing verbs and recorded call shapes stay the same.
...
- `tests/terminals/test_runtime_contract.py` — no-edit-reason: NativeTerminalRuntime gains grant_input and revoke_input as additive methods and every existing method keeps its signature; TerminalHostManager gains set_input_activity_sink, keeps its constructor, and the event reader split keeps its public methods.
...
- `tests/test_runner_lifecycle_processes.py` — no-edit-reason: TerminalHostManager gains set_input_activity_sink, keeps its constructor, and the event reader split keeps its public methods.
...
- 1.2.6 - The sink arms or pops turn-interrupt candidates and lifts the automatic-write quarantine. symbol: `WriteCoordinator`. file: `src/gobby/servers/websocket/server.py`. test: `tests/terminals/test_composition_roots.py::test_configure_terminals_installs_input_activity_sink`.
...
**Granularity:** nine items and eleven production files, kept as one leaf because the

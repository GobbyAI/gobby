...
- Claude/Codex `SessionStart(source=compact)` handoff injection (`inject-compact-handoff`)
  is unchanged. `wait_for_summary`'s existing staleness behaviour from #20393
  (`summary_is_stale` → `live_handoff_context`) is preserved.
- Grok delivery — the compact continuation and every other `inject_context` effect — is
  owned by plan `grok-hook-deferred-materialization` (planning epic #20916). Nothing
  here touches Grok hooks, the `wait_for_summary` response shape, or the `turn_start`
  handoff rules.
...
**Supersession — 2026-08-25** `kind: verification`

- P2 (2.1–2.3, Grok compact handoff delivery through `wait_for_summary`) and §3
  (Grok-wide context channel, deferral carrier #20635) are superseded by plan
  `grok-hook-deferred-materialization` (planning epic #20916), which delivers Grok
  context through the channels Grok reads — PreToolUse deny reason and
  Stop/SubagentStop block + `additionalContext` — after first-activity session
  materialization. Both sections, their M1 entries, and their ledger rows are removed;
  leaves #20733–#20735 and sub-epic #20727 closed `obsolete`; #20635 closed `duplicate`
  of that plan. This plan now lands on P1 alone.

...
- enhancer_run: 028f4d5a-965f-4c2e-b62a-eaf77606d8df
- enhancer_session: 54f7be4d-79cf-4c01-b4b8-a2494804c0b6
- converged: false
- suggestions_presented: 6
- accepted:
  - E1 / better / reject the three removed top-level agent step keys instead of letting `extra="ignore"` drop them silently
  - E2 / better / prove baseline-vs-migration DDL parity by catalog comparison rather than text snippets
  - E3 / better / post-copy payload equivalence guard on every domain copy migration
  - E4 / better / fail closed when step-instance persistence fails at spawn or persona activation
  - E5 / better / widen the standing audit to baseline SQL and bundled sources with a self-pruning allowlist
  - E6 / better / structured warning when restart recovery rebuilds a snapshot from a newer definition
- declined: none
- resolution_notes: >-
  All six accepted and folded in. E1 added a `mode="before"` validator
  requirement to 2.1 plus acceptance 2.1.6 and write-path acceptance 2.3.7.
  E2 rewrote 1.1's test paragraph, kept snippet checks as diagnostics, split
  acceptance into 1.1.3 (diagnostics) and new 1.1.4 (catalog equivalence).
  E3 was specified once in Constraints as the shared equivalence-guard
  contract and referenced from each copy migration, with acceptance items
  2.3.6, 3.2.7, 4.1.6, 4.2.5, 4.3.6. E4 added the fail-closed persistence
  boundary to 3.2 with acceptance 3.2.8 and a seventh fault-injection
  behavior in 3.4. E5 rewrote 7.2's scope and allowlist shape, added the
  baseline SQL and `src/gobby/install/shared/` to its Targets, and added
  acceptance 7.2.2. E6 extended `_ensure_step_instance` in 3.3 with one
  structured warning, acceptance 3.3.6, and an E1 restart-scenario check.
  `uv run gobby plans validate` passes including the consumer sweep.

**Round 1** `kind: adversary`

- evidence_id: 631cf584-6996-4e89-af4a-88043e3a039c
- plan_hash: edceb032b3f9eacf75761dd25ab8d38507360520e138fe9ffb84158e44e702ce
- adversary_run: 9381b691-ce44-47a0-8272-5f93799c046c
- reviewer_session: ddd73329-5443-4e44-a600-50dcad5d6a33
- provider/model/reasoning: codex / gpt-5.6-sol / xhigh (applied)
- verdict: needs_review
- coverage_attestation: 3/3 lanes completed (requirements_traceability 12,
  repository_blast_radius 16, runtime_invariants 12 candidates); 40 candidates,
  14 emitted, 26 dismissed; cross_lane_interaction_complete: true;
  adjacent_variant_complete: true; shadow_manifest valid, 22 entries
- source_digest: 090802ba129ad979bd973eddcd6d855a1b43652b0c9c52e4bd493a28737a50e4
- manifest_digest: 421e8e4cfc44e4e7f056bfa16aaa9d795e13b117ebe2b451f8c251ece83e0041
- attestation_digest: c9875d819da0875e9509387019dc96dc6a27c59dda26e31d4b69c121be7bde2d
- findings_presented: 14 (all severity blocking)
- accepted:
  - APR-001 / bad-sequencing / engine/core.py constructor and the live WorkflowInstanceManager importers must move before the class is deleted
  - APR-002 / unhandled-edge / save() cannot replace a persona instance; needs replace_for_session on the public apply_persona_impl path
  - APR-003 / unhandled-edge / instance copy must include handoff_ready sessions
  - APR-004 / unhandled-edge / instance copy must map variables, counters, timestamps, and flags with nullable normalization
  - APR-005 / unhandled-edge / equivalence guard needs a per-branch definition and a current_step membership assertion
  - APR-006 / bad-sequencing / fail-closed save must precede child start and task auto-claim
  - APR-007 / unhandled-edge / save and merge_variables must serialize on the per-session mutation lock
  - APR-008 / unhandled-edge / completion seeding must re-gate on the typed instance and run after recovery
  - APR-009 / unhandled-edge / drop backstop must re-run payload equivalence, not key presence
  - APR-010 / traceability / six deliverables were missing real consumers from Targets
  - APR-011 / bad-sequencing / the absence audit must depend on the documentation sweep it scans
  - APR-012 / weak-testability / the E1 row arithmetic was internally inconsistent
  - APR-013 / weak-testability / several acceptance items cited unrelated artifacts
  - APR-014 / gobby-format / module-qualified symbol artifacts do not resolve in gcode, so the consumer sweep silently skipped them
- declined: none
- deferred: none
- resolution_notes: >-
  All fourteen accepted and folded in. P3 absorbed the largest share: 3.1 gained
  `replace_for_session`, a writer-serialization contract, and acceptance 3.1.4
  and 3.1.5; 3.2 gained `engine/core.py` and `hooks/factory.py` in Targets, a
  rule-engine constructor bullet, a rewritten persona bullet on the
  `apply_persona_impl` path including the step-less delete case, a placement
  requirement on the fail-closed boundary, a restructured copy migration
  (live-status set including `handoff_ready`, full column mapping with
  COALESCE, two snapshot branches, per-branch guard with the membership
  assertion), and acceptance 3.2.9 through 3.2.12; 3.3 gained the
  completion-seed re-gating, the pre-deletion rewiring of
  `mcp_proxy/tools/workflows/__init__.py` and `servers/routes/workflows.py`,
  and acceptance 3.3.7 and 3.3.8; 3.4's fault-injection behavior now asserts no
  live child process, broadcast, or task claim survives. APR-009 rewrote 7.1's
  backstop to a payload-equivalence check with one-to-one matching and added
  7.1.5 for the session-lifecycle purge job. APR-011 changed 7.2 to
  `(depends: 7.1, 7.3)` and gave `config/tasks.py` an owner in 7.3 with
  acceptance 7.3.4. APR-010 added the missing consumers to 2.4, 4.1, 4.3, 5.1,
  5.2, and 7.1 Targets, including all eleven PipelineLoader callers. APR-013
  corrected 5.1.4 and 5.2.2, split `get_step_status` into its own 5.2.3, and
  added 1.2.6, 1.2.7, 2.3.8, 4.3.7, and 6.1.6. APR-014 rewrote all eighteen
  module-qualified symbol artifacts to gcode `qualified_name` values paired
  with file artifacts; the sweep then resolved them for the first time and
  immediately caught a real omission (`hooks/factory.py` missing from 3.2),
  confirming the finding. APR-012 was settled against the live hub rather than
  arithmetic alone: the `+1` agent was the soft-deleted `holistic-reviewer`,
  which also carried steps and so implied a 26th child. The operator then hard
  deleted that row with its generated `-steps` row and 4 dead-session
  instances, so the plan now states 167 rules, 29 agents, 42 variables, 11
  pipelines = 249 definition rows plus 25 children, and E1 requires the counts
  be re-derived from the restored hub rather than trusted as literals — the
  legacy tables stay writable until P7, so retiring any bundled agent moves
  them again. That query also exposed a defect no lane
  reported: the 2.3 child-insert predicate calls `jsonb_array_length` on the
  four `"steps": null` rows, which aborts with `cannot get array length of a
  scalar`; the predicate now guards on `jsonb_typeof(...) = 'array'`.
  `uv run gobby plans validate` passes including the consumer sweep.

**Round 2** `kind: adversary`

- evidence_id: df50231d-10cb-4265-a16a-f4db0ddc7156
- plan_hash: 53c7317fbcc7bb3bb58285b6bbc79e233279cb85b259d54d23b532f37bf32106
- adversary_run: 1e4303cc-e4f4-4c6f-b7e0-06541a7f70e6
- reviewer_session: 0d516656-0c43-43ae-ba3e-87bc631f5c17
- provider/model/reasoning: codex / gpt-5.6-sol / xhigh (applied)
- verdict: needs_review
- coverage_attestation: 3/3 lanes completed (requirements_traceability 6,
  repository_blast_radius 10, runtime_invariants 5 candidates); 21 candidates,
  16 emitted, 5 dismissed; cross_lane_interaction_complete: true;
  adjacent_variant_complete: true; shadow_manifest valid, 22 entries
- source_digest: 600181e7a4bdbdccdf96b86542f98407502180d5155e3fb2f50eaad8d945ec1e
- manifest_digest: 1f5e5ce384386605cf509f814f218add595d4c99691f23c79ca730cf87478219
- attestation_digest: d6cb9081a4596e870e0ef68a4cc7f5100f33d99f1b7619822fa22cc2170c9e8b
- findings_presented: 16 (all severity blocking; 7 marked introduced_in_round 2
  as fixer-induced by the round-1 repairs)
- accepted:
  - APR2-001 / bad-sequencing / 1.2 and 1.3 promise revision bumps before 1.4 creates the revision API
  - APR2-002 / traceability / deleting loader.py requires owning every importer, not only method callers; two round-1 Target paths do not exist
  - APR2-003 / traceability / the absence audit had unowned production occurrences
  - APR2-004 / missing-requirement / the doc sweep missed active guides documenting removed routes
  - APR2-005 / traceability / 2.1 claims a full field-access rewrite its Targets do not cover
  - APR2-006 / traceability / 5.1 named a module for a package and the wrong stdio client
  - APR2-007 / gobby-format / 2.3.8 still carried a bare method name instead of the gcode qualified_name
  - APR2-008 / bad-sequencing / the dry_run.py extraction must be unconditional and precede 2.4
  - APR2-009 / weak-testability / 7.1 removes baseline objects the 1.1 contract test still requires
  - APR2-010 / unhandled-edge / per-mutator locking leaves the read-compute-write window open
  - APR2-011 / bad-sequencing / the fail-closed boundary still sits after the provider process launches
  - APR2-012 / bad-sequencing / moving the save before auto-claim loses the successful claim transition
  - APR2-013 / unhandled-edge / persona instance replacement and the identity variable merge are not one transaction
  - APR2-014 / unhandled-edge / index-targeted ON CONFLICT cannot catch the primary-key collision soft-deleted rows hit on rerun
  - APR2-015 / bad-sequencing / P4 could start while P2 and P3 still owned its files, and 4.1 overlapped 4.2
  - APR2-016 / weak-testability / multi-owner acceptance items still cited a single artifact
- declined: none
- deferred: none
- corrections_to_reviewer:
  - APR2-002 asserted `workflows/engine/evaluation.py` still imports
    `WorkflowDefinitionRow`. It imports neither that name nor `workflow_type`;
    verified with `gcode imports` and `gcode grep`. The real unowned symbol is
    `WorkflowEvaluation.workflow_type` in `workflows/dry_run.py:101`, which the
    reviewer named separately in APR2-003. The substance is fixed under 4.3;
    the misattributed file was not added to Targets.
  - The reviewer missed a third dead Target path of the same class it flagged
    twice: `src/gobby/workflows/CLAUDE.md` in 7.3 does not exist. The real
    module guidance is `src/gobby/dispatch/CLAUDE.md`. Fixed under APR2-004.
- resolution_notes: >-
  All sixteen accepted. Every checkable claim was verified against the tree
  before application rather than taken on the reviewer's word: the two dead
  Target paths (`tasks/expansion/expansion_service.py`,
  `storage/sessions/lifecycle.py`) and `servers/routes/sessions.py` were
  confirmed absent; the `/api/workflows` client literals were located in
  `mcp_proxy/stdio_proxy.py:462,476`; `gcode` resolved
  `TemplateHashCache._load_agents`; `dry_run.py` measured 992 lines;
  `cleanup_failed_spawn` was read and confirmed to perform no PID or tmux
  termination; and `task_owned_by_child` was confirmed to be computed at
  `_implementation.py:854`, after both `start_run_or_cleanup` calls.
  The correctness findings drove the largest changes. APR2-010 added a
  critical-section-scope contract to 3.1 covering the whole
  read-compute-write span with a re-entrant lock and a compare-and-set
  fallback, plus acceptance 3.1.6 and 3.1.7. APR2-011 and APR2-012 together
  rewrote 3.2's fail-closed bullet into a two-part design — hoist
  child-session preregistration out of `execute_spawn` so the snapshot is
  durable before any process exists, and add PID/tmux termination to
  `cleanup_failed_spawn` for the remaining post-launch failures — with the
  auto-claim resolved as an atomic post-claim update rather than a
  reordering; acceptance 3.2.13 through 3.2.16 and 3.4 behaviors (h), (i),
  (j). APR2-013 added a single-transaction, fixed-lock-order requirement
  across the instance row and the session-variable merge. APR2-014 rewrote
  the Constraints conflict-target rule to targetless `ON CONFLICT DO
  NOTHING` with id-based matching for soft-deleted rows and five-case rerun
  coverage per domain. APR2-015 moved every P4 cutover behind completed P3
  and serialized 4.2 after 4.1, keeping 4.3 parallel now that its
  `dry_run.py` extraction has moved into 2.4 per APR2-008. APR2-001 inverted
  the 1.2/1.4 edge. APR2-002, APR2-003, APR2-005, and APR2-006 closed the
  traceability gaps, including all eleven remaining `WorkflowLoader`
  importers and a checked owner-inventory table in 7.2 so the audit's
  closure is asserted rather than claimed. APR2-016 split fourteen
  multi-owner acceptance items. `uv run gobby plans validate` passes
  including the consumer sweep.

**Round 3** `kind: adversary`

- evidence_id: 2220a657-e228-47bf-a93d-5276feb95d92
- plan_hash: db1a72c2806dd7ce0b27b640bace4fb7517061ab754f2f1ec65e9deab7b9a612
- adversary_run: 67e91aba-23fb-4679-b466-a8a546ba1f07
- reviewer_session: 10a0a49e-82e2-4672-85eb-0b23c94a8775
- provider/model/reasoning: codex / gpt-5.6-sol / xhigh (applied)
- verdict: needs_review
- coverage_attestation: 3/3 lanes completed (requirements_traceability 4,
  repository_blast_radius 6, runtime_invariants 6 candidates); 16 candidates,
  13 emitted, 3 dismissed; cross_lane_interaction_complete: true;
  adjacent_variant_complete: true; shadow_manifest valid, 22 entries
- source_digest: 858f8ec82acd91b9e247728f009472255960eb5de134e5858b1fbf3f5d16c2ad
- manifest_digest: abfff2024426d5c06d50ca41dc64f12a766046003bbf51de8df7bef62f0941fd
- attestation_digest: 5c67e8edc6961d16bc00ea70b19e60ba71f9ad940c0c8b8a2b484aff2cd613ac
- findings_presented: 13 (all severity blocking; 6 marked introduced_in_round 3
  as fixer-induced by the round-2 repairs, 3 marked introduced_in_round 2 as
  pre-existing gaps the round-1 repairs left open)
- accepted:
  - APR3-001 / traceability / the widened critical section is accepted in manager-only 3.1 while its production files belong to 3.2
  - APR3-002 / traceability / hoisting preregistration owns prepare_terminal_spawn, SpawnRequest, five provider paths, and four test seams
  - APR3-003 / unhandled-edge / "a single immediate db.transaction()" names no callable API on this adapter
  - APR3-004 / unhandled-edge / lease attach fails between launch and runtime persistence, so cleanup has no process identity
  - APR3-005 / unhandled-edge / current_step membership was asserted on only one of the two snapshot-source branches
  - APR3-006 / unhandled-edge / the persona state table omitted same-agent-with-no-row
  - APR3-007 / unhandled-edge / the web-chat caller swallows persona failure and returns a started session
  - APR3-008 / unhandled-edge / selecting only enabled legacy rows contradicts copying enabled verbatim
  - APR3-009 / traceability / two rule-path modules still import WorkflowDefinitionRow
  - APR3-010 / unhandled-edge / targetless ON CONFLICT plus a payload-only guard hides a changed UUID
  - APR3-011 / weak-testability / the shared rerun matrix is unreachable from the leaves that must implement it
  - APR3-012 / weak-testability / a relocated client endpoint was covered only by a server-route test
  - APR3-013 / weak-testability / P5 deletes surfaces whose existing suites it does not own
- declined: none
- deferred: none
- retractions:
  - **Round 2's `corrections_to_reviewer` was wrong on its first item and is
    retracted here.** Round 2 asserted that `workflows/engine/evaluation.py`
    imports neither `WorkflowDefinitionRow` nor `workflow_type` and declined to
    add the file to Targets on that basis. APR3-009 contradicted it and
    re-verified: `gcode imports src/gobby/workflows/engine/evaluation.py`
    returns `gobby.storage.workflow_definitions`, and `gcode grep
    WorkflowDefinitionRow` returns the import at `:12` and annotations at
    `:96`, `:207`, and `:229`. The reviewer was right and the round-2
    correction was a bad reading; the file is now a 4.1 Target along with
    `workflows/reserved_variables.py:6,39`, which neither round found until
    now. Prior-round entries are immutable, so the record stands and this
    retraction supersedes it.
- corrections_to_reviewer:
  - APR3-007 cites `servers/websocket/chat/_session.py:846-871` as the
    swallowing caller. That range is the web-chat **activation** block, which
    already fails closed — it stops the runtime and re-raises. The actual
    swallow is the persona call at `:910-923`, whose `except Exception`
    downgrades the failure to a warning before `registry.register` at `:925`.
    The finding's substance is correct and was applied against the real range.
  - APR3-002 enumerates five `prepare_terminal_spawn` call sites in
    `spawn_executor.py`. There is a **sixth** at `agents/resume_executor.py:192`
    that the reviewer missed; it crosses the same boundary and is now in 3.2
    Targets with `tests/agents/test_resume_executor.py`.
  - APR3-004 names lease attach as the missed post-launch failure point. A
    **fourth** point the reviewer did not name is worse: the tmux live-pane
    verification at `_implementation.py:752-762` sets
    `spawn_result.success = False` and then falls through the
    `if spawn_result.success` guard at `:788`, reaching no cleanup call at all,
    so it leaks both the process and the attached lease. Covered in the same
    repair.
- resolution_notes: >-
  All thirteen accepted. Every checkable claim was re-verified against the tree
  before application, which is how the round-2 retraction above surfaced. The
  verifications: `gcode imports` and `gcode grep` confirmed both
  `WorkflowDefinitionRow` consumers; `prepare_terminal_spawn` was read at
  `spawn.py:76-249` and confirmed to create the child session, the agent_runs
  row, and the env vars, returning the existing `PreparedSpawn` — so the repair
  reuses that type rather than inventing a prepared-spawn object;
  `_ambient.py:45`, `_PostgresTransaction.acquire_additional_lock`
  (`postgres_pool.py:239-250`), and `_acquire_lock` (`:503-511`) were read and
  confirm that `transaction_immediate` is the only immediate-transaction entry
  point and that nested lock priority must strictly increase; the lock order in
  the plan turned out to be not merely correct but **forced**, since
  `AgentStepInstanceMutation` inherits `PRIORITY = 875` against
  `SessionVariableMutation`'s `950`, and the plan now cites the constants
  instead of asserting a convention; the spawn sequence was read end to end and
  yielded the unnamed fourth failure branch; and all four P5 test suites were
  confirmed to contain the asserted imports, literals, and tool calls.
  The repairs. APR3-001 split the concurrency contract along ownership lines:
  3.1 now ships only the re-entrant lock and the compare-and-set precondition
  as manager primitives, and 3.2 gained an enforcement-critical-sections bullet
  plus acceptance 3.2.19 that widens the production read-compute-write spans in
  the three enforcement files it already owns. APR3-002 rewrote the
  preregistration bullet around `prepare_terminal_spawn`, added `spawn.py`,
  `spawn_models.py`, `resume_executor.py`, and four test suites to Targets, and
  split 3.2.13 into caller preregistration, provider consumption, and test
  retargeting. APR3-003 replaced the non-existent API with
  `db.transaction_immediate(AgentStepInstanceMutation(session_id))` plus an
  inner `acquire_additional_lock`. APR3-004 rebuilt the compensation bullet
  around process identity that outlives the failure and enumerated all four
  post-launch failure points, with acceptance 3.2.14a/14b and 3.4 behavior (h)
  extended to all four. APR3-005 made the membership assertion unconditional
  across both snapshot-source branches, with the generated-row refresh case
  spelled out. APR3-006 replaced the three-case persona rule with the full
  four-case state table and added 3.2.17 and 3.4 behavior (k). APR3-007 added
  caller fail-closed propagation with 3.2.18 and behavior (l). APR3-008 removed
  the `enabled` restriction from candidate selection and added 3.2.20.
  APR3-009 added both rule-path modules to 4.1 with acceptance 4.1.10 and to
  the 7.2 owner inventory. APR3-010 added `target.id = source.id` to every copy
  guard and the P7 backstop and grew the matrix from five cases to six.
  APR3-011 restated the full six-case matrix inside 4.1, 4.2, and 4.3 rather
  than referencing Constraints, since expansion agents receive only their own
  deliverable section, with per-domain acceptance for the live rerun and
  identity cases. APR3-012 added `tests/mcp_proxy/test_stdio_proxy.py` as the
  client seam on 5.1.6. APR3-013 pulled the four deleted-surface suites into
  P5 with explicit delete/retarget dispositions and acceptance 5.1.8 and 5.2.7,
  so the phase's own prescribed test run is green at its own commit.

**Round 4** `kind: adversary`

- evidence_id: bdd38dde-8f9c-4ca2-983e-cfdaabbcd0ae
- plan_hash: f98a3a3b3955a99d0127502955990e8bd8de2876f6dd4bd4fb21c5751adfcaa7
- adversary_run: bcedc1dd-8606-44ee-b079-d33dd297b091
- reviewer_session: a01fc0f6-9d08-4540-ac3d-5d5c85e77a34
- provider/model/reasoning: codex / gpt-5.6-sol / xhigh (applied)
- verdict: needs_review
- coverage_attestation: 3/3 lanes completed (requirements_traceability 4,
  repository_blast_radius 4, runtime_invariants 2 candidates); 10 candidates,
  7 emitted, 3 dismissed; cross_lane_interaction_complete: true;
  adjacent_variant_complete: true; shadow_manifest valid, 22 entries
- source_digest: 93102a3ded8e2ef4e5bfd57fe0620edc0ae0456a81cdf24bd9a21b2941b214df
- manifest_digest: 0f05917225a6213b6dcc2ea782db4b03aa8a0613053fca376ea21f5731c8a22e
- attestation_digest: 58058b0f7c714c8600b4627512c0883ac6cd6d88a606899366c5d166d4768e11
- findings_presented: 7 — **3 blocking, 4 major**. This is the first round in
  the recorded corpus to emit any severity other than blocking, following a
  prompt change that requires a concrete failure trace for `blocking` and
  forces completeness observations to `major`/`minor` by construction.
- accepted (all 7; the 4 `major` findings are real defects and were applied,
  they simply carry no standalone failure trace):
  - APR4-001 / blocking / bad-sequencing / the round-3 preregistration repair was applied to the resume chain, which is not on the SpawnRequest path
  - APR4-002 / blocking / unhandled-edge / SESSION_END deletes the instance unconditionally, including on COMPACT, which is a live handoff
  - APR4-003 / blocking / unhandled-edge / the drop backstop checks live rows only while copies preserve soft-deleted rows
  - APR4-004 / major / traceability / the instance column mapping omits agent_step_workflow_id
  - APR4-005 / major / weak-testability / the droid and SRT spawn suites sit on the moved boundary and were unowned
  - APR4-006 / major / weak-testability / bundled-definition contract suites still read the flat agent shape
  - APR4-007 / major / weak-testability / the persona suite still asserts _step_workflow_name and WorkflowInstanceManager
- declined: none
- deferred: none
- resolution_notes: >-
  All seven verified against the tree before application and all seven applied.
  APR4-001 is the most consequential: `resume_agent_run` calls
  `prepare_terminal_spawn` inline at `resume_executor.py:192` and builds its own
  spawn context, so it never receives a `SpawnRequest` — the round-3 repair
  would have deleted the only thing constructing its session. The resume call
  is now explicitly excluded from the hoist, and the review surfaced a gap no
  earlier round had touched: a resumed run creates a **new** child session,
  and `agent_step_instances` is keyed `UNIQUE(session_id)`, so without an
  explicit clone a daemon-stop resume of a mid-workflow agent restarts at
  `steps[0]`. 3.2 now specifies that clone, with acceptance 3.2.13c and 3.4
  behavior (n). APR4-002 was confirmed by reading the handler: the delete at
  `_session_end.py:94-110` is unconditional and `:168-169` sets
  `end_status = "handoff_ready"` for `SessionEndReason.COMPACT` — the same
  status 3.2's live-status filter preserves, so the plan contradicted itself
  and would have carried a silent per-compaction rewind onto the typed table.
  3.3 now branches deletion on end reason, taking the live set from the shared
  status constants, with acceptance 3.3.10 and behavior (m). APR4-003 widened
  the P7 backstop to every non-generated row, live by natural key and
  soft-deleted by preserved id, closing the window where generic legacy CRUD
  survives to P5. The four `major` findings closed ownership gaps:
  `agent_step_workflow_id` is now resolved and populated by the copy rather
  than left NULL (3.2.10); `test_spawn_executor_droid.py` and
  `test_srt_spawn.py` join 3.2 Targets, both verified to construct
  `SpawnRequest` directly and patch `spawn_executor.prepare_terminal_spawn`;
  a test inventory of eleven bundled-definition suites reading
  `agent["steps"]`/`agent["step_variables"]` was added to 2.1 with
  `tests/dispatch/test_bundled_agent_contract.py` moved into 2.2 alongside the
  YAML rewrite; and `tests/mcp_proxy/tools/test_apply_persona.py`, verified to
  assert `_step_workflow_name` at `:169`/`:214`/`:273` and use
  `WorkflowInstanceManager` at `:172`-`:244`, joins 3.2 with acceptance 3.2.21.
  Trend across four rounds: 14 → 16 → 13 → 7 findings, with calls-per-finding
  8.07 → 8.50 → 10.00 → 17.14. Only one round-4 finding was fixer-induced,
  down from six.

**Round 5** `kind: adversary`
- verdict: needs_review
- findings: 11 (6 blocking, 5 major, 0 minor); 161 calls; calls-per-finding
  14.64
- retractions:
  - Round 4's COMPACT retention rule (then 3.3 prose, 3.3.10, behavior (m))
    is retracted as **ineffective**, not merely over-scoped. It was written on
    the assumption that the compacted session is the session that resumes. It
    is not: `_session_start/flow.py:509` registers a new child row, `:527`
    expires the parent, and `:567-575` activates the child, while
    `handoff.py:50-183` transfers no step state. Retaining the row therefore
    strands it on an expired parent and the child still rebuilds at `steps[0]`
    — the same rewind plus a leak, since the orphan sweep
    (`storage/session_lifecycle.py:88-117`) only flips status. APR5-001,
    APR5-002, APR5-005, and APR5-010 are all consequences of that one rule.
  - Round 4's resume-clone rule (then 3.2 prose, 3.2.13c, behavior (n)) is
    retracted for the same class of reason: `runtime_cleanup.py:24-65` already
    deletes the source row when shutdown terminalizes the run
    (`runner_lifecycle_agents.py:349-411`), so the clone has nothing to read.
    APR5-003, APR5-004, and APR5-007 follow from it.
  - Both were attempts to fix **pre-existing** rewind bugs rather than to
    preserve current behavior across a storage split. Each needs a lifecycle
    protocol — source retention, an atomic ownership transfer, a rollback
    contract, an expiry path — spanning four modules. They are now tasks
    #18973 (compaction) and #18974 (daemon-stop resume), and 3.2/3.3/3.4 state
    the exclusion explicitly with a behavior-parity assertion (3.3.10, 3.4
    behavior (m)) so the excluded behavior is pinned rather than unasserted.
- Applied without retraction: APR5-006 narrowed 3.2.10 — the plan required
  lineage on every migrated row while declaring the column nullable with
  `ON DELETE SET NULL` and pinning snapshot-outlives-definition in 3.4(b); the
  copy now populates lineage where the child exists and permits NULL for a
  validated snapshot whose child does not, still failing when neither is
  present. APR5-011 corrected 7.1.5: `sessions/lifecycle.py:347` is
  `_purge_soft_deleted_definitions`, a **definition** purge, not the
  instance-oriented sweep the plan described — it now fans out over the four
  typed parent managers with children removed by cascade. APR5-008 extended
  2.1 past raw-key readers to the two consumer classes it had missed, verified
  concrete: `test_skill_composition.py:46` passes `step_variables=` and
  `test_apply_persona.py:158,202,260` pass `steps=` to the constructor, and
  `test_agent_definitions_v2.py:67,115` assert the removed field and its
  presence in the field set. APR5-009 added `test_factory.py` (imports the
  deleted `_register_agent_step_workflow` at `:976`) to 3.2 and
  `test_initial_variables.py` (imports `WorkflowInstanceManager` at
  `:302`/`:421`) to 3.3.
- Trend across five rounds: 14 → 16 → 13 → 7 → 11 findings, with
  calls-per-finding 8.07 → 8.50 → 10.00 → 17.14 → 14.64. Findings rose because
  8 of 11 carried `introduced_in_round: 4` — round 4's own repairs generated
  the surface. The corpus in `docs/plans/adversary-convergence.md` names this:
  over-scoped remedies manufacture the next round's findings. The response was
  to remove the mechanism rather than extend it, which is why round 5 shrinks
  the plan's behavioral scope instead of growing it.

**Re-baseline after #18973** `kind: verification`
- Task #18973 landed as commit `92b3ca567` between rounds 5 and 6, changing the
  behavior 3.3 is written to preserve. Round 5 had deferred compaction
  continuity to that task and asserted the *pre-fix* behavior (delete on every
  end reason) as parity; that assertion is now false and has been inverted.
- What landed, read from the commit rather than the task record:
  `_session_end.py` classifies the end reason first and derives
  `terminal_outcome = end_status == "expired"`, gating both `complete_agent_run`
  and the instance delete on it, so `COMPACT` → `handoff_ready` and IDLE
  web-chat → `paused` retain state. `_session_start/flow.py:524-560` calls the
  new `transfer_compact_handoff_state`, and on failure expires the child and
  returns `decision="block"` rather than activating it.
  `storage/session_lifecycle.py::transfer_compact_handoff_state` locks both
  sessions `FOR UPDATE`, rejects a child that already owns workflow state, then
  moves the rows, `sessions.agent_run_id`, and `agent_runs.child_session_id`,
  and expires the parent — all in one transaction.
  `expire_orphaned_handoff_sessions` now deletes instances in the same CTE that
  flips status.
- Plan effect: 3.3 gains the transfer and orphan-expiry statements as **port
  obligations** — both are raw SQL naming `workflow_instances`, which P7 drops,
  so neither can be left behind; 3.3 Targets gain `session_lifecycle.py`,
  `_lifecycle_delegate.py`, `_session_start/flow.py`, and the five suites
  #18973 added or changed; 3.3.10 inverts to preserving the terminal-outcome
  gate; 3.3.12-3.3.14 are new; 3.4 behavior (m) inverts to continuity. The
  `UNIQUE(session_id)` key on `agent_step_instances` is stricter than the
  legacy table's, so the transfer's existing "child already owns workflow
  state" precondition is what keeps the move legal — noted in 3.3 so the port

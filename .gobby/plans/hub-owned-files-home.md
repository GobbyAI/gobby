...
`update_bootstrap_yaml` is a read-modify-write. Hold `exclusive_file_lock` from `src/gobby/utils/durable_file.py` across re-read, updater, present-file validation, and durable replace. After acquiring the lock, re-read the file. Do not add a second lock type.

Present-file local/remote fixtures in the existing bootstrap, app-config, daemon-url, postgres-bootstrap, falkordb-bootstrap, remote-daemon, UI-exposure, and datastores-expose suites include the required field. True missing-file default cases stay owner-less.
...
Local full install takes `--files-home` naming an existing absolute directory (no root create). Persist that bootstrap via `ensure_daemon_config` before `ensure_personal_project_identity` and before managed-service startup. A bootstrap write failure leaves no personal identity marker and starts no managed services. Remote install does not take `--files-home`. It still requires the existing hub `local_cli_token` already on disk (the installer does not generate or rotate it). `run_remote_preflight` performs an authenticated owner probe against `hub_daemon_url` with that token before managed-service startup. Missing token or a rejected probe is a typed install failure. Do not add a second credential store or `--hub-token-stdin`.
...
- 2.1.9 - Remote install refuses when the hub `local_cli_token` is missing or the authenticated owner probe against `hub_daemon_url` is rejected, and it does not start managed services. test: `tests/cli/test_install_coverage.py`.
...
Remote-mode cleanup returns before any shared-row mutation. On the owner, claim the row into a deleting state that bind waits on or rejects. Unlink through the reconstructed resolver first; `FileNotFoundError` counts as completed cleanup; any other unlink failure keeps the row and releases the claim. Delete only the still-claimed row after unlink succeeds. Remote daemons never delete the only hub locator. Dual-worker and bind-versus-cleanup barrier tests cover the claim.
...
- 3.1.11 - Bind waits on or rejects a cleanup-claimed row; owner unlink failure keeps the row and releases the claim. test: `tests/storage/test_chat_attachments.py`.
...
- 4.1.12 - After owner bootstrap, gwiki init, legacy migrate, and an idempotent rerun, `<files_home>/wiki/_gwiki/scope.json` is absent. test: `tests/wiki/test_wiki_files_home.py`.
...
- 6.2.10 - A late invalid files archive member or destination collision refuses before any datastore row or files_home path changes, in both confirm and `--force` modes. test: `tests/cli/test_hub_files_restore.py`.
...
- Local bootstrap + `require_files_home`; present-file fixtures carry the required field, including daemon-url, postgres/falkordb, remote-daemon, and UI-exposure suites. `to_config_dict` and `DaemonConfig` carry the mode-specific fields. Concurrent bootstrap writers serialize through `exclusive_file_lock`.
- Personal marker and USER.md under files_home; remote startup keeps the sentinel and skips filesystem identity.
- Attachments reconstruct under `_personal/attachments`; cleanup claims the row, unlinks on the owner before row delete, and is a no-op remotely. Storage-layer visibility is by project and attachment id.
- Topic/personal wiki paths under `files_home/wiki`; registry writes stay relative; personal sentinel resolves without repo_path. Init and register_scope refuse a missing or replaced owner root. Rust integration fixtures use wiki-home children.
- Remote daemon proxies HTTP, MCP, jobs, topic/personal watchers, prune, and conversation clear/delete, and writes no daemon-home copies. Streaming uses connect, inactivity, and overall deadlines. Range and conditional headers keep FileResponse 206/304 behavior. Remote install authenticates the existing hub token against `hub_daemon_url` before services start.
...
- Pack/hub-backup restore round-trips files_home bytes with chunked publication; archive-wide preflight refuses oversized, duplicate, prefix-conflict, and late-invalid members before either domain mutates; existing inventory contracts stay green.

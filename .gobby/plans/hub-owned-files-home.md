...
Present-file local/remote fixtures in the existing bootstrap, app-config, daemon-url, postgres-bootstrap, falkordb-bootstrap, remote-daemon, and UI-exposure suites include the required field. True missing-file default cases stay owner-less.
...
- 5.1.16 - Topic and personal watchers and prune use `owner_dispatch`; checkout-project watchers stay local; remote mode creates no node-local wiki state. test: `tests/wiki/test_scheduled_jobs.py`.
...
- Remote daemon proxies HTTP, MCP, jobs, topic/personal watchers, prune, and conversation clear/delete, and writes no daemon-home copies. Streaming uses connect, inactivity, and overall deadlines.

...
export function useSessionDetail(sessionId: string | null) {
  ...
  const [transcriptDownloadUrl, setTranscriptDownloadUrl] = useState<
    string | null
  >(null);
  ...
  const [transcriptDegradedReason, setTranscriptDegradedReason] = useState<
    string | null
  >(null);
  ...
  const applyTranscriptWindow = useCallback(
    (
      update:
        | TranscriptWindowState<SessionMessage>
        | TranscriptWindowUpdate<SessionMessage>,
    ) => {
  ...
  const applyClearedDetail = useCallback(
    (error: string | null) => {
      setSession(null);
      messagesRef.current = [];
      setMessages([]);
  ...
  const loadSessionDetail = useCallback(
    async (
      ...
      try {
        ...
        const renderedResult = await fetchRenderedSessionMessages(
          activeSessionId,
          0,
          "tail",
        );
        if (!isCurrent() || tailWindowVersionRef.current !== tailWindowVersion)
          return;
        ...
        if (renderedResult.mapped.length === 0) {
          const nextTranscriptStatus =
            await fetchTranscriptStatus(activeSessionId);
          if (isCurrent()) {
  ...
  const applyRefreshedTailMessages = useCallback(
    (result: MessageLoadResult) => {
      ...
      setTotalMessages((prev) =>
        Math.max(result.totalCount, prev + update.addedCount),
      );
  ...
  const scheduleTailRefresh = useCallback(
    (activeSessionId: string) => {
  ...
  const loadMore = useCallback(async () => {
    ...
    const requestedOffset =
      currentWindow.renderedTotal - currentWindow.windowStart;
  ...
  const { generateSummary, isGeneratingSummary } = useSessionSummaryActions(
    sessionId,
    setSession,
  );

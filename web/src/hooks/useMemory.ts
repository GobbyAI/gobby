...
export function useMemory(projectId?: string | null) {
  ...
  const [searchResults, setSearchResults] = useState<GobbyMemory[] | null>(
    null,
  );
  ...
  const restoreMemory = useCallback(
    async (memoryId: string): Promise<boolean> => {
      ...
      setMemories((prev) =>
        prev.map((m) =>
          m.id === memoryId
            ? { ...m, deleted_at: null, dream_action: null }
            : m,
        ),
      );
      try {
        ...
        const response = await fetch(
          `${baseUrl}/api/memories/${memoryId}/restore`,
          {
            method: "POST",
          },
        );
  ...
  const promoteMemoryToGlobal = useCallback(
    async (memoryId: string): Promise<GobbyMemory | null> => {
      try {
        const baseUrl = getBaseUrl();
        const response = await fetch(
          `${baseUrl}/api/memories/${memoryId}/promote`,
          {
            method: "POST",
          },
        );
    ...
    (query: string) => {
      ...
      debounceRef.current = window.setTimeout(async () => {
        try {
          ...
          const response = await fetch(
            `${baseUrl}/api/memories/search?${params}`,
          );
          if (response.ok) {
            const data = await response.json();
            setSearchResults(
  ...
  const fetchKnowledgeGraph = useCallback(
    async (
      ...
      try {
        ...
        params.set("visibility", filters.visibility);
        const response = await fetch(
          `${baseUrl}/api/memories/graph/entities?${params}`,
        );
        if (response.ok) {
  ...
  const fetchEntityNeighbors = useCallback(
    async (entityKey: string): Promise<KnowledgeGraphData | null> => {
      try {
        const baseUrl = getBaseUrl();
        const params = new URLSearchParams();
        if (filters.projectId) params.set("project_id", filters.projectId);
  ...
  const fetchGraphData = useCallback(
    async (memoryLimit?: number): Promise<MemoryGraphData | null> => {
      try {
        const baseUrl = getBaseUrl();
        const params = new URLSearchParams();
        if (filters.projectId) params.set("project_id", filters.projectId);
        ...
        if (memoryLimit !== undefined)
          params.set("memory_limit", String(memoryLimit));

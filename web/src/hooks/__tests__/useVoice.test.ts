...
class MockAudioContext {
  ...
  audioWorklet = {
    addModule: vi.fn(
      () => deferredAudioWorkletModule?.promise ?? Promise.resolve(),
    ),
  };
...
describe("useVoice", () => {
  ...
  beforeEach(() => {
    ...
    voiceMocks.mockMicVADNew.mockImplementation(
  ...
  const sentPayloads = () =>
    wsRef.current?.send.mock.calls.map(([raw]) => JSON.parse(raw)) ?? [];
  ...
  const expectVoiceLog = (
    event: string,
    details: Record<string, unknown> = {},
  ) => {
    expect(voiceLogCalls()).toEqual(
  ...
  const submitPttAudio = async (result: {
    current: ReturnType<typeof useVoice>;
  }) => {
    await act(async () => {
    ...
    act(() => {
      lastWorkletNode?.port.onmessage?.(
        workletMessage(new Float32Array(16_000).fill(0.25)),
      );
    });
  ...
  it("sends voice_prepare immediately when requested voice targets change", async () => {
    ...
    expect(voicePreparePayloads()[1]).toEqual(
  ...
  it("warms voice and resends TTS state when the socket reconnects", async () => {
    ...
    await waitFor(() => {
      const payloads = wsRef.current?.send.mock.calls.map(([raw]) =>
        JSON.parse(raw),
      );
  ...
  it("records PTT audio, normalizes to 16 kHz, and logs the send lifecycle", async () => {
    ...
    const [[encodedAudio]] = voiceMocks.mockEncodeWAV.mock.calls as unknown as [
      [Float32Array],
    ];
    expect(encodedAudio.length).toBe(Math.round(16_000 / 3));
    expect(voiceMocks.mockEncodeWAV).toHaveBeenCalledWith(
    ...
    const voiceAudio = payloads?.find(
      (payload) => payload.type === "voice_audio",
    );
  ...
  it("abandons a delayed PTT start when recording is stopped", async () => {
    ...
    expect(voiceLogCalls()).not.toEqual(
      expect.arrayContaining([expect.arrayContaining(["[Voice] ptt_started"])]),
    );
  });
  ...
  it.each([
    ...
    "clears PTT transcribing state on voice_status:%s",
    async (status, error) => {
      const { result } = renderHook(() =>
        useVoice(
          wsRef as any,
  ...
  it("does not leave PTT transcribing state stuck for local send failures", async () => {
    ...
    const missingConversation = renderHook(() =>
      useVoice(
        wsRef as any,
        "",
        0,
    ...
    expect(warnSpy).toHaveBeenCalledWith(
      "Voice: WebSocket not open, discarding audio",
    );
  ...
  it("discards short PTT captures without sending audio", async () => {
    ...
    act(() => {
      lastWorkletNode?.port.onmessage?.(
        workletMessage(new Float32Array([0.2, -0.1])),
      );
    });
  ...
  it("blocks oversized PTT payloads before WebSocket send", async () => {
    ...
    await waitFor(() => {
      expect(result.current.voiceError).toBe(
        "Audio clip too long — try a shorter recording",
      );
    });
    expect(voiceAudioPayloads()).toHaveLength(0);
  ...
  it("drops incoming TTS audio when the playback queue is full", async () => {
    ...
    await waitFor(() => {
      expect(result.current.voiceError).toBe(
        "Audio dropped — connection too slow",
      );
    });
    expect(warnSpy).toHaveBeenCalledWith(
  ...
  it("detaches a stopped TTS source before starting replacement playback", async () => {
    ...
    expect(
      Math.round((startedSources[1].buffer?.getChannelData(0)[0] ?? 0) * 32768),
    ).toBe(2);
  });

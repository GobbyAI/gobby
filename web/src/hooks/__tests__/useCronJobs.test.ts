...
beforeEach(() => {
  ...
  mockFetch.mockJsonResponse(/\/api\/cron\/jobs\?project_id=project-123$/, { jobs: [] })
...
describe('useCronJobs', () => {
  it('includes the active project ID in the create payload', async () => {
    ...
    const createCall = mockFetch.fn.mock.calls.find(([, init]) => init?.method === 'POST')
    const payload = JSON.parse(String(createCall?.[1]?.body)) as CreateCronJobRequest
    ...
    expect(String(mockFetch.fn.mock.calls[0]?.[0])).toContain('project_id=project-123')
  })
  ...
  it('does not create an unscoped job', async () => {
    ...
    let createdJob: Awaited<ReturnType<typeof result.current.createJob>> | undefined
    await act(async () => {
    ...
    expect(mockFetch.fn.mock.calls.some(([, init]) => init?.method === 'POST')).toBe(false)
    expect(errorSpy).toHaveBeenCalledWith('Cannot create cron job without a project ID')
  })
})

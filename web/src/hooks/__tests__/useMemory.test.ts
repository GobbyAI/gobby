...
describe("useMemory", () => {
  ...
  it("requests the active visibility scope by default", async () => {
    ...
    expect(urls).toEqual(
      expect.arrayContaining([expect.stringContaining("visibility=active")]),
    );
  });
  ...
  it("fetchEntityNeighbors returns neighbors", async () => {
    mockFetch.mockJsonResponse(
      "/api/memories/graph/entities/proj-1%3A%3Areact/neighbors",
      {
        entities: [
          {
    ...
    const neighbors = await act(() =>
      result.current.fetchEntityNeighbors("proj-1::react"),
    );

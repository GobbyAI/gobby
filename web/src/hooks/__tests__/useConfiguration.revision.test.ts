...
describe("useConfiguration revision authority", () => {
  ...
  it("preserves_draft_after_conflict", async () => {
      ...
      async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (input === "/api/config/values") {
          ...
          return jsonResponse({
            revision,
            desired: {
              ui_settings: { theme: revision === 7 ? "dark" : "light" },
            },
            active: {
              ui_settings: { theme: revision === 7 ? "dark" : "light" },
            },
    ...
    let saveResult:
      Awaited<ReturnType<typeof result.current.saveConfig>> | undefined;
    await act(async () => {
    ...
    expect(result.current.configValues).toEqual({
  ...
  it("round_trips_codec_vectors", async () => {
    ...
    const fetchMock = vi.fn(
      async (input: RequestInfo | URL, init?: RequestInit) => {
        if (input === "/api/config/values" && init?.method === "PATCH") {
          expect(JSON.stringify(JSON.parse(String(init.body)).values)).toBe(
            canonical,
          );
          return jsonResponse({
            committed: true,
    ...
    expect(parseConfigRevision(Number.MAX_SAFE_INTEGER)).toBe(
      Number.MAX_SAFE_INTEGER,
    );
    expect(() => parseConfigRevision(Number.MAX_SAFE_INTEGER + 1)).toThrow(
      TypeError,
    );

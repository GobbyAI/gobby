...
import { buildToolIndex, buildTopLevelCommands } from "../useColonAutocomplete";
...
describe("buildToolIndex", () => {
  ...
  it("builds entries from a single server with tools", () => {
    ...
    const toolsByServer = {
      gobby: [
        makeTool("create_task", "Create a task"),
...
describe("buildTopLevelCommands", () => {
  it("merges ACP commands without duplicating built-in command names", () => {
    ...
    expect(commands.filter((command) => command.name === "plan")).toHaveLength(
      1,
    );
    expect(
      commands.filter((command) => command.name === "research"),
    ).toHaveLength(1);
  });

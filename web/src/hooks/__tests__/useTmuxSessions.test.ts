...
function sentMessages(ws: MockWebSocketInstance, type?: string): WireMessage[] {
  const messages = ws.send.mock.calls.map(([payload]) => JSON.parse(payload as string) as WireMessage)
...
describe('useTmuxSessions', () => {
  it('reconnect generation guard', () => {
    ...
    expect(mockWs.instances.filter((socket) => socket.readyState !== WebSocket.CLOSED)).toEqual([live])
    ...
    respondToAttach(live, requestId(live, 'tmux_attach'), 'worker', 'default', 'stream-1')
    expect(liveMount.result.current.attachedTarget).toEqual({ name: 'worker', socket: 'default' })
  ...
  it('socket qualified identity', () => {
    ...
    respondToAttach(ws, requestId(ws, 'tmux_attach'), 'shared', 'default', 'stream-default')
    expect(result.current.attachedTarget).toEqual({ name: 'shared', socket: 'default' })
    ...
    act(() => ws.simulateMessage({ type: 'tmux_detach_result', request_id: detachId, success: true }))
    ...
    respondToAttach(ws, requestId(ws, 'tmux_attach'), 'shared', 'gobby', 'stream-gobby')
    expect(result.current.attachedTarget).toEqual({ name: 'shared', socket: 'gobby' })
    ws.send.mockClear()
  ...
  it('wire payloads', () => {
    ...
    respondToAttach(ws, requestId(ws, 'tmux_attach'), 'worker', 'gobby', 'stream-wire')
  ...
  it('attach error handling', () => {
    ...
    act(() => ws.simulateMessage({
      type: 'error', request_id: failedAttachId, message: 'Attach failed',
    }))
    ...
    act(() => result.current.attachSession('worker', 'default'))
    respondToAttach(ws, requestId(ws, 'tmux_attach'), 'worker', 'default', 'stream-retry')
    expect(result.current.requestPending).toBe(false)

    act(() => result.current.detachSession())
    const failedDetachId = requestId(ws, 'tmux_detach')
    act(() => ws.simulateMessage({
      type: 'error', request_id: failedDetachId, message: 'Detach failed',
    }))
    expect(result.current.requestPending).toBe(false)
    ...
    act(() => ws.simulateMessage({
      type: 'tmux_detach_result', request_id: retryDetachId, success: true,
    }))
    expect(result.current.requestPending).toBe(false)
  ...
  it('duplicate send suppression', () => {
    ...
    respondToAttach(ws, requestId(ws, 'tmux_attach'), 'worker', 'default', 'stream-pending')
    ws.send.mockClear()
    act(() => {
      result.current.detachSession()
      result.current.detachSession()
  ...
  it('close clears pending request', () => {
    ...
    act(() => {
      staleAttachMessage?.(new MessageEvent('message', { data: JSON.stringify({
        type: 'tmux_attach_result',
      ...
      staleAttachMessage?.(new MessageEvent('message', { data: JSON.stringify({
        type: 'error', request_id: staleAttachId, message: 'Stale',
      }) }))
    })
    expect(result.current.requestPending).toBe(true)
    expect(result.current.attachError).toBeNull()
    ...
    act(() => {
      staleDetachMessage?.(new MessageEvent('message', { data: JSON.stringify({
        type: 'tmux_detach_result', request_id: staleDetachId, success: true,
      }) }))
      staleDetachMessage?.(new MessageEvent('message', { data: JSON.stringify({
        type: 'error', request_id: staleDetachId, message: 'Stale detach',
      }) }))
    })
    expect(result.current.requestPending).toBe(true)
    expect(result.current.attachError).toBeNull()
    ...
    expect(result.current.requestPending).toBe(false)
    expect(result.current.attachedTarget).toEqual({ name: 'worker', socket: 'gobby' })

...
describe("useSettings", () => {
  ...
  it("normalizes and persists plan pending variant", async () => {
    localStorage.setItem(
      "gobby-settings",
      JSON.stringify({ planPendingVariant: "bad-value" }),
    );
    const { result } = renderHook(() => useSettings());
  ...
  it("persists_settings_through_config_patch", async () => {
    ...
    await waitFor(() => {
      expect(
        fetchMock.mock.calls.some(([url]) => url === "/api/config/values"),
      ).toBe(true);
    });
    ...
    await waitFor(() => {
      const patchCall = fetchMock.mock.calls.find(([, init]) => {
        ...
        return (
          JSON.parse(String(init.body)).values.ui_settings.theme === "light"
        );
      });
  ...
  it("normalizes an empty persisted plan pending variant", () => {
    localStorage.setItem(
      "gobby-settings",
      JSON.stringify({ planPendingVariant: "" }),
    );
    const { result } = renderHook(() => useSettings());
  ...
  it.each([
    ...
    localStorage.setItem(
      "gobby-settings",
      JSON.stringify({ fontSize: persisted }),
    );
  ...
  it.each([null, "18"])(
    "falls back for invalid persisted font size %s",
    (persisted) => {
      localStorage.setItem(
        "gobby-settings",
        JSON.stringify({ fontSize: persisted }),
  ...
  it.each(["invalid-root", null, []])(
    "discards malformed persisted root %j",
    (persisted) => {
  ...
  it.each([
    ...
    await waitFor(() => {
      expect(
        JSON.parse(localStorage.getItem("gobby-settings") ?? "{}").fontSize,
      ).toBe(expected);
    });
    expect(result.current.settings.fontSize).toBe(expected);
  });

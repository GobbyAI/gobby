...
describe('useIsMobile', () => {
  ...
  it.each([
    { viewport: { width: 767, height: 900 }, expected: true, boundary: '767px width' },
    { viewport: { width: 768, height: 900 }, expected: false, boundary: '768px width' },
    { viewport: { width: 1200, height: 500 }, expected: true, boundary: '500px height' },
    { viewport: { width: 1200, height: 501 }, expected: false, boundary: '501px height' },
  ])('returns $expected at the $boundary boundary', ({ viewport, expected }) => {
  ...
  it('renders a fine-pointer landscape phone in the mobile tier', () => {
    ...
    expect(matchMedia).toHaveBeenCalledWith('(max-width: 767px), (max-height: 500px)')
  ...
  ])('warns and uses authored defaults for $invalidKind tokens', ({ width, height }) => {

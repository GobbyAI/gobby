...
export function useConfiguration() {
  ...
  const [activeConfigValues, setActiveConfigValues] = useState<
  ...
  const [failedLiveKeys, setFailedLiveKeys] = useState<
    Record<string, ConfigApplyFailure>
  ...
  const [builtInApprovalExemptions, setBuiltInApprovalExemptions] = useState<
    string[]
  >([]);
  ...
  useEffect(
    () => configurationClient.subscribe(applySnapshot),
    [applySnapshot],
  );

  useWebSocketEvent("config_event", (event) => {
    if ("revision" in event)
      configurationClient.observeRevision(event.revision);
  });
  ...
  const setRulesEnforcement = useCallback(
  ...
  const saveGlobalApprovalRules = useCallback(
    async (rules: string[]): Promise<boolean> => {
      try {
        const result = await configurationClient.patch({
  ...
  const saveTemplate = useCallback(
    async (content: string): Promise<{ ok: boolean; errors?: string[] }> => {
      try {
  ...
  const fetchSecrets = useCallback(async () => {
    try {
      ...
      if (data) {
        setSecrets(
          Array.isArray(data.secrets) ? data.secrets.filter(isSecretInfo) : [],
        );
        setSecretCategories(
  ...
  const saveSecret = useCallback(
    async (
      ...
      try {
        if (
          await configurationClient.saveSecret({
            name,
            value,
            category,
            description,
  ...
  const fetchPrompts = useCallback(async () => {
    try {
      ...
      if (data) {
        setPrompts(
          Array.isArray(data.prompts) ? data.prompts.filter(isPromptInfo) : [],
        );
  ...
  const getPromptDetail = useCallback(
    async (path: string): Promise<PromptDetail | null> => {
      try {
  ...
  const deletePromptOverride = useCallback(
  ...
  const exportConfig =
    useCallback(async (): Promise<ConfigExportBundle | null> => {
      try {
  ...
  const importConfig = useCallback(
    async (content: string): Promise<{ success: boolean; summary: string }> => {
      try {
  ...
  const runManagedAction = useCallback(
    async (
      ...
      configurationClient.runManagedAction(action, payload),
    [],
  );

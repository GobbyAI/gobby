...
export const VOICE_TRANSCRIPTION_WATCHDOG_MS =
  VOICE_TRANSCRIPTION_BACKEND_TIMEOUT_MS + 5_000;
...
function conversationLogPrefix(conversationId: string): string | null {
  return conversationId
    ? conversationId.slice(0, VOICE_CONVERSATION_LOG_PREFIX_LENGTH)
    : null;
}
...
function downsamplePcm(
  ...
  for (let index = 0; index < outputLength; index += 1) {
    ...
    output[index] =
      audio[leftIndex] + (audio[rightIndex] - audio[leftIndex]) * weight;
  }
...
export function useVoiceCapture({
  ...
  const transcriptionWatchdogRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  ...
  const startTranscriptionRequest = useCallback(
    (requestId: string) => {
  ...
  const markTranscriptionInProgress = useCallback(
    (requestId?: unknown) => {
      ...
      startTranscriptionRequest(
        activeRequestId ?? incomingRequestId ?? createVoiceRequestId(),
      );
      return true;
    },
    [startTranscriptionRequest],
  ...
  const finishTranscriptionRequest = useCallback(
    (requestId?: unknown) => {
      const incomingRequestId = requestIdFrom(requestId);
      const activeRequestId = activeTranscriptionRequestRef.current;
      if (
  ...
  const sendVoiceAudio = useCallback(
    async (audio: Float32Array, sampleRate: number) => {
      ...
      if (!conversationId && ensureConversationId) {
        try {
          conversationId =
            (await ensureConversationId()) ?? conversationIdRef.current;
        } catch (err) {
          logVoice("send_failed", {
            requestId,
          ...
          console.error(
            "Voice: Failed to create chat session before audio send:",
            err,
          );
          resetTranscriptionRequest();
      ...
      try {
        ...
        const vad = await MicVAD.new({
          ...
          onSpeechEnd: (audio: Float32Array) => {
            const speechGate = isLikelyVadSpeech(
              audio,
              VOICE_AUDIO_TARGET_SAMPLE_RATE,
            );

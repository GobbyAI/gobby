...
export function useTTSPlayback({
  ...
  const ensureAudioContextRunning = useCallback(
    (ctx: AudioContext): Promise<AudioContext | null> => {
      ...
      if (!resumePromiseRef.current || resumeContextRef.current !== ctx) {
        ...
        const resumePromise: Promise<AudioContext | null> = ctx
          .resume()
  ...
  const queueAudioChunk = useCallback(
    (pcmData: ArrayBuffer, sampleRate: number) => {
  ...
  const handleTTSAudioMeta = useCallback(

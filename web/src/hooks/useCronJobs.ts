...
export function useCronJobs(projectId?: string | null) {
  ...
  const fetchRuns = useCallback(async (jobId: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `${baseUrl}/api/cron/jobs/${encodeURIComponent(jobId)}/runs?limit=20`,
      );
      if (response.ok) {
        const data = await response.json();
  ...
  const updateJob = useCallback(
    async (
      ...
      try {
        const baseUrl = getBaseUrl();
        const response = await fetch(
          `${baseUrl}/api/cron/jobs/${encodeURIComponent(jobId)}`,
          {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
  ...
  const deleteJob = useCallback(
    async (jobId: string): Promise<boolean> => {
      if (
        !projectId ||
        !jobs.some((job) => job.id === jobId && job.project_id === projectId)
      ) {
        return false;
      }
      try {
  ...
  const toggleJob = useCallback(
    async (jobId: string): Promise<CronJob | null> => {
      try {
        const baseUrl = getBaseUrl();
        const response = await fetch(
          `${baseUrl}/api/cron/jobs/${encodeURIComponent(jobId)}/toggle`,
          {
            method: "POST",
          },
        );
  ...
  const runNow = useCallback(
    async (jobId: string): Promise<CronRun | null> => {
      if (
        !projectId ||
        !jobs.some((job) => job.id === jobId && job.project_id === projectId)
      ) {
      ...
      try {
        const baseUrl = getBaseUrl();
        const response = await fetch(
          `${baseUrl}/api/cron/jobs/${encodeURIComponent(jobId)}/run`,
          {
            method: "POST",
          },
        );

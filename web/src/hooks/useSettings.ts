...
const [ICON_CACHE_KEY = "v", ICON_CACHE_VALUE = ""] =
  ICON_CACHE_PARAM.split("=");
...
function normalizePersistedSettings(value: unknown): Partial<Settings> {
  ...
  if (settings.chatMode)
    normalized.chatMode = normalizeChatMode(settings.chatMode);
  if (settings.defaultChatMode) {
    normalized.defaultChatMode = normalizeChatMode(settings.defaultChatMode);
  }
  if (
    settings.planPendingVariant !== undefined &&
    settings.planPendingVariant !== null
  ) {
    normalized.planPendingVariant = normalizePlanPendingVariant(
      settings.planPendingVariant,
    );
  }
...
export function cacheBustedIconHref(path: string): string {
  ...
  const pathWithoutFragment =
    fragmentIndex >= 0 ? path.slice(0, fragmentIndex) : path;
...
export function useSettings() {
  const [settings, setSettings] = useState<Settings>(() => {
    ...
    return {
      ...DEFAULT_SETTINGS,
      ...normalizePersistedSettings(loadFromLocalStorage()),
    };
  });
  ...
  const updateDefaultChatMode = useCallback(
  ...
  const updateVoiceInputMode = useCallback(
    (voiceInputMode: VoiceInputMode) => {
  ...
  const updatePlanPendingVariant = useCallback(
    (planPendingVariant: PlanPendingVariant) => {

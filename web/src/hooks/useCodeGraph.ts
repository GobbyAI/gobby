...
export function useCodeGraph() {
  const fetchFileGraph = useCallback(
    async (
      projectId: string,
      limit: number = 200,
      ...
      try {
        ...
        const res = await fetch(
          `${getBaseUrl()}/api/code-index/graph?${params}`,
        );
  ...
  const expandFile = useCallback(
    async (
      ...
      try {
        ...
        const res = await fetch(
          `${getBaseUrl()}/api/code-index/graph/file/${encodeURIComponent(filePath)}?${params}`,
        );
        if (!res.ok) return null;
        return await res.json();
      } catch (e) {
  ...
  const expandSymbol = useCallback(
    async (
      ...
      try {
        const params = new URLSearchParams({ project_id: projectId });
        const res = await fetch(
          `${getBaseUrl()}/api/code-index/graph/symbol/${encodeURIComponent(symbolId)}/neighbors?${params}`,
        );
        if (!res.ok) return null;
        return await res.json();
      } catch (e) {
  ...
  const fetchBlastRadius = useCallback(
    async (
      ...
      try {
        ...
        const res = await fetch(
          `${getBaseUrl()}/api/code-index/graph/blast-radius?${params}`,
        );
        if (!res.ok) return null;
        return await res.json();
      } catch (e) {
  ...
  const searchSymbols = useCallback(
    async (
      ...
      try {
        ...
        const res = await fetch(
          `${getBaseUrl()}/api/code-index/graph/search?${params}`,
        );
        if (!res.ok) return [];

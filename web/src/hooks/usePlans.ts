...
export function usePlans() {
  ...
  const createPlan = useCallback((content: string, title = 'Implementation Plan'): string => {
  ...
  const updatePlan = useCallback((id: string, content: string, messageId?: string) => {
    setPlans((previous) => {
  ...
  const setPlanVersion = useCallback((id: string, index: number) => {
    setPlans((previous) => {
      const existing = previous.get(id)
      if (!existing || index < 0 || index >= existing.versions.length) return previous
      const next = new Map(previous)

...
export function useFiles() {
  ...
  const expandDir = useCallback(
    async (projectId: string, dirPath: string) => {
  ...
  const openFile = useCallback(
    async (projectId: string, path: string, name: string) => {
      ...
      const language =
        filenameToLanguage(name) || extensionToLanguage(ext || "");
  ...
  const saveFile = useCallback(
    async (index: number) => {
      ...
      if (!file || file.truncated || !file.dirty || file.editContent === null)
        return;
  ...
  const fetchDiff = useCallback(
    async (projectId: string, path: string): Promise<string> => {
      try {
        const res = await fetch(

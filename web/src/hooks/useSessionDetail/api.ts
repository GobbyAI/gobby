...
async function sessionMetadataFetchError(response: Response): Promise<Error> {
  ...
  return new Error(
    `Session metadata fetch failed (${response.status} ${response.statusText})${detail}`,
  );
}
...
export function mapRenderedRecordToSessionMessage(
  ...
  return {
    id: String(
      message.id ??
        message.message_index ??
        stableFallbackMessageId("hist", message, fallbackIndex),
    ),
...
function mapWebChatRecordToSessionMessage(
  ...
  return {
    id: String(
      message.id ?? stableFallbackMessageId("chat", message, fallbackIndex),
    ),
    role: (message.role as string) ?? "assistant",
    content,
    timestamp:
...
export async function fetchRenderedSessionMessages(
  ...
  const rawMessages = Array.isArray(messageData?.messages)
  ...
  const returnedCount =
    typeof messageData.returned_count === "number"
      ? messageData.returned_count
      : mapped.length;
  return {
    mapped,
    totalCount:
      typeof messageData.total_count === "number"
        ? messageData.total_count
        : mapped.length,
    renderedTotal:
      typeof messageData.rendered_count === "number"
        ? messageData.rendered_count
        : mapped.length,
    returnedCount,
...
export async function fetchTranscriptStatus(
  ...
  const statusRes = await fetch(
    `${baseUrl}/api/sessions/${sessionId}/transcript/status`,
  );
  if (!statusRes.ok) {
...
export async function generateSessionSummary(
  ...
  const res = await fetch(
    `${baseUrl}/api/sessions/${sessionId}/generate-summary`,
    {
      method: "POST",
    },
  );
  ...
  return typeof data.summary_markdown === "string"

...
export interface ConfigMutationSuccess {
  ...
  changedKeys: string[];
  applyStatus:
    "applied" | "pending_restart" | "failed_live" | "reconcile_failed";
...
export type ConfigMutationResult =
  ConfigMutationSuccess | ConfigMutationFailure;
...
export function parseConfigRevision(value: unknown): number {
  if (!Number.isSafeInteger(value) || (value as number) < 0) {
    throw new TypeError(
      "Configuration revision must be a safe non-negative integer",
    );
  }
...
export class ConfigurationClient {
  ...
  private readonly listeners = new Set<
    (snapshot: ConfigValuesSnapshot) => void
  >();
  ...
  async patch(
    ...
    if (!response.ok) {
      ...
      if (error.code === "revision_conflict") {
        ...
        if (!refreshed || refreshed.revision <= snapshot.revision)
    ...
    if (!isRecord(body) || body.committed !== true) {
      return {
        kind: "error",
        message: "Invalid configuration mutation response",
        retryable: false,
      };
    }
    ...
    return {
      ...
      applyStatus:
        body.apply_status === "pending_restart" ||
  ...
  async fetchTemplate(): Promise<string | null> {
    return (
      (await this.fetchYamlDocument("/api/config/template", "GET"))?.content ??
      null
    );
  }
  ...
  async saveSecret(payload: Record<string, unknown>): Promise<boolean> {
    return (
      await this.fetcher("/api/config/secrets", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async deleteSecret(name: string): Promise<boolean> {
    return (
      await this.fetcher(`/api/config/secrets/${encodeURIComponent(name)}`, {
        method: "DELETE",
      })
    ).ok;
  }
  ...
  async savePrompt(path: string, content: string): Promise<boolean> {
    return (
      await this.fetcher(`/api/config/prompts/${encodeURIComponent(path)}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  async runManagedAction(
    ...
    if (!action.startsWith("/api/") || action.includes("://")) {
      throw new TypeError(
        "Managed configuration action must be a local API path",
      );
    }
    return (
      await this.fetcher(action, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      })
  ...
  private async fetchYamlDocument(
    ...
    const body = await this.fetchRecord(
      path,

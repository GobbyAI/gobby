...
export function decodeDynamicSegment(value: string): string {
  ...
  while (index < value.length) {
    ...
    if (char === '%') {
      if (index + 2 >= value.length) {
        throw new DynamicSegmentError('Truncated percent escape in dynamic config segment')
      }
      const digits = value.slice(index + 1, index + 3)
      if (!UPPER_HEX.has(digits[0]) || !UPPER_HEX.has(digits[1])) {
        throw new DynamicSegmentError('Percent escapes must use uppercase hexadecimal digits')
      }
    ...
    if (!SAFE_SEGMENT_CHARS.has(code)) {
      throw new DynamicSegmentError('Dynamic config segment is not canonically encoded')
    }
    bytes.push(code)
    index += 1
  }
  ...
  if (encodeDynamicSegment(decoded) !== value) {
    throw new DynamicSegmentError('Dynamic config segment uses a noncanonical escape')
  }
  return decoded
}

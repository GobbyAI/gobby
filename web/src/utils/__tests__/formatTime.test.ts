...
describe("relativeTime", () => {
  it.each(["", "not-a-date"])(
    'returns "Invalid date" for invalid input %j',
    (input) => {
...
describe("formatDuration", () => {
  ...
  it("returns DURATION_INVALID for invalid end", () => {
    expect(formatDuration("2024-01-01T00:00:00Z", "garbage")).toBe(
      DURATION_INVALID,
    );
  });

  it('returns "<1m" for negative durations', () => {
    expect(formatDuration("2024-01-02T00:00:00Z", "2024-01-01T00:00:00Z")).toBe(
      "<1m",
    );
  });

  it('returns "<1m" for durations under 1 minute', () => {
    expect(formatDuration("2024-01-01T00:00:00Z", "2024-01-01T00:00:30Z")).toBe(
      "<1m",
    );
  });

  it("returns minutes for durations under 1 hour", () => {
    expect(formatDuration("2024-01-01T00:00:00Z", "2024-01-01T00:25:00Z")).toBe(
      "25m",
    );
  });

  it("returns hours and minutes", () => {
    expect(formatDuration("2024-01-01T00:00:00Z", "2024-01-01T02:30:00Z")).toBe(
      "2h 30m",
    );
  });

  it("returns hours only when no remaining minutes", () => {
    expect(formatDuration("2024-01-01T00:00:00Z", "2024-01-01T03:00:00Z")).toBe(
      "3h",
    );
  });

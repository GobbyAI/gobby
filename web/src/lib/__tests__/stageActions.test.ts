...
describe("stageActions shared helper contract", () => {
  it("test_resolve_advance_required_chain", async () => {
    ...
    expect(resolveAdvanceAction("in_progress", "required")).toBe(
      "submit_for_review",
    );
    expect(resolveAdvanceAction("needs_review", "required")).toBe(
      "approve_review",
    );
    expect(resolveAdvanceAction("review_approved", "required")).toBe(
      "complete",
    );
  });
  ...
  it("test_stage_state_view_shape", () => {
    ...
    expect(source).toMatch(
      /export\s+interface\s+StageStateView\s*{[\s\S]*name:\s*string/,
    );
    ...
    expect(source).toMatch(
      /StageStateView\s*{[\s\S]*review_policy:\s*ReviewPolicy/,
    );
    expect(source).toMatch(
      /StageStateView\s*{[\s\S]*updated_at:\s*string\s*\|\s*null/,
    );
  });
  ...
  it("test_optimistic_move_matches_backend_row_rules", async () => {
    const { canonicalBoardStage, optimisticMoveTaskToStage } =
      await loadStageActions();
    const lifecycleTask = {
      id: "task-1",
      title: "Build task",
      task_type: "feature",
    ...
    expect(
      moved.stages.map((stage: { name: string; state: string }) => [
        stage.name,
        stage.state,
      ]),
    ).toEqual([
  ...
  it("test_optimistic_move_uses_canonical_stage_order", async () => {
    const { currentStage, optimisticMoveTaskToStage } =
      await loadStageActions();
    const task = {
      id: "task-1",
      title: "Build task",

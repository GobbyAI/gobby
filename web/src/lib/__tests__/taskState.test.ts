...
function stage(
  state: "ready" | "in_progress" | "needs_review" | "review_approved" | "done",
) {
  return {
    name: "build",
    display_name: "Build",
...
describe("taskState helpers", () => {
  ...
  it("derives ready and in-progress states from ownership and stage rows", () => {
    ...
    expect(getTaskDisplayState({ stages: [stage("in_progress")] })).toBe(
      "in_progress",
    );
  });

  it("derives review, approved, blocked, and closed states from canonical data", () => {
    expect(getTaskDisplayState({ stages: [stage("needs_review")] })).toBe(
      "needs_review",
    );
    expect(getTaskDisplayState({ stages: [stage("review_approved")] })).toBe(
      "review_approved",
    );
    expect(
      getTaskDisplayState({ state: { is_blocked: true, is_escalated: true } }),
    ).toBe("blocked");

...
describe('colorContrast', () => {
  it('parses oklch() strings into normalized components', () => {
    expect(parseOklch('oklch(62% 0.005 125)')).toEqual({ l: 0.62, c: 0.005, h: 125 })
  ...
  it('returns the canonical 21:1 ratio for black on white', () => {
    expect(contrastRatio('oklch(0% 0 0)', 'oklch(100% 0 0)')).toBeCloseTo(21, 1)
  })
  ...
  it('matches the expected ratio for the gutter token on the code background', () => {
    ...
    expect(contrastRatio('oklch(62% 0.005 125)', 'oklch(11% 0.005 125)')).toBeCloseTo(5.63, 1)
  })
})

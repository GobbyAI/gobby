...
Stay concise and direct.`;
  ...
  it("does not reclassify ordinary user text", () => {
    expect(
      normalizeChatRole("user", "Please read AGENTS.md and summarize it."),
    ).toBe("user");
  });
  ...
  it("flattens tool chain blocks without dropping earlier entries", () => {
    ...
    expect(message.toolCalls?.map((tool) => tool.id)).toEqual([
      "tool-1",
      "tool-2",
    ]);
  });
  ...
  it("pairs duplicate tool ids with the pending call", () => {
    ...
    expect(
      messages[0].toolCalls?.map((tool) => [tool.status, tool.result?.content]),
    ).toEqual([
      ["completed", "first"],
      ["completed", "second"],
    ]);

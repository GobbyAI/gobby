...
describe("sessionPersistence", () => {
  ...
  it("loads and saves the viewing session id", () => {
    ...
    expect(localStorage.getItem(VIEWING_SESSION_ID_STORAGE_KEY)).toBe(
      "view-session",
    );
  ...
  it.each(["observe", "proxy"] as const)(
    "loads and saves viewing mode %s",
    (mode) => {
  ...
  it.each(["", "manual", "invalid"])(
    "falls back to none for malformed viewing mode %s",
    (mode) => {
      if (mode) {

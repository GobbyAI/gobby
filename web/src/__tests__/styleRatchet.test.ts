    ...
    if (rel.endsWith(".css")) {
      ...
      if (rel.endsWith(".tsx") && !rel.startsWith(UI_PRIMITIVES_DIR)) {
        for (const [element, pattern] of Object.entries(RAW_ELEMENTS)) {
          ...
          if (count > 0)
            scan.rawElements[element as RawElement].set(rel, count);
        }
      }
    }
...
function ratchet(
  ...
  for (const [file, count] of [...actual.entries()].sort(([a], [b]) =>
    a.localeCompare(b),
  )) {
    const ceiling = allowed[file] ?? 0;
    if (count > ceiling) {
      failures.push(
        `${file}: ${count} ${label} (allowlist permits ${ceiling}). ${remedy}`,
      );
    }
  }
...
function delimitedBody(
  ...
  if (markerIndex < 0)
    throw new Error(`Missing ${marker} in style ratchet allowlist`);
...
function parseAllowlistSnapshot(source: string): AllowlistSnapshot {
  ...
  for (const name of [
    ...
    for (const [file, count] of pathCounts(
      delimitedBody(source, `export const ${name}`, "{", "}"),
    )) {
      counts.set(`${name}:${file}`, count);
    }
  }

  const rawBody = delimitedBody(
    source,
  ...
  const cssFiles = new Set(
    [
      ...delimitedBody(
        source,
        "export const CSS_FILE_ALLOWLIST",
        "[",
        "]",
      ).matchAll(/'([^']+)'/g),
    ].map((match) => match[1]),
  );
  const linePinMatch = source.match(
    /export const CSS_TOTAL_LINE_(?:PIN|CEILING)\s*=\s*(\d+)/,
  );
  if (!linePinMatch) {
    throw new Error(
      "Missing CSS_TOTAL_LINE_PIN or legacy CSS_TOTAL_LINE_CEILING",
    );
  }
...
function targetBranchFailures(
  ...
  for (const [key, count] of current.counts) {
    ...
    if (targetCount === undefined) {
      ...
      failures.push(
        `${key}: allowance increased from ${targetCount} to ${count}`,
      );
    }
  }
...
describe("style ratchet", () => {
  ...
  it("parses legacy target-branch line ceilings as exact pins", () => {
    ...
    expect(parseAllowlistSnapshot(currentSource).cssTotalLinePin).toBe(
      CSS_TOTAL_LINE_PIN,
    );
    expect(parseAllowlistSnapshot(legacyTargetSource).cssTotalLinePin).toBe(
      CSS_TOTAL_LINE_PIN,
    );
  });
  ...
  it("keeps raw interactive elements at or below the recorded per-file counts", () => {
    const failures = (Object.keys(RAW_ELEMENTS) as RawElement[]).flatMap(
      (element) =>
        ratchet(
  ...
  it("keeps agent editors at zero raw-control and shared-style debt", () => {
    for (const element of Object.keys(RAW_ELEMENTS) as RawElement[]) {
      for (const file of AGENT_EDITOR_FILES) {
        expect(
          scan.rawElements[element].get(file) ?? 0,
          `${file} raw <${element}> count`,
        ).toBe(0);
      }
    }
    expect(
      scan.clsConstant.get("src/components/agents/agents-styles.ts") ?? 0,
    ).toBe(0);
  });
  ...
  it("keeps swept agent surfaces at zero raw-control and SidebarPanel stylesheet debt", () => {
    ...
    expect(scan.cssFiles).not.toContain(
  ...
  it("keeps activity filter-panel selects on SelectField", () => {
    for (const file of ACTIVITY_SELECT_FIELD_FILES) {
      expect(
        ...
        `${file} SelectField composition`,
      ).toMatch(/<SelectField\b/);
    }
  });
  ...
  it("keeps activity metadata chip adopters on ui Chip", () => {
    for (const file of ACTIVITY_CHIP_ADOPTER_FILES) {
      ...
      expect(source, `${file} legacy chip class residue`).not.toMatch(
        /activity-(?:mcp-)?chip/,
      );
    }
  });
  ...
  it("bans stylesheets beyond the recorded set", () => {
    ...
    const failures = [
      ...
      ...CSS_FILE_ALLOWLIST.filter((file) => !actual.has(file)).map(
        (file) =>
          `${file}: recorded stylesheet no longer exists. Delete its entry in ${ALLOWLIST}.`,
      ),
    ];
    expect(failures).toEqual([]);
  });

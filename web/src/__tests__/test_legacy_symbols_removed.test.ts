...
const selfPath = join(
  sourceRoot,
  "__tests__/test_legacy_symbols_removed.test.ts",
);
const legacyTerms = [
  ["getTask", "Bucket"].join(""),
  ["Task", "Bucket"].join(""),
...
const retiredSkillsPath = ["src", "components", "skills"].join("/");
...
describe("Legacy task-state symbols are removed from web source", () => {
  ...
  it("test_no_legacy_skills_page_subtree", () => {
    expect(legacySkillsSubtreeFiles()).toEqual([]);
  });
});

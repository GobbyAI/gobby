...
function isDynamicTokenPrefix(token: string): boolean {
  return (
    DYNAMIC_TOKEN_PREFIXES.includes(token) ||
    DYNAMIC_TOKEN_PREFIXES.some((prefix) => {
...
describe("CSS token integrity", () => {
  it("keeps the compiled mobile variant aligned with emitted tier properties", async () => {
    const theme = readFileSync(
      join(process.cwd(), "src/styles/tailwind-theme.css"),
      "utf8",
    );
    ...
    expect(compiledMedia?.slice(1)).toEqual([
      ...
      emittedProperties
        .get("--breakpoint-mobile-max-height")
        ?.replace("px", ""),
    ]);
  ...
  it("keeps review and inactive colors in sanctioned palette lanes", () => {
    ...
    expect(
      tokens.match(/--color-review:\s*var\(--color-info\);/g),
    ).toHaveLength(2);
    expect(
      tokens.match(/--color-review-bg:\s*var\(--color-info-bg\);/g),
    ).toHaveLength(2);
    expect(
      tokens.match(/--color-review-soft:\s*var\(--color-info-soft\);/g),
    ).toHaveLength(2);
  ...
  it("keeps the typography ladder defined once, in tokens.css", () => {
    ...
    for (const step of [
      ...
      expect(
        theme.match(new RegExp(`^\\s*--text-${step}:`, "gm")),
      ).toHaveLength(1);
      expect(
        tokens.match(new RegExp(`^\\s*--text-${step}:`, "gm")),
      ).toHaveLength(1);
    }
  });
  ...
  it("keeps state colors on the foreground-first sibling contract", () => {
    ...
    for (const lane of ["info", "warning", "error", "success"]) {
      for (const suffix of ["", "-bg", "-foreground", "-soft", "-tint"]) {
        expect(
          tokens.match(new RegExp(`--color-${lane}${suffix}:`, "g")),
        ).toHaveLength(2);
        expect(theme).toContain(
          `--color-${lane}${suffix}: var(--color-${lane}${suffix});`,
        );
      }

      expect(tokens.match(new RegExp(`--text-on-${lane}:`, "g"))).toHaveLength(
        2,
      );
      const values = (name: string) =>
        [...tokens.matchAll(new RegExp(`--${name}:\\s*([^;]+);`, "g"))].map(
          (match) => match[1],
        );
      expect(values(`color-${lane}-foreground`)).toEqual(
        values(`color-${lane}`),
      );
    }
  });

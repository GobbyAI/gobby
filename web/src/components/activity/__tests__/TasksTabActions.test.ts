...
describe("TasksTabActions", () => {
  it("extracts JSON detail, JSON message, and text response errors", () => {
    ...
    expect(
      extractResponseErrorMessage("plain failure", "Bad", "Fallback", 500),
    ).toBe("plain failure");
    expect(extractResponseErrorMessage("", "", "Fallback", 503)).toBe(
      "Fallback (503)",
    );
  });
  ...
  it("uses response body details for task action failures", async () => {
    ...
    await expect(claimTaskForSession("", "task-1", "sess-1")).rejects.toThrow(
      "claim denied",
    );
  });

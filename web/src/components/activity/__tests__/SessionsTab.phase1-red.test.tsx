...
describe("SessionsTab Phase 1 chip contract", () => {
  ...
  it("renders session badges with the shared chip class", async () => {
    ...
    expect(screen.getByText("SB")).toHaveClass(
      "rounded-full",
      "uppercase",
      "font-mono",
    );
  ...
  it("shows LOCAL only for sessions marked as local", async () => {
    ...
    expect(
      screen
        .getByText(/(?:^|: )Legacy Local Session$/)
        .closest(".session-entry")?.textContent,
    ).toContain("LOCAL");
    expect(
      screen.getByText(/(?:^|: )Cloud Session$/).closest(".session-entry")
        ?.textContent,
    ).not.toContain("LOCAL");
  });
});

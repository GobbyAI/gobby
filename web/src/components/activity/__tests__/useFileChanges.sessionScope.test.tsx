...
describe("useFileChanges — session scoping", () => {
  ...
  it("fetches the viewed session and re-fetches when the session switches", async () => {
    const { result, rerender } = renderHook(
      ({ sid }: { sid: string | null }) =>
        useFileChanges(sid, noMessages, false),
      { initialProps: { sid: "sess-A" as string | null } },
    );

    await waitFor(() => {
      expect(result.current.changedFiles.map((f) => f.path)).toEqual([
        "src/a.ts",
      ]);
    });
    ...
    expect(fetch).toHaveBeenCalledWith(
      expect.stringContaining("/sessions/sess-A/changes"),
    );
    expect(fetch).toHaveBeenCalledWith(
      expect.stringContaining("/sessions/sess-B/changes"),
    );
  });
  ...
  it("surfaces an error when the session changes request fails", async () => {
    ...
    const { result } = renderHook(() =>
      useFileChanges("sess-A", noMessages, false),
    );

    await waitFor(() => {
      expect(result.current.error).toBe(
        "Could not load changes for this session.",
      );
    });
  ...
  it("merges the live message overlay only for the active chat", async () => {
    ...
    const { result } = renderHook(() =>
      useFileChanges("sess-A", messages, true),
    );

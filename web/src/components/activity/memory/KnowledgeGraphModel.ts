...
export function edgeColor(relType: string): string {
  const colorVar =
    EDGE_COLOR_VARS[hashString(relType) % EDGE_COLOR_VARS.length];
...
export function mergeGraphData(
  ...
  const edgeKey = (r: KnowledgeRelationship) =>
    `${r.source_key}|${r.type}|${r.target_key}`;
 ...
 */
export function buildNodeCardHtml(
  entity: KnowledgeEntity,
  connections: NodeConnection[],
): string {
  const opaqueName = isOpaqueIdentifier(entity.name);
  const title = opaqueName
    ? `Unlabeled ${entity.entity_type.toLowerCase()}`
    : entity.name;
...
export function buildForceData(data: KnowledgeGraphData): {
  nodes: GraphNode[];
  links: GraphLink[];
} {
...
export function effectiveGraphLimits(
  limits: GraphLimits,
  isMobile: boolean,
): GraphLimits {
  ...
  return {
    entities:
      limits.entities === 0
        ? MOBILE_ENTITY_CAP
        : Math.min(limits.entities, MOBILE_ENTITY_CAP),
    relationships:
      limits.relationships === 0

  ...
  return (
    <>
      ...
      <FormField
        label="Arguments"
        group
        className="mb-2.5 [&>span:first-child]:text-xs"
      >
        {() => (
          <KeyValueEditor
...
export function InvokePipelineFields({ step, onChange }: StepFieldsProps) {
  ...
  const name = isObject
    ...
    : ((raw as string) ?? "");
  const args = isObject
    ? ((rawObject.arguments as Record<string, string>) ?? {})
    : {};
  const argPairs: KVPair[] = Object.entries(args).map(([key, value]) => ({
    key,
    value: String(value),
...
export function CommonFields({
  ...
  const toolsValue = Array.isArray(step.tools)
    ? (step.tools as string[]).join(", ")
    : "";
        ...
        {({ id, describedBy, invalid }) => (
          <DraftTextInput
            ...
            onCommit={(draft) => {
              ...
              onChange({
                condition: value ? wrapTemplateExpr(value) : undefined,
              });
              return value;
            }}
      ...
      <div className="mb-2.5 flex items-center gap-1.5 text-sm">
        <Input
          ...
          onChange={(e) => {
            if (e.target.checked) {
              onChange({
                approval: { required: true, message: "", timeout: 0 },
              });
            } else {
          ...
          >
            {({ id, describedBy, invalid }) => (
              <Input
                ...
                onChange={(e) =>
                  onChange({
                    approval: { ...approval, message: e.target.value },
                  })
                }
  ...
  return (
    <div className="flex flex-col gap-1">
      {draftPairs.map((pair, index) => (
        <div key={index} className="flex items-center gap-1">
          ...
          <Button
            ...
            className={cn(
              coarseHitAreaCls,
              "size-7 min-h-7 shrink-0 border-border",
            )}
            onClick={() => {
              const next = draftPairs.filter(
      ...
      <Button
        ...
        className={cn(
          coarseHitAreaCls,
          "justify-start border-dashed border-border",
        )}

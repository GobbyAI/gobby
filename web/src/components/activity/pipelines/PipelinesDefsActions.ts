...
async function parseDefinitionResponse(response: Response): Promise<PipelineDefinition | null> {
...
export async function updatePipelineDefinition(
  ...
  const response = await fetch(`${getBaseUrl()}/api/workflows/${encodeURIComponent(id)}`, {
    method: "PUT",
...
export async function togglePipelineDefinition(id: string): Promise<PipelineDefinition | null> {
  const response = await fetch(`${getBaseUrl()}/api/workflows/${encodeURIComponent(id)}/toggle`, {
    method: "PUT",
  });
  return parseDefinitionResponse(response);
}
...
export async function exportPipelineYaml(id: string): Promise<string | null> {
  const response = await fetch(`${getBaseUrl()}/api/workflows/${encodeURIComponent(id)}/export`);

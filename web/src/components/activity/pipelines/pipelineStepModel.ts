...
export function getTypeColor(type: StepType): string {
  return STEP_TYPES.find((item) => item.value === type)?.color ?? 'var(--text-muted)'
}
...
export function createDefaultStep(type: StepType, existingIds: string[]): PipelineStep {
  const base = 'step'
...
export function changeStepPayload(step: PipelineStep, type: StepType): PipelineStep {
  const cleaned = { ...step }

...
export function SkillsHubDetail({
  result,
  projectId,
  onInstalled,
  onError,
}: SkillsHubDetailProps) {
  ...
  const canAttemptInstall =
    Boolean(content && activeScanResult) &&
    !activeScanning &&
    !activeInstalling;
  ...
  async function handleInstall() {
    ...
    if (!activeScanResult.is_safe) {
      ...
      const confirmed = await confirm({
        ...
        description:
          "Review the scan findings before installing this hub skill into your local skill library.",
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
        ...
        actions={
          <>
            {scanStatusLabel && (
              ...
              >
                <SeverityIcon
                  severity={severityKey(activeScanResult?.max_severity)}
                />
                {scanStatusLabel}
              </span>
            )}
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-3">
        <div className="grid [grid-template-columns:repeat(auto-fit,minmax(min(24rem,100%),1fr))] gap-3">
            ...
            <div className="grid gap-2 rounded-md border border-border bg-[var(--bg-secondary)] p-3">
              <div className="grid grid-cols-[6rem_minmax(0,1fr)] gap-2 text-sm">
                ...
                <span className="text-foreground">
                  {result.version ? `v${result.version}` : "Unknown"}
                </span>
                    ...
                    return (
                      ...
                      >
                        ...
                        <p className="mt-2 mb-0 text-sm leading-relaxed text-muted-foreground">
                          ...
                          {finding.remediation ||
                            "Review the finding before installing."}
                        </p>
                      </article>
                    );
                  })}
          ...
          <section className="flex min-h-0 min-w-0 flex-col gap-2">
            ...
            <pre
              ...
              className="m-0 min-h-64 flex-1 overflow-auto rounded-md border border-border bg-[var(--bg-secondary)] p-3 font-mono text-xs leading-relaxed whitespace-pre-wrap text-foreground"
            >

...
export function TaskCreateForm({
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      if (labelsInput.trim()) {
        params.labels = labelsInput
          .split(",")
          .map((l) => l.trim())
          .filter(Boolean);
      }
      if (validationCriteria.trim())
        params.validation_criteria = validationCriteria.trim();
  ...
  const parentOptions = tasks.filter(
    (t) => t.task_type === "epic" || t.task_type === "task",
  );
      ...
      >
        ...
        >
          ...
          >
            ...
            {error && (
              <p className="text-md text-[var(--color-error)]" role="alert">
                {error}
              </p>
            )}

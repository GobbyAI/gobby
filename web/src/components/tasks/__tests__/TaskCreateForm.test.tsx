...
function renderForm(onClose = vi.fn()) {
  render(
    <TaskCreateForm isOpen tasks={[]} onSubmit={vi.fn()} onClose={onClose} />,
  );
  const dialog = screen.getByRole("dialog");
  return {
    onClose,
    dialog,
    backdrop: dialog.previousElementSibling as HTMLElement,
  };
}
...
describe("TaskCreateForm dismissal", () => {
  ...
  it.each(["Escape", "backdrop"])(
    "dismisses a clean form via %s without confirmation",
    async (method) => {
  ...
  it.each(["Escape", "backdrop"])(
    "keeps a dirty form open when %s discard is cancelled",
    async (method) => {

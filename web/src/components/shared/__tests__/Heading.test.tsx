...
describe("Heading", () => {
  ...
  it("warns in dev mode when no explicit level or provider is present", async () => {
    ...
    expect(
      screen.getByRole("heading", { level: 1, name: "Untitled Section" }),
    ).toBeInTheDocument();
    await waitFor(() => {
  ...
  it("does not warn when a provider supplies an implicit level", async () => {
    ...
    expect(
      screen.getByRole("heading", { level: 3, name: "Nested Section" }),
    ).toBeInTheDocument();
  ...
  it("does not warn when the heading level is explicit", async () => {
    ...
    expect(
      screen.getByRole("heading", { level: 2, name: "Explicit Section" }),
    ).toBeInTheDocument();
    await waitFor(() => expect(warn).not.toHaveBeenCalled());
  });
});

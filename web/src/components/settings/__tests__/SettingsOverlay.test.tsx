...
async function openOverlay(): Promise<HTMLElement> {
  ...
  await waitFor(() => {
    const heading = within(dialog).getByRole("heading", {
      name: defaultSection?.label,
    });
    expect(heading.closest("[aria-busy]")).toHaveAttribute(
      "aria-busy",
      "false",
    );
  });
...
describe("SettingsOverlay mobile-tier logout entry (#19185)", () => {
  ...
  it("renders no logout entry at the desktop tier (header cluster owns it)", async () => {
    ...
    expect(
      within(dialog).queryByRole("button", { name: "Log out" }),
    ).toBeNull();
  });
  ...
  it("blocks leaving a dirty section when discard is declined", async () => {
    ...
    render(
      <Harness
        ...
        dirtySections={[DEFAULT_SETTINGS_SECTION]}
      />,
    );
    const dialog = await openOverlay();
    selectSection(dialog, "Observability");

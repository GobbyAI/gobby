...
function GuardHarness({
  ...
  const context = makeContext({ registerDirtyGuard: overlay.registerDirtyGuard })
  return (
    <SettingsSectionContext.Provider value={context}>
      <span data-testid="active-section">{overlay.activeSection}</span>
      <button type="button" onClick={() => overlay.selectSection('observability')}>
        Switch section
      </button>
      <DemoSection />
    </SettingsSectionContext.Provider>
...
describe('SettingsSection blocks navigation while dirty', () => {
  ...
  it('allows a section switch when discard is confirmed', () => {
    ...
    expect(confirmDiscard).toHaveBeenCalledTimes(1)
    expect(screen.getByTestId('active-section').textContent).toBe('observability')
  })

...
function AppearanceControls({ client }: { client: UseSettingsReturn }) {
  ...
  return (
    <>
      <AppearanceField
        label="Theme"
        description="Match your system or pin a fixed appearance."
      >
      ...
      >
        <div className="flex items-center gap-3">
          <Input
            ...
            className="h-auto max-w-full cursor-pointer rounded-none border-0 bg-transparent p-0 accent-accent focus-visible:outline-2 focus-visible:outline-offset-[3px] focus-visible:outline-accent pointer-coarse:min-h-11"
            type="range"
            min={FONT_SIZE_MIN}
            max={FONT_SIZE_MAX}
            step={1}
      ...
      {() =>
        clientSettings ? (
          ...
          <p className="rounded-lg border border-border bg-muted px-5 py-4 text-sm leading-[1.5] text-foreground-muted">
            Appearance settings are unavailable — the settings provider is not
            mounted.
          </p>
        )
      }

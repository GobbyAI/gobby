...
import type { UseSettingsReturn, VoiceInputMode } from '../../../hooks/useSettings'
 ...
 */
function ClientPreferences({ clientSettings }: { clientSettings?: UseSettingsReturn }) {
  if (!clientSettings) {
    return (
  ...
  return (
    ...
    >
      <BoundedSelectField
        ...
        onChange={(value) => clientSettings.updateDefaultChatMode(value as ChatMode)}
      />
      <SwitchField
      ...
      <BoundedSelectField
        ...
        onChange={(value) => clientSettings.updateVoiceInputMode(value as VoiceInputMode)}
      />
    </Subsection>
  )
}
...
function ChatGroup({ fields }: { fields: SettingsSectionFields }) {
  return (
    <Subsection title="Chat" hint="Daemon-side chat defaults and attachment limits.">
      <SchemaSelectField
        fields={fields}
        path="chat.profile"
...
function VoiceGroup({ fields }: { fields: SettingsSectionFields }) {
  return (
    <Subsection title="Voice" hint="Master switch for the daemon voice subsystem (STT + TTS).">
      <SwitchConfigField
        fields={fields}
        path="voice.enabled"
...
function TextToSpeechGroup({ fields }: { fields: SettingsSectionFields }) {
  return (
    <Subsection title="Text-to-speech" hint="Spoken responses via the configured TTS provider.">
      <SwitchConfigField
        fields={fields}
  ...
  return (
    <div className="flex min-w-0 flex-1 flex-col gap-3">
      ...
      <NumberField
        ...
        value={typeof value.timeout_seconds === 'number' ? value.timeout_seconds : null}
        min={0}
        onChange={(timeout_seconds) => patch({ timeout_seconds: timeout_seconds ?? undefined })}
      />
    </div>
  )
}

...
describe("ChatVoiceSection", () => {
  it("wires client voice toggles and input mode to the shared useSettings instance", () => {
    ...
    expect(ctx.clientSettings?.updateVoiceInputMode).toHaveBeenCalledWith(
      "vad",
    );
  });
  ...
  it("ports the legacy default-mode control to the client chat-mode select", () => {
    ...
    expect(ctx.clientSettings?.updateDefaultChatMode).toHaveBeenCalledWith(
      "bypass",
    );
  });
  ...
  it("reads voice config rows: bounded selects, numbers, and the vocabulary list", () => {
    ...
    expect(
      screen.getByRole("switch", { name: "Voice features enabled" }),
    ).toBeChecked();
    ...
    expect(screen.getByLabelText("Whisper vocabulary item 1")).toHaveValue(
      "Gobby",
    );
  });
  ...
  it("renders OpenAI-compatible audio bindings as structured editors", () => {
    ...
    expect(screen.getByLabelText("Audio model (whisperx)")).toHaveValue(
      "large-v3",
    );
    expect(
  ...
  it("degrades gracefully when client settings are absent", () => {
    ...
    expect(
      screen.queryByRole("switch", { name: "Speech-to-text enabled" }),
    ).toBeNull();
    expect(
    ...
    expect(screen.getByLabelText("Chat capability profile")).toHaveValue(
      "feature_high",
    );
  });
});

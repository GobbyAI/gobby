...
describe("IntegrationsHooksSection", () => {
  it("reads the communications scalar rows", () => {
    ...
    expect(
      screen.getByLabelText("Communications webhook base URL"),
    ).toHaveValue("https://gobby.example/webhooks");
    expect(
  ...
  it("reads the channel-default number rows", () => {
    ...
    expect(screen.getByLabelText("Channel rate limit per minute")).toHaveValue(
      30,
    );
    ...
    expect(
      screen.getByLabelText("Channel poll interval (seconds)"),
    ).toHaveValue(30);
    expect(
  ...
  it("reads and persists the hook timeout rows", async () => {
    ...
    expect(
      screen.getByLabelText("Hook provider timeout (seconds)"),
    ).toHaveValue(120);
  ...
  it("renders broadcast_events as an editable string list, not a text fallback", () => {
    ...
    expect(
      screen.getByRole("switch", {
        name: "Include event payload in broadcasts",
      }),
    ).toBeChecked();
  });
  ...
  it("renders webhooks.endpoints as a structured typed-list editor", () => {
    ...
    expect(screen.getByLabelText("ci-bridge timeout (seconds)")).toHaveValue(
      12,
    );
    expect(screen.getByLabelText("ci-bridge retry count")).toHaveValue(2);
    expect(
      screen.getByLabelText("ci-bridge retry delay (seconds)"),
    ).toHaveValue(1.5);
    expect(

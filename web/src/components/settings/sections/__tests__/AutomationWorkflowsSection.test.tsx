  ...
  return {
    'gobby-tasks': {
      ...
      expansion: {
        ...
        research_model: 'claude/haiku',
        research_max_steps: 8,
        research_system_prompt: 'Research the codebase.',
...
describe('AutomationWorkflowsSection', () => {
  it('reads task-system and file-extraction rows under the hyphenated gobby-tasks key', () => {
    ...
    expect(screen.getByLabelText('Task file extensions item 1')).toHaveValue('.py')
  ...
  it('reads workflow-engine, tmux, cron int-list, system-loop, and pipeline rows', () => {
    ...
    expect(screen.getByLabelText('Workflow timeout (seconds)')).toHaveValue(300)
    expect(screen.getByLabelText('Scrollback history limit')).toHaveValue(10000)
    ...
    expect(screen.getByLabelText('Pipeline nesting depth limit')).toHaveValue(3)
  })

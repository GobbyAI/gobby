  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-8 [--control-row-height:var(--status-bar-control-height)] mobile:p-0">
      <Button
        ...
        variant="ghost"
        dense
        className="absolute inset-0 m-0 animate-settings-overlay-fade cursor-pointer bg-[var(--surface-scrim)] hover:bg-[var(--surface-scrim)]"
      ...
      <div
        ref={dialogRef}
        className="relative flex h-[min(640px,100%)] max-h-full w-[min(720px,100%)] animate-settings-overlay-rise flex-col overflow-hidden rounded-xl border border-border bg-surface-secondary shadow-lg outline-none mobile:h-full mobile:w-full mobile:rounded-none mobile:border-0"
        role="dialog"
        aria-modal="true"
      ...
      >
        <header className="flex shrink-0 items-center justify-between gap-3 border-b border-border px-5 py-4">
          <h2
            id={headingId}
            className="text-lg leading-[1.2] font-semibold text-foreground"
          >
            Settings
          </h2>
        ...
        <div className="flex shrink-0 items-center border-b border-border px-5 py-3">
          <div className="relative inline-block mobile:w-full">
            <Button
              ...
              dense
              className={cn(
                "min-w-56 cursor-pointer py-1.5 [font-family:inherit] aria-expanded:bg-[var(--accent-tint)] aria-expanded:text-accent mobile:w-full [&_svg]:shrink-0 [&_svg]:text-[var(--text-muted)] aria-expanded:[&_svg]:text-accent",
                coarseHitAreaCls,
              )}

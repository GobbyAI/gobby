...
export function ToolApprovalCard({
  ...
  return (
    <div className="rounded-lg border border-warning-foreground/30 bg-warning/20 overflow-hidden my-1.5">
      ...
      {sendError && (
        <div className={cn(TOOL_CARD_SPACING.bodyCompact, 'text-xs text-warning-foreground')}>
          {sendError}
        </div>
      )}
    </div>
...
export function AskUserQuestionCard({
        ...
        {questions.map((question, questionIndex) => {
          ...
          return (
            <div key={questionIndex} className="mb-3 last:mb-0">
              ...
              <div className="text-sm text-foreground mb-2">{question.question}</div>
              ...
              {answer &&
                !question.options.some(option => answerLabels.includes(option.label)) && (
    ...
    setSelectedOptions(previous => {
      ...
      if (label === '__other__') {
        if (current.includes('__other__')) {
          return {
            ...previous,
            [questionIndex]: current.filter(currentLabel => currentLabel !== '__other__'),
          }
        }
        return multiSelect
      ...
      if (multiSelect) {
        return current.includes(label)
          ? {
              ...previous,
              [questionIndex]: current.filter(currentLabel => currentLabel !== label),
            }
          : {
              ...previous,
              [questionIndex]: [...current.filter(currentLabel => currentLabel !== '__other__'), label],
            }
      }
  ...
  const hasSelection = Object.values(selectedOptions).some(selected => selected.length > 0)

  return (
    <div
      className={cn(
        'rounded-lg border border-accent/30 bg-accent/5 overflow-hidden my-1.5 p-3',
    ...
    >
        ...
        <div key={questionIndex} className="mb-3 last:mb-0">
          ...
          <div className="flex flex-wrap gap-1.5">
            {question.options.map((option, optionIndex) => {
              ...
              return (
                <Button
                  ...
                  onClick={() =>
                    handleOptionClick(questionIndex, option.label, question.multiSelect)
                  }
                  disabled={submitted}
                >
                  <div className="font-medium">{option.label}</div>
            ...
            <Button
              ...
              onClick={() => handleOptionClick(questionIndex, '__other__', question.multiSelect)}
              disabled={submitted}
            >
              Other
            </Button>
      ...
      {sendError && <div className="mt-1.5 text-xs text-warning-foreground">{sendError}</div>}
    </div>
  )
}

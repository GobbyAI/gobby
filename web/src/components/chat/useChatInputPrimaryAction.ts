...
export type ChatInputPrimaryButtonKind =
  "stop" | "mic-idle" | "mic-recording" | "send";
...
export function useChatInputPrimaryAction({
  ...
  const handleMicPointerDown = useCallback(
    (event: PointerEvent<HTMLButtonElement>) => {
      ...
      pointerStartedWhileRecordingRef.current = Boolean(
        isRecording && latchedRef.current,
      );
      holdActiveRef.current = false;
      clearHoldTimer();
  ...
  const primaryButtonDisabled =
    primaryButtonKind === "send"
      ? !isStreaming &&
        (disabled || !hasInput || hasPendingUploads || hasUploadErrors)
  ...
  const primaryButtonClassName = cn(
    "chat-input-primary-button inline-flex h-[36px] w-[36px] items-center justify-center self-end rounded-md text-sm font-medium transition-colors focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2 focus-visible:ring-offset-background focus-visible:outline-none disabled:pointer-events-none disabled:opacity-50 pointer-coarse:h-11 pointer-coarse:w-11",
    primaryButtonKind === "stop"

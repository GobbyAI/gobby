...
function RichContentBlock({ block, id }: { block: ContentBlock; id: string }) {
  ...
  if (block.type === "resource") {
    ...
    const title =
      stringValue(resource.name) || stringValue(resource.uri) || "Resource";
  ...
  if (block.type === "audio") {
    ...
    if (!src)
      return <div className="text-xs text-muted-foreground">Audio content</div>;
    return (
      <audio src={src} controls aria-label="Audio content" className="w-full" />
    );
  }

...
const COMPACT_HEADER_TOOL_TYPES = new Set([
  "read",
  "bash",
  "grep",
  "glob",
...
function getMcpEnvelopeCandidate(content: unknown): unknown {
  const parsedContent =
    typeof content === "string" ? tryParseJsonValue(content) : content;
  if (!isRecord(parsedContent) || !("output" in parsedContent))
    return parsedContent;

  const output =
    typeof parsedContent.output === "string"
...
function normalizeDisplayResult(
  ...
  if (isRecord(innerContent) && mergedMetadata.response_time_ms != null) {
    return {
      content:
        innerContent.response_time_ms == null
          ? {
              ...innerContent,
              response_time_ms: mergedMetadata.response_time_ms,
            }
          : innerContent,
...
export function getToolSummary(call: ToolCall): string | null {
  ...
  switch (name) {
    ...
    case "Agent": {
      const desc = args.description as string;
      const agentType = args.subagent_type as string;
      const parts = [agentType, desc ? truncStr(desc, 50) : null].filter(
        Boolean,
      );
...
export function groupToolCalls(toolCalls: ToolCall[]): ToolCallSegment[] {
  ...
  while (i < toolCalls.length) {
    ...
    if (
      UNGROUPABLE_TOOLS.has(call.tool_name) ||
    ...
    if (j - i >= 3) {
      ...
      segments.push({
        ...
        hasInFlight: calls.some(
          (entry) => entry.status === "calling" || entry.status === "pending",
        ),
      });
    } else {
      for (let k = i; k < j; k++) {
...
export function unwrapMcpResultEnvelope(
  value: unknown,
): McpEnvelopeUnwrap | null {
    ...
    if (textBlocks.length > 0) {
      ...
      const meta =
        remainingBlockCount > 0
          ...
          : rest;
      return {
        primary: textBlocks.map((block) => block.text).join("\n"),
        meta,
      };
    }
  }

...
describe("PlanApprovalActions", () => {
  ...
  it("falls back to a single Approve when options is empty", () => {
    ...
    expect(screen.getByTestId("x-approve")).toBeInTheDocument();
    expect(
      screen.queryByTestId("x-option-approve_yolo"),
  ...
  it("maps emphasis to button hierarchy: YOLO solid primary, Act tinted accent", () => {
    ...
    expect(
      screen.getByTestId("x-option-approve_yolo").className.split(/\s+/),
    ).toContain("bg-accent");
    expect(
      screen.getByTestId("x-option-approve_act").className.split(/\s+/),
    ).toContain("bg-accent-tint");
  });
  ...
  it("holds the single-accent contract: exactly one solid primary, however many approve options (#15680)", () => {
    ...
    expect(
      screen.getByTestId("x-option-approve_edits").className.split(/\s+/),
    ).toContain("bg-accent-tint");
  });
  ...
  it("reject opens the comment box and submits with the entered comment", () => {
    ...
    fireEvent.change(screen.getByTestId("x-feedback"), {
      target: { value: "tweak step 2" },
    });
  ...
  it("never shows a manual-switch note (approval now executes on every CLI)", () => {
    ...
    expect(
      screen.queryByTestId("plan-manual-switch-note"),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByText(/stays in plan mode after approval/i),
    ).not.toBeInTheDocument();
  });
  ...
  it("inline layout (default) keeps one wrapping row and no full-width buttons", () => {
    ...
    expect(screen.getByTestId("x-reject").parentElement?.className).toContain(
      "flex-wrap",
    );
  });
});

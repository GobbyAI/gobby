...
describe("extractResultContent", () => {
  ...
  it("parses and flattens stringified MCP proxy envelopes", () => {
    const result = {
      content:
        '{"success":true,"result":{"task_id":"#11820"},"response_time_ms":42}',
      kind: "text",
      truncated: false,
      metadata: { source: "mcp" },
    };
...
describe("getLanguageFromPath", () => {
  ...
  it("handles full paths", () => {
    expect(getLanguageFromPath("/home/user/project/src/main.py")).toBe(
      "python",
    );
  });
});
...
describe("defaultExpandedForCall", () => {
  ...
  it("expands edit/write tools", () => {
    expect(
      defaultExpandedForCall(makeCall({ id: "1", tool_name: "Edit" })),
    ).toBe(true);
    expect(
      defaultExpandedForCall(makeCall({ id: "2", tool_name: "Write" })),
    ).toBe(true);
    expect(
      defaultExpandedForCall(makeCall({ id: "3", tool_name: "multiedit" })),
    ).toBe(true);
  });

  it("collapses read/grep/glob/protocol tools", () => {
    expect(
      defaultExpandedForCall(makeCall({ id: "1", tool_name: "Read" })),
    ).toBe(false);
    expect(
      defaultExpandedForCall(makeCall({ id: "2", tool_name: "Grep" })),
    ).toBe(false);
    expect(
      defaultExpandedForCall(makeCall({ id: "3", tool_name: "Glob" })),
    ).toBe(false);
    expect(
      defaultExpandedForCall(
        makeCall({ id: "4", tool_name: "protocol_context" }),
      ),
    ).toBe(false);
  });
  ...
  it("collapses unknown tool types so the header is sufficient", () => {
    const unknown = makeCall({
      id: "1",
      tool_name: "totally-not-a-thing",
      tool_type: "",
    });

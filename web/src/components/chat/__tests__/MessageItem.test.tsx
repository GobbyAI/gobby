...
describe("MessageItem", () => {
  ...
  it("renders thinking block when thinkingContent exists", () => {
    render(
      <MessageItem
        message={makeMessage({ thinkingContent: "Let me think..." })}
      />,
    );
  ...
  it("suppresses known protocol metadata while preserving unknown payloads", () => {
    const blocks = [
      {
        type: "unknown",
        block_type: "turn_completed",
        raw: { stop_reason: "end_turn" },
      },
      { type: "unknown", block_type: "retry_state", raw: { attempt: 1 } },
      {
        type: "unknown",
        block_type: "ui_telemetry",
        raw: { event: "prompt_submitted" },
      },
      {
        type: "unknown",
        block_type: "file_history_snapshot",
        raw: { files: [] },
      },
  ...
  it("renders stored image attachments inline", () => {
    ...
    expect(img.getAttribute("src")).toBe(
      "/api/chat/attachments/att-img/content",
    );
  });

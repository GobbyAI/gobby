...
describe("extractBase64Image", () => {
  ...
  it("detects MCP/Anthropic image content block", () => {
    ...
    expect(extractBase64Image(result)).toBe(
  ...
  it("returns null for malformed image objects", () => {
    ...
    expect(
      extractBase64Image({ type: "image", source: { type: "url" } }),
    ).toBeNull();
    expect(
      extractBase64Image({
        type: "image",
        source: { type: "base64", data: 123 },
      }),
    ).toBeNull();
  });
});
...
describe("ToolCallCards rendering", () => {
  it("wraps generic JSON arguments without horizontal overflow", () => {
    const call = makeCall({
      ...
      arguments: {
        url: "https://example.test/a/very/long/path/that/should/wrap/instead/of/forcing/a/horizontal/scrollbar?with=query-values-and-more-values",
      },
    });
    ...
    expect(
      (jsonBlock as HTMLElement).querySelector("span[style]"),
    ).not.toBeNull();
  ...
  it("renders JSON results without forcing horizontal scroll on long values", () => {
    const call = makeCall({
      ...
      result: {
        content: {
          url: "https://example.test/a/very/long/result/path/that/should/wrap/instead/of/forcing/a/horizontal/scrollbar?with=query-values-and-more-values",
        },
        kind: "json",
        truncated: false,
      },

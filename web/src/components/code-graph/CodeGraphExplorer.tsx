...
export function CodeGraphExplorer({ projectId }: CodeGraphExplorerProps) {
  ...
  return (
    <div
      className="relative h-full w-full overflow-hidden bg-[var(--bg-primary)] [background-image:radial-gradient(circle_at_50%_50%,color-mix(in_srgb,var(--color-info)_2%,transparent),transparent_70%)]"
      ref={containerRef}
    >
      {/* Controls */}
      ...
      <div className="absolute top-3 left-3 z-10 w-[260px]">
        <Input
          ...
          className="h-auto rounded border-[var(--border)] bg-[var(--bg-secondary)] px-2.5 py-1.5 font-mono text-[length:var(--text-sm)] text-[var(--text-primary)] placeholder:text-[var(--text-muted)]"
        />
        {searchResults.length > 0 && (
          <Card className="mt-1 max-h-[240px] overflow-y-auto rounded border-[var(--border)] bg-[var(--bg-secondary)]">
            {searchResults.map((r) => (
              <Button
                key={r.id}
                variant="ghost"
      ...
      <Card className="absolute right-3 bottom-3 z-10 flex flex-col gap-[3px] rounded border-[var(--border)] bg-[color-mix(in_srgb,var(--bg-primary)_85%,transparent)] px-2.5 py-2">
        {Object.keys(NODE_COLOR_VARS)
          ...
          .map((type) => (
            <div
              key={type}
              className="flex items-center gap-1.5 font-mono text-[length:var(--text-2xs)] text-[var(--text-secondary)]"
            >
              <span
                className="h-2 w-2 shrink-0 rounded-full"
                style={{ background: getNodeColorCss(type) }}
              />
              <span>{type}</span>
            </div>
          ))}
        ...
        {Object.keys(EDGE_COLOR_VARS).map((type) => (
          ...
          >
            <span
              ...
              style={{ background: `var(${edgeColorVar(type)})` }}
            />
      ...
      {selectedNode && (
        <Card className="absolute top-[50px] right-3 z-10 w-[250px] rounded-md border-[var(--border)] bg-[var(--bg-secondary)] p-3">
          ...
          {selectedNode.symbol_count !== undefined && (
            <div className="font-mono text-[length:var(--text-2xs)] text-[var(--text-muted)]">
              {selectedNode.symbol_count} symbols
            </div>
          )}
        </Card>
      )}

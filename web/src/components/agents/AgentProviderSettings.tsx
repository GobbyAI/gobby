...
export function AgentProviderSettings({
  ...
  const providerOptions =
    discoveredProviders.length > 0 ? discoveredProviders : FALLBACK_PROVIDER_OPTIONS
  ...
  const showCustomModel =
    !isInheritProvider && (customModelInput || (!isKnownModel && form.model !== ''))
  ...
  const showCustomBranch =
    isGitProject && (customBranchInput || (!branchKnown && form.base_branch !== ''))
      ...
      <MetaRow label="Provider">
        <NativeSelect
          ...
          onChange={event => {
            ...
            const providerModels = getModelsForProvider(providerCatalog, provider)
            const model = providerModels.some(option => option.value === form.model)
              ? form.model
              : ''
          ...
          <div className="flex items-center gap-1">
            ...
            >
              {agentNames
                ...
                <option value={form.fallback_agent}>{form.fallback_agent} (missing)</option>
              )}
              {agentNames
                .filter(name => name !== form.name)
                .map(name => (
      ...
      <MetaRow label="Require support">
        ...
        >
          <Input
            ...
            onChange={event => set('reasoning_required', event.target.checked)}
          />
          <span>

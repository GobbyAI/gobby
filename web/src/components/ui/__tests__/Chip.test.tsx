...
describe('Chip', () => {
  ...
  it.each([
    ...
    expect(chip.className).toContain(`bg-[color-mix(in_srgb,${token}_15%,transparent)]`)
  })
  ...
  it('lets call-site classes win over the tone fill', () => {
    ...
    expect(chip.className).toContain('text-[color:var(--color-warning-foreground)]')
  })
  ...
  it('composes the session identity treatment with the accent tone', () => {
    ...
    expect(chip).toHaveClass('font-mono', 'border', 'border-border', 'uppercase')
    ...
    expect(chip.className).toContain('[[data-theme=light]_&]:text-muted-foreground')
  })
})
...
describe('Chip tone ladder non-hue cues', () => {
  ...
  function lightness(themeCss: string, name: string): number {
    const match = new RegExp(`--${name}:\\s*oklch\\((\\d+(?:\\.\\d+)?)%`).exec(themeCss)
  ...
  it.each(Object.entries(STATE_TONE_TOKENS))(
    ...
    (tone, token) => {
      expect(chipVariants({ tone: tone as keyof typeof STATE_TONE_TOKENS })).toContain(
        `var(--${token})`,
      )
    },
  )

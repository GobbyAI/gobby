...
describe('ToolBrowserModal', () => {
  ...
  it('keeps the selected tool schema when an older request resolves last', async () => {
    const toolA = deferred<{ description: string; inputSchema: { type: string } }>()
    const toolB = deferred<{ description: string; inputSchema: { type: string } }>()
    mcpMock.fetchToolSchema.mockImplementation((_serverName: string, toolName: string) =>
    ...
    await act(async () => {
      toolB.resolve({ description: 'Schema for B', inputSchema: { type: 'object' } })
    })
    expect(screen.getByText('Schema for B')).toBeInTheDocument()

    await act(async () => {
      toolA.resolve({ description: 'Schema for A', inputSchema: { type: 'object' } })
    })
    expect(screen.getByText('Schema for B')).toBeInTheDocument()

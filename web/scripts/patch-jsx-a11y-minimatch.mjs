...
const previousReplacement = '(0, _minimatch["default"].minimatch)(elementType';
...
if (source.includes(marker)) {
  if (source.includes(previousReplacement)) {
    writeFileSync(targetPath, source.replace(previousReplacement, replacement));
    console.log("eslint-plugin-jsx-a11y minimatch patch: updated");
    process.exit(0);
  }
  if (!source.includes(replacement)) {
    throw new Error(
      `Unexpected patched eslint-plugin-jsx-a11y contents in ${targetPath}`,
    );
  }
...
if (!source.includes(original)) {
  throw new Error(
    `Unexpected eslint-plugin-jsx-a11y contents in ${targetPath}`,
  );
}

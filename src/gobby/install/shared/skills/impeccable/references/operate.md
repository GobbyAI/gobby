...
When design SERVES the product: app UIs, admin dashboards, settings panels, data tables, tools, authenticated surfaces, anything where the user is in a task. The essentials live in the main skill's visitor-mode section and `craft-floor.md` (load via `get_skill_file(name="impeccable", path="references/craft-floor.md")` on `gobby-skills`); this file is extended depth, written for Operate surfaces. Read surfaces (docs, guides, long-form) take the main skill's Read mode plus this file's typography and consistency rules; their prose measure and navigation matter more than component density.

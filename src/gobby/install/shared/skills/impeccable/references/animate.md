> You are continuing a session under the `impeccable` skill; the design-context protocol and anti-pattern rules already apply. Additional context needed: performance constraints.
...
When motion earns its place, hand off to the `polish` steering command (load via `get_skill_file(name="impeccable", path="references/polish.md")` on `gobby-skills`) for the final pass.

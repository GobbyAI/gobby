> You are continuing a session under the `impeccable` skill; the design-context protocol and anti-pattern rules already apply. Additional context needed: the brand's emotional range.
...
Inspect the target, `.impeccable.md` (the project design contract at the repo root), product voice, repeated-use frequency, and emotional context. Look for:
...
For authored motion, load `animate.md` (via `get_skill_file(name="impeccable", path="references/animate.md")` on `gobby-skills`). Respect screen readers, keyboard use, touch, localization, and cultural context. Nonessential loops stop when hidden. Make celebration intensity proportional to frequency and consequence.
...
When the personality feels earned, hand off to the `polish` steering command (load via `get_skill_file(name="impeccable", path="references/polish.md")` on `gobby-skills`) for the final pass.

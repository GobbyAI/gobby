...
Never ask for CSS values or canned aesthetic lanes. Visual-world and concept choices belong to the design-direction step (Phase 2), not the interview.

## Phase 2: Resolve the design direction

For new surfaces, brand expansion, or replacement, commit to a visual world and concept direction using the main `impeccable` skill's Design Direction guidance and `.impeccable.md` (the project design contract) where it already fixes these choices. Reuse discovery; stop before implementation. Inside an established world, revisit concept choice only when composition or interaction remains materially open.

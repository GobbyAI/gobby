...
Preserve the established visual world. A layout command changes structure inside it; identity replacement belongs to the `craft` flow (load via `get_skill_file(name="impeccable", path="references/craft.md")` on `gobby-skills`).
...
```bash
node <scripts_dir>/detect.mjs --json --scope layout [target files or dirs]
```
...
When the structure holds, hand off to the `polish` steering command (load via `get_skill_file(name="impeccable", path="references/polish.md")` on `gobby-skills`).

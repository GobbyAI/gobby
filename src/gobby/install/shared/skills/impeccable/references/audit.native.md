...
This is a code-level audit, not a design critique. Audit from source (SwiftUI / UIKit / Compose / React Native / Flutter); no browser tooling or `detect.mjs` applies. Score against the platform references: call `get_skill_file(name="impeccable", path="references/ios.md")` and/or `get_skill_file(name="impeccable", path="references/android.md")` on `gobby-skills`, using both for `adaptive`. Read them before scoring if they are not already loaded. The report skeleton mirrors `audit`: call `get_skill_file(name="impeccable", path="references/audit.md")` on `gobby-skills` when changing it and keep the two in sync.

> You are continuing a session under the `impeccable` skill; the design-context protocol and anti-pattern rules already apply. Additional context needed: which section is the target, and what must stay untouched.
...
When the target holds its own without pulling the page apart, hand off to the `polish` steering command (load via `get_skill_file(name="impeccable", path="references/polish.md")` on `gobby-skills`) for the final pass.

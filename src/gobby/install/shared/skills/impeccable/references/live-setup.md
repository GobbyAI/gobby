> You are continuing an ordinary Gobby session under the `impeccable` skill; the live-mode contract and design-context protocol already apply.

# Live-mode setup

One-time project setup, loaded by the `live` flow through `get_skill_file` only
when `live.mjs` reports `config_missing` / `config_invalid`, when `configDrift`
needs handling, or when the config lacks `cspChecked`. It is outside the
per-session hot path and does not manage the app dev server or Gobby daemon.

Reuse the `<scripts_dir>` resolved by `references/live.md`. If this reference
was loaded independently, call `materialize_skill_scripts(name="impeccable")`
...
```bash
node <scripts_dir>/detect-csp.mjs
```

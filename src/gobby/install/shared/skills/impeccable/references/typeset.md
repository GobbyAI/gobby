...
If typography replacement would create a new identity, treat it as a redesign: confirm the direction with the user and update `.impeccable.md` (the project design contract) via the impeccable skill's teach mode. Otherwise preserve confirmed families and improve their use.
...
When the hierarchy holds, hand off to the `polish` steering command (load via `get_skill_file(name="impeccable", path="references/polish.md")` on `gobby-skills`).

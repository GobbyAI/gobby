> You are continuing a session under the `impeccable` skill; the design-context protocol and anti-pattern rules already apply. Additional context needed: the "aha moment" you want users to reach, and users' experience level.
...
When users hit the aha moment fast and don't drop off, call `get_skill_file(name="impeccable", path="references/polish.md")` on `gobby-skills` and follow it for the final pass.

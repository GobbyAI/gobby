...
Reviewer-miss requires every participating section hash unchanged since an earlier finalized round. Fixer-induced-defect requires changed causal section hashes, causal_finding_id and introduced_in_round. Both classes need both proof bundles. Use review-learning's class-scoped identity, source_kind plan_review, guardrail_target checklist, rule_id plan-review:<category>; a plan-file path is not a promotion anchor. Mint at most five proven lessons per plan, reserving a slot per present class and using its deterministic ranking. No proof means no lesson.
If implementation handoff is requested while a reviewer is active, mark it pending, finish the run, votes, accepted edits and checkpoints before handing off; launch no new optional round.

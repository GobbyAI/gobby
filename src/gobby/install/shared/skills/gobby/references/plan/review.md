...
Derive shadow manifest through derive_plan_review_manifest even on rejection. Call validate_plan_review_coverage with complete lanes, dispositions and exact derivation status: the unmodified derive_plan_review_manifest result, ok included. Preserve its returned attestation verbatim. If it fails, return protocol_failure with exact tool error and draft findings, no verdict.

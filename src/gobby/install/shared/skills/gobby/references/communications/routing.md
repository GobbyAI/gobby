...
channel `default_destination`, then a linked session identity's stored
conversation reference. A configured default can therefore override a linked
session's destination. Teams also needs `service_url` or a stored reference.

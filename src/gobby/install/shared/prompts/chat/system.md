...
Everything runs locally. The runtime hub is PostgreSQL reached via bootstrap/keyring `database_url` settings. Bootstrap at ~/.gobby/bootstrap.yaml holds pre-DB settings. Runtime config lives in the PostgreSQL hub (config_store table). HTTP on :60887, WebSocket on :60888. No cloud dependency. Git is the source of truth — tasks sync to .gobby/tasks.jsonl so they travel with the repo.
...
gobby-tasks, gobby-sessions, gobby-memory, gobby-workflows, gobby-agents, gobby-worktrees, gobby-clones, gobby-skills, gobby-metrics, gobby-hub, gobby-merge.

...
class PipelineStepStorageMixin:
    ...
    def update_step_execution(
        ...
        self.db.execute(
            # updates contains only fixed column assignments assembled above.
            f"UPDATE step_executions SET {', '.join(updates)} WHERE id = %s",  # nosec
            tuple(params),
        )
    ...
    def consume_step_approval(
        ...
        *,
        ...
        """
        if status not in (StepStatus.PENDING, StepStatus.FAILED):
            raise ValueError("Approval decisions must reset or fail the step")

        now = utc_now()
        approved_at = now if status == StepStatus.PENDING else None
        completed_at = now if status == StepStatus.FAILED else None
    ...
    def get_expired_approval_steps(self, *, limit: int = 100) -> list[StepExecution]:
        ...
        project_predicate = "1 = 1" if self.project_id is None else "pe.project_id = %s"
        project_params = () if self.project_id is None else (self.project_id,)
        params: tuple[Any, ...] = (
            StepStatus.WAITING_APPROVAL.value,
            *project_params,
            limit,
        ...
        rows = self.db.fetchall(
            # Both fragments are generated by the internal storage dialect.
            f"""
...
""",  # nosec
    ...
    def get_steps_for_executions(self, execution_ids: list[str]) -> dict[str, list[StepExecution]]:
        ...
        rows = self.db.fetchall(
            # placeholders is a generated comma-separated sequence of %s parameters.
            f"SELECT * FROM step_executions WHERE execution_id IN ({placeholders}) ORDER BY id",  # nosec
            tuple(execution_ids),
        )

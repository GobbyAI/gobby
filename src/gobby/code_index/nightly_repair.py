"""Nightly code-index repair and graph reconciliation."""
...
CODE_INDEX_NIGHTLY_REPAIR_JOB_NAME = "gobby:code-index-nightly-repair"
CODE_INDEX_NIGHTLY_REPAIR_HANDLER = "code-index:nightly-repair"
CODE_INDEX_NIGHTLY_REPAIR_DESCRIPTION = "Nightly code-index repair and graph reconciliation"
...
class CodeIndexNightlyRepairer:
    """Runs one gcode repair pass across all indexed projects."""
    ...
    async def run_once(self) -> str:
        if self._running:
            return "Code index nightly repair skipped: already running"
    ...
    async def _run_locked(self) -> str:
        ...
        if gateway is None:
            return "Code index nightly repair skipped: gcode gateway unavailable"
        factory = self._context.launch_factory
        if factory is None:
            logger.error("Nightly repair skipped: launch factory is not configured")
            return "Code index nightly repair skipped: launch factory is not configured"

        projects = await self._context.run_db(self._context.storage.list_indexed_projects)
        concurrency = max(1, int(config.nightly_repair_concurrency))
        semaphore = asyncio.Semaphore(concurrency)
        run_id = uuid4().hex

        async def repair_project(project: Any) -> str:
            ...
            try:
                ...
                async with semaphore:
                    ...
                    async with open_launch_async(
                        ...
                        result = await gateway.nightly_repair(root, timeout=timeout, env=launch.env)
                    if result.timed_out:
                        status = "timed_out"
                    elif result.success:
                        status = "completed"
                ...
                logger.exception(
                    "Code index nightly repair failed for %s at %s",
                    project_id,
                    root_path,
                )
        ...
        if concurrency == 1:
            outcomes = [await repair_project(project) for project in projects]
        else:
            outcomes = await asyncio.gather(*(repair_project(project) for project in projects))
        ...
        return (
            "Code index nightly repair completed: "
            f"run_id={run_id} completed={completed} failed={failed} skipped={skipped}"
        )


def create_code_index_nightly_repair_handler(
    repairer: CodeIndexNightlyRepairer,
) -> CronHandler:
    async def _handler(_job: CronJob) -> str:
        return await repairer.run_once()

    return _handler


def register_code_index_nightly_repair_cron(
    *,
    ...
    """Register the global nightly repair system cron job."""
    cron_executor.register_handler(
        CODE_INDEX_NIGHTLY_REPAIR_HANDLER,
        create_code_index_nightly_repair_handler(repairer),
    )
    timezone = resolve_local_timezone(config.nightly_repair_timezone)
    enabled = bool(config.nightly_repair_enabled)
    action_config = {
        "handler": CODE_INDEX_NIGHTLY_REPAIR_HANDLER,
        "purpose": CODE_INDEX_NIGHTLY_REPAIR_DESCRIPTION,
        "timeout_seconds": config.nightly_repair_timeout_seconds,
        "concurrency": config.nightly_repair_concurrency,
        "log_file": config.maintenance_log_file,
    }

    existing = cron_storage.get_job_by_name(CODE_INDEX_NIGHTLY_REPAIR_JOB_NAME)
    if existing is None:
        cron_storage.create_job(
            project_id=project_id or PERSONAL_PROJECT_ID,
            name=CODE_INDEX_NIGHTLY_REPAIR_JOB_NAME,
            description=CODE_INDEX_NIGHTLY_REPAIR_DESCRIPTION,
            schedule_type="cron",
            cron_expr=config.nightly_repair_cron,
            timezone=timezone,
            action_type="handler",

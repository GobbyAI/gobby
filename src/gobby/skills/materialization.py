"""Materialize installed skill revisions into immutable cache generations."""
...
class SkillMaterializationResult:
    ...
    parser_deps: PreparationResult
    browser: PreparationResult
    node: NodeRuntimeResult
    ...
    def to_tool_result(self) -> dict[str, Any]:
        return {
            ...
            "parser_deps": {
                "installed": self.parser_deps.ready,
                "warning": self.parser_deps.warning,
            },
            "browser": {"ready": self.browser.ready, "warning": self.browser.warning},
            "node": {
                "version": self.node.version,
                "satisfies_floor": self.node.satisfies_floor,
            },
        }


class SkillScriptMaterializer:
    """Resolve installed revisions with project-scoped in-process single-flight."""
    ...
    async def _materialize(
        ...
        *,
        ...
        snapshot = await _run_db(
            run_db,
            self._storage.get_skill_with_scripts,
            name=name,
            project_id=project_id,
        )
        return await _materialize_snapshot(snapshot, name=name)


def get_skill_script_materializer(
    db: HubDatabase,
    *,
    storage: LocalSkillManager | None = None,
) -> SkillScriptMaterializer:
    """Return the process-wide materializer for a database connection."""
    with _MATERIALIZERS_LOCK:
        materializer = _MATERIALIZERS.get(db)
        if materializer is None:
            materializer = SkillScriptMaterializer(db, storage)
            _MATERIALIZERS[db] = materializer
        return materializer
...
async def _materialize_snapshot(
    ...
    *,
    ...
    return SkillMaterializationResult(
        scripts_dir=(generation / "scripts").resolve(),
        files_written=len(payloads),
        environment={"PUPPETEER_CACHE_DIR": str(browser_cache)},
        parser_deps=PreparationResult(dependency_installed, parser_warning),
        browser=PreparationResult(browser_ready, browser_warning),
        node=NodeRuntimeResult(node.version, node.satisfies_floor),
    )
    ...
    while True:
        if root.exists():
            async with async_export_file_lock(skill_scripts_root_lock_target(root)):
                ...
                if generation.exists() and not await run_blocking_safely(
                    _generation_complete, generation, payloads
                ):
                    await run_blocking_safely(_remove_generation, generation)
...
def _generation_complete(generation: Path, payloads: list[_ScriptPayload]) -> bool:
    ...
    for payload in payloads:
        relative = PurePosixPath(payload.path).relative_to("scripts")
        target = scripts.joinpath(*relative.parts)
        if not target.is_file() or _file_hash(target) != payload.content_hash:
            return False
    return True

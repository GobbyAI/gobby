"""Managed bounded executor for daemon-owned blocking work."""
...
class ManagedExecutorStats:
    """Diagnostic snapshot for a managed executor."""
...
class ManagedThreadPoolExecutor:
    """Bounded async bridge with admission, cancellation, and drain controls."""
    ...
    def __init__(
        ...
        *,
        ...
        self._queue_wait_metric = queue_wait_metric
        ...
        self._join_finished = threading.Event()

    async def run(self, func: Callable[..., T], *args: Any, **kwargs: Any) -> T:
        """Run blocking work on the bounded executor."""
    ...
    def submit(self, func: Callable[..., T], *args: Any, **kwargs: Any) -> Future[T]:
        """Submit blocking work for synchronous callers."""
        context = contextvars.copy_context()
        with self._lock:
            if self._shutdown:
                raise RuntimeError(f"{self._executor_name} is shut down")
    ...
    def _execute(
        ...
        **kwargs: Any,
        ...
        with self._lock:
            ...
            self._active += 1
        observe_histogram(self._queue_wait_metric, time.monotonic() - queued_at)
        try:
    ...
    def join(self) -> None:
        ...
        with self._lock:
            if self._join_started:
                ...
                owns_join = True
        if not owns_join:
            self._join_finished.wait()
            return
        try:
            self._executor.shutdown(wait=True, cancel_futures=False)
        finally:
            self._join_finished.set()

    def is_joined(self) -> bool:
        """Return whether running work has completely drained."""
        return self._join_finished.is_set()

...
_GENERATED_HEADER_RE = re.compile(
    r"@generated|DO NOT EDIT|\bauto-generated\b|\bGenerated by\b",
    re.IGNORECASE,
)
_RUST_TEST_MODULE_RE = re.compile(r"^\s*#\[cfg\(test\)\]")
_PRIMARY_TOKEN_SEPARATOR = " \u2014 "
...
def lint_plan_document(
    ...
    *,
    ...
    if project_root is None:
        warnings.append("production-size-growth skipped: no project root")
...
def collect_strict_target_inventory(
    plan_doc: PlanDocument,
    section: PlanSection,
) -> frozenset[str]:
    """Return the primary file path of each Target/Targets inventory entry.

    Unlike :func:`collect_target_inventory`, this reads one path per entry —
    the first backticked span, else the first path token — and ignores
    scope-reasons and commentary after ``" — "``.
    """

    targets: set[str] = set()
    for line in iter_target_block_lines(plan_doc, section):
        primary = _primary_target_path(line)
        if primary is not None:
            targets.add(primary)
    return frozenset(targets)


def _primary_target_path(line: str) -> str | None:
...
def _lint_shared_target_ordering(plan_doc: PlanDocument) -> list[SemanticLintIssue]:
    ...
    sections_by_id = {section.section_id: section for section in deliverables}
    sections_by_file: dict[str, list[str]] = {}
    for section in deliverables:
        for file_path in collect_strict_target_inventory(plan_doc, section):
        ...
        for first, second in combinations(dict.fromkeys(section_ids), 2):
            ...
            issues.append(
                SemanticLintIssue(
                    ...
                    section_id=second,
                    line=sections_by_id[second].source_span[0],
                    message=(
...
def _lint_production_size_growth(
    ...
    issues: list[SemanticLintIssue] = []
    targets = collect_strict_target_inventory(plan_doc, section)
...
def _lint_derived_carriers(plan_doc: PlanDocument) -> list[SemanticLintIssue]:
    ...
    graph = _dependency_graph(plan_doc, deliverables)
    targets_by_section = {
        section.section_id: collect_strict_target_inventory(plan_doc, section)
        for section in deliverables
    }
    issues: list[SemanticLintIssue] = []
    for section in deliverables:
...
def _lint_dependency_resolution(plan_doc: PlanDocument) -> list[SemanticLintIssue]:
    deliverables = _deliverables(plan_doc)
        ...
        for ref in section_dependency_refs(section, sections_by_id):
            ...
            issues.append(
                SemanticLintIssue(
                    ...
                    line=section.source_span[0],
                    message=(
                        f"dependency reference {ref!r} does not resolve to a deliverable "
                        "section or a phase with deliverables"
                    ),
...
def _dependency_graph(
    ...
    for section in deliverables:
        for ref in section_dependency_refs(section, sections_by_id):
            ...
            graph[section.section_id].update(set(resolved) - {section.section_id})
    return graph
...
def _line_count(path: Path) -> int:
    """Count production lines; a Rust ``#[cfg(test)]`` tail is not production."""
    count = 0
    try:
        with path.open(encoding="utf-8", errors="ignore") as source:
            for line in source:
                if path.suffix == ".rs" and _RUST_TEST_MODULE_RE.match(line):
                    break
                count += 1
    except OSError:
        return 0

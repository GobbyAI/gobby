...
class OutboundCommunications:
    ...
    def enrich_metadata(
        ...
        if "platform_destination" not in effective:
            ...
            if not effective.get("platform_destination") and session_id:
                identity = manager._identity_manager.get_identity_by_session(channel.id, session_id)
    ...
    async def send_message(
        ...
        message = CommsMessage(
            ...
            metadata_json=self.enrich_metadata(channel, channel_name, session_id, metadata),
    ...
    async def send_attachment(
        ...
        try:
            manager._store.create_message(message)
            manager._store.create_attachment(attachment)
        except Exception as e:

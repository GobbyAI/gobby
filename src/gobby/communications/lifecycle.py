...
class AdapterLifecycleOperations:
    ...
    async def start(self) -> None:
        ...
        channels = manager._store.list_channels(enabled_only=True)
    ...
    async def init_adapter(self, channel: ChannelConfig) -> BaseChannelAdapter:
        ...
        await adapter.initialize(channel, manager._secret_store.get)
        return adapter
    ...
    async def add_channel(
        ...
        if secrets:
            for key, value in secrets.items():
                if key == "webhook_secret" or not value:
                    continue
        ...
        manager._store.create_channel(channel_config)

        try:
            adapter = await self.init_adapter(channel_config)
            manager._adapters[name] = adapter
            manager._channel_by_name[name] = channel_config
            self._configure_rate_limit(channel_config, config)
            self._start_polling_if_needed(name, channel_config, adapter)
    ...
    async def remove_channel(self, name: str) -> None:
        ...
        channel = manager._channel_by_name.pop(name, None)
        ...
        if channel is not None:
            ...
            try:
                manager._store.delete_channel(channel.id)
    ...
    async def ensure_gobby_chat_channel(self) -> None:
        ...
        try:
            manager._store.create_channel(channel)
    ...
    def _start_polling_if_needed(
        ...
        manager = self._manager
        if adapter.supports_polling and not manager._config.webhook_base_url:

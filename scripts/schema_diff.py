...
import os
import re
import subprocess  # nosec B404 - fixed pg_dump/psql argv, never shell=True
import sys
import uuid
...
from gobby.storage.migrations import _split_statements_respecting_dollar_quotes
...
def normalize_schema_dump(
    ...
    *,
    ...
    statements: list[str] = []
    for raw_statement in _split_statements_respecting_dollar_quotes(_strip_dump_noise(dump)):
...
def _scoped_conninfo(database_url: str, schema_name: str) -> str:
    raw_options = conninfo_to_dict(database_url).get("options")
    current_options = str(raw_options) if raw_options is not None else ""
    search_path_option = f"-csearch_path={schema_name}"
    options = " ".join(part for part in (current_options, search_path_option) if part)
    return make_conninfo(database_url, options=options)
...
def _apply_reconcile_migration(database_url: str, schema_name: str) -> None:
    migration = _RECONCILE_MIGRATION.read_text(encoding="utf-8")
    with psycopg.connect(database_url) as conn:
        conn.execute(sql.SQL("SET search_path TO {}").format(sql.Identifier(schema_name)))
        for statement in _split_statements_respecting_dollar_quotes(migration):
            if statement.strip():
                conn.execute(statement)


def _project_live_schema(
    database_url: str,
    live_dump: str,
    *,
    live_schema: str,
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Diff a fresh migration-built PostgreSQL schema against the live hub.",
        epilog=(
            "Examples:\n"
...
def run(args: argparse.Namespace) -> int:
    ...
    with _scratch_schema(database_url, "fresh") as fresh_schema:
        fresh_db = PostgresHubDatabase(_scoped_conninfo(database_url, fresh_schema))
        try:
            fresh_db.apply_migrations()
        finally:
            fresh_db.close()
        ...
        comparison_normalized = artifacts["live.normalized.sql"]
        comparison_label = f"live:{live_schema}"
        if live_head < _RECONCILE_MIGRATION_VERSION:
            if live_head != _RECONCILE_MIGRATION_VERSION - 1:
                errors.append(
                    f"live head {live_head} cannot be projected through migration "
                    f"{_RECONCILE_MIGRATION_VERSION}"
                )
            else:
                with _scratch_schema(database_url, "projection") as projection_schema:
                    # pg_dump includes CREATE SCHEMA, so restore into a name that does not exist yet.
                    with psycopg.connect(database_url, autocommit=True) as conn:
                        conn.execute(
                            sql.SQL("DROP SCHEMA {}").format(sql.Identifier(projection_schema))
                        )
                    _project_live_schema(
                        database_url,
                        live_dump,
                    ...
                    projected_dump = _dump_schema(
                        database_url,
                        projection_schema,
                        pg_dump=args.pg_dump,
                    )
                    comparison_normalized = normalize_schema_dump(
                        projected_dump,
                        schema_name=projection_schema,
                        accepted_tables=accepted_tables,
                    )
                    comparison_label = (
                        f"live:{live_schema}+migration-{_RECONCILE_MIGRATION_VERSION}"
                    )
                    artifacts["live.projected.normalized.sql"] = comparison_normalized

        schema_diff = _schema_diff(
            fresh_normalized,
            comparison_normalized,
            actual_label=comparison_label,
        )
        if schema_diff:

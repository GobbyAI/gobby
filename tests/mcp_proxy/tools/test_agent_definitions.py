...
def _setup(db: HubDatabase) -> LocalWorkflowDefinitionManager:
...
class TestListAgentDefinitions:
    def test_empty(self, temp_db: HubDatabase) -> None:
        mgr = _setup(temp_db)
    ...
    def test_with_agents(self, temp_db: HubDatabase) -> None:
        mgr = _setup(temp_db)
    ...
    def test_filter_enabled(self, temp_db: HubDatabase) -> None:
        mgr = _setup(temp_db)
    ...
    def test_filter_surface(self, temp_db: HubDatabase) -> None:
        mgr = _setup(temp_db)
...
class TestGetAgentDefinition:
    def test_found(self, temp_db: HubDatabase) -> None:
        mgr = _setup(temp_db)
    ...
    def test_not_found(self, temp_db: HubDatabase) -> None:
        mgr = _setup(temp_db)
        result = get_agent_definition(mgr, "nonexistent")
        assert result["success"] is False
        assert "not found" in result["error"]

    def test_detail_fields(self, temp_db: HubDatabase) -> None:
        mgr = _setup(temp_db)
        _insert_agent(
            mgr,
            "detailed",
...
class TestCreateAgentDefinition:
    def test_basic(self, temp_db: HubDatabase) -> None:
        mgr = _setup(temp_db)
        result = create_agent_definition(mgr, "new-agent", {"provider": "claude"})
        assert result["success"] is True
        assert result["agent"]["name"] == "new-agent"

    def test_with_all_fields(self, temp_db: HubDatabase) -> None:
        mgr = _setup(temp_db)
    ...
    def test_invalid_definition(self, temp_db: HubDatabase) -> None:
        mgr = _setup(temp_db)
        result = create_agent_definition(mgr, "bad", {"isolation": "invalid_isolation"})
        assert result["success"] is False
        assert "Validation failed" in result["error"]

    def test_persists_to_db(self, temp_db: HubDatabase) -> None:
        mgr = _setup(temp_db)
...
class TestToggleAgentDefinition:
    def test_disable(self, temp_db: HubDatabase) -> None:
        mgr = _setup(temp_db)
    ...
    def test_enable(self, temp_db: HubDatabase) -> None:
        mgr = _setup(temp_db)
...
class TestDeleteAgentDefinition:
    def test_delete_user_created(self, temp_db: HubDatabase) -> None:
        mgr = _setup(temp_db)
    ...
    def test_delete_not_found(self, temp_db: HubDatabase) -> None:
        mgr = _setup(temp_db)
        result = delete_agent_definition(mgr, "nonexistent")
        assert result["success"] is False
        assert "not found" in result["error"]

    def test_bundled_protected(self, temp_db: HubDatabase) -> None:
        mgr = _setup(temp_db)
    ...
    def test_bundled_force_delete(self, temp_db: HubDatabase) -> None:
        mgr = _setup(temp_db)
        _insert_agent(mgr, "bundled-agent", tags=["gobby"])
        result = delete_agent_definition(mgr, "bundled-agent", force=True)
        assert result["success"] is True

    def test_deleted_not_in_list(self, temp_db: HubDatabase) -> None:
        mgr = _setup(temp_db)

...
async def test_infra_failure_records_backoff_and_skips_while_active(
    ...
    first = await validate_leaf_task_with_llm(
        task, validator, "context", None, ctx, task.id, None
    )
    ...
    second = await validate_leaf_task_with_llm(
        task, validator, "context", None, ctx, task.id, None
    )
...
async def test_real_verdict_after_window_clears_backoff(
    ...
    final = await validate_leaf_task_with_llm(
        task, validator, "context", None, ctx, task.id, None
    )

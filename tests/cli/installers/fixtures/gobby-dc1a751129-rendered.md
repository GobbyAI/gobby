...
- `<trigger> tasks` — Discover, own, verify, and complete task work
- `<trigger> plan` — Draft, verify, approve, and expand implementation plans
- `<trigger> build` — Start, coordinate, monitor, and recover stage automation
- `<trigger> sessions` — Discover session work, preserve context, and inspect terminals
- `<trigger> agents` — Define, launch, coordinate, and recover agent work
- `<trigger> memory` — Recall, capture, and maintain durable project knowledge
- `<trigger> code-index` — Search, navigate, and verify source and dependency evidence
- `<trigger> skills` — Discover, load, author and manage installed skills
- `<trigger> rules` — Author, inspect and diagnose reactive hook policy
- `<trigger> mcp-servers` — Discover proxy tools and manage downstream server connections
- `<trigger> pipelines` — Run deterministic workflows and scheduled automation
- `<trigger> source-control` — Manage commits, isolated repositories and verified delivery
- `<trigger> review` — Review epic implementation, task evidence and feedback outcomes
- `<trigger> intro` — Capture a global working profile and resolve onboarding prerequisites
- `<trigger> development` — Follow contributor obligations and develop channels or native components
- `<trigger> variables` — Inspect defaults, scope and live session values
- `<trigger> config` — Inspect and update revisioned daemon configuration
- `<trigger> hub` — Identify machines and projects and query hub-wide activity
- `<trigger> communications` — Manage external channels, messages, identities, and event delivery
- `<trigger> voice` — Configure and diagnose speech recognition and playback
- `<trigger> observability` — Inspect metrics, provider capacity, usage, and daemon diagnostics
- `<trigger> admin` — Install, operate, diagnose, back up, and recover Gobby
- `<trigger> integrations` — Connect hooks, webhooks, extension packages and external issue trackers

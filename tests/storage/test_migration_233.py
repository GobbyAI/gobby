"""Red tests for migration 233 stage-registry bootstrap."""
...
def _migration_233():
    for migration in MIGRATIONS:
        if migration[0] == 233:
            return migration
    pytest.fail("migration version 233 is missing from MIGRATIONS")


def _db_before_233(tmp_path: Path) -> LocalDatabase:
    db = LocalDatabase(tmp_path / "before-233.db")
    _apply_baseline(db)
    pre_233 = [
        migration
        for migration in MIGRATIONS
        if BASELINE_VERSION < migration[0] < 233
    ]
    _run_migration_list(db, BASELINE_VERSION, pre_233)
    return db


def _apply_233(db: LocalDatabase) -> None:
    current_version = get_current_version(db)
    assert current_version < 233
    _run_migration_list(db, current_version, [_migration_233()])
...
def _yaml_payload() -> dict[str, Any]:
    assert STAGES_YAML.exists(), f"{STAGES_YAML} must be bundled with migration 233"
    payload = yaml.safe_load(STAGES_YAML.read_text(encoding="utf-8"))
    assert isinstance(payload, dict)
    return payload


def test_migration_233_registered() -> None:
    assert _migration_233()[0] == 233


def test_creates_registry_tables(tmp_path: Path) -> None:
    db = _db_before_233(tmp_path)
...
def test_fresh_db_fk_resolution_into_234(tmp_path: Path) -> None:
    ...
    assert get_current_version(db) >= 233

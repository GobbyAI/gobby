...
@pytest.fixture
def cron_storage(temp_db) -> CronJobStorage:
...
def _job(storage: CronJobStorage, *, is_system: bool) -> object:
...
@pytest.mark.parametrize("field,value", [("name", "renamed"), ("action_type", "shell"), ("action_config", {"x": 1})])
def test_system_row_definition_updates_refused(cron_storage: CronJobStorage, field: str, value: Any) -> None:
    job = _job(cron_storage, is_system=True)
...
def test_bookkeeping_partial_updates_preserve_other_fields(cron_storage: CronJobStorage) -> None:
    job = _job(cron_storage, is_system=True)

    cron_storage.update_system_job_bookkeeping(job.id, next_run_at="2026-05-01T00:00:00Z")
    updated = cron_storage.update_system_job_bookkeeping(job.id, last_status="completed")
...
def test_reconcile_refuses_non_system_row(cron_storage: CronJobStorage) -> None:
    ...
    with pytest.raises(ValueError, match="non-system"):
        cron_storage.reconcile_system_job_definition(job.id, action_type="handler", action_config={})
...
def test_toggle_job_on_system_row_recomputes_next_run_at(cron_storage: CronJobStorage) -> None:
    job = _job(cron_storage, is_system=True)
    cron_storage.update_job(job.id, enabled=False, next_run_at=None)

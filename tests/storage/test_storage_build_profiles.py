...
def test_bundled_build_profiles_sync_and_resolve(
    ...
    assert submit.delivery_mode == "auto"
    assert submit.skip_stages == ["merge"]


def test_project_profile_rejects_pull_request_delivery_mode(
    temp_db: HubDatabase, sample_project: dict[str, Any]
) -> None:
    manager = BuildProfileManager(temp_db)
    with pytest.raises(BuildProfileError, match="delivery_mode must be auto"):
        manager.create(

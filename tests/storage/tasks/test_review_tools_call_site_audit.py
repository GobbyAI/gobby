...
def test_all_callers_satisfy_policy() -> None:
    workflows = source_texts(("src/gobby/install/shared",))

    assert "review_policy: none" not in workflows
    assert "mark_task_needs_review" not in workflows or "review_policy: required" in workflows


def test_test_architect_yaml_does_not_call_review_tools() -> None:
    source = source_text("src/gobby/install/shared/agents/test-architect.yaml")

    assert "mark_task_review_approved" not in source
    assert "mark_task_review_rejected" not in source
    assert "mark_task_needs_review" not in source
...
def test_no_residual_success_hook_keyed_on_unused_tool() -> None:
    ...
    assert "on_mcp_success: mark_task_review_approved" not in source
...
def test_post_cutover_allowlist_matches_phase_2_6_6() -> None:
    ...
    assert "test-architect.yaml" not in source or "mark_task_needs_review" not in source

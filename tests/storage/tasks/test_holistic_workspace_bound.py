"""Bounds for the holistic_qa cited retry-neutral dispatch cycle (gobby-#17668).
...
``dispatch_spawn_failed:`` holistic_qa fail_stage that cites every closed
automated child. That path is intentionally retry-neutral: it decrements the
holistic work_attempt_count and reopens the cited children so the epic can be
retried once the workspace is buildable again.

These tests pin the persistent bound that stops the retry-neutral path from
looping forever:

- ``retry_neutral_failure_count`` accumulates on the holistic_qa stage row and
...
from gobby.storage.tasks._stage_state_transitions import MAX_HOLISTIC_WORKSPACE_FAILURES
...
def _epic_with_holistic_manifest(manager: LocalTaskManager, sample_project: dict) -> object:
...
def test_persistent_workspace_failure_escalates_epic_after_cap(temp_db, sample_project) -> None:
    manager = LocalTaskManager(temp_db)
    epic = _epic_with_holistic_manifest(manager, sample_project)
    leaf = _closed_child(temp_db, sample_project, epic.id, "Leaf")

    for _ in range(MAX_HOLISTIC_WORKSPACE_FAILURES):
        # Each heartbeat: children re-run, development completes, holistic runs.
        set_stage_state(temp_db, epic.id, "development", "done", work_attempt_count=1)
        set_stage_state(temp_db, epic.id, "holistic_qa", "in_progress", work_attempt_count=1)
    ...
    holistic = stage_row(temp_db, epic.id, "holistic_qa")
    assert holistic["retry_neutral_failure_count"] == MAX_HOLISTIC_WORKSPACE_FAILURES
    assert epic_row["is_escalated"] is True
    assert epic_row["escalation_reason"] == "holistic_workspace_failed:max"
...
def test_transient_workspace_failure_does_not_escalate_or_burn_attempts(
    ...
    set_stage_state(temp_db, epic.id, "holistic_qa", "in_progress", work_attempt_count=3)
    ...
    assert holistic["retry_neutral_failure_count"] == 1
    assert holistic["work_attempt_count"] == 2
...
def test_genuine_reviewer_citation_clears_child_escalation(temp_db, sample_project) -> None:
    ...
    manager.stage_states.fail_stage(
        epic.id,
        "holistic_qa",
        reason="holistic_review_findings:child_regressed",
    ...
    assert holistic["retry_neutral_failure_count"] == 0

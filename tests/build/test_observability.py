...
def test_get_build_status_reports_agents_mutex_artifacts_events_and_comments(
    ...
    temp_db.execute(
        ...
        VALUES (%s, %s, 'dispatcher', 'system', '## Epic QA Failure\n\nNeeds work')
        """,
...
def test_explain_dispatch_reports_epic_descendant_gate(temp_db) -> None:
    ...
    temp_db.execute(
        "UPDATE task_stage_states SET state = 'ready' WHERE task_id = %s AND stage_name = 'epic_qa'",
        (root.id,),
    )
    ...
    gate = explanation["epic_descendant_gate"]
    assert gate["reason"] == "epic_descendants_nonterminal"
    ...
    assert action["body"].startswith("Epic QA is waiting for nonterminal descendants:")

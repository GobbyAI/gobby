...
SKILL_DIR = REPO_ROOT / "src/gobby/install/shared/skills/c"


def test_c_skill_parses_with_references() -> None:
    ...
    assert reference_paths == {
        ...
        "references/memory-and-lifetime.md",
        "references/portability-and-performance.md",
        "references/testing.md",
        "references/types-and-abi.md",
    }


def test_synced_c_skill_is_searchable(temp_db: HubDatabase) -> None:
    ...
    results = search.search("c cmake make autotools sanitizer malloc header abi", top_k=3)
    assert results
    assert results[0].skill_name == "c"

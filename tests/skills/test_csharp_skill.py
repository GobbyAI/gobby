...
SKILL_DIR = REPO_ROOT / "src/gobby/install/shared/skills/csharp"


def test_csharp_skill_parses_with_references() -> None:
    ...
    assert 'get_skill_file(name="csharp", path="references/configuration.md")' in (parsed.content)
    assert 'get_skill_file(name="csharp", path="references/framework-boundaries.md")' in (
        parsed.content
    )
    ...
    assert reference_paths == {
        ...
        "references/data-and-serialization.md",
...
def test_synced_csharp_skill_is_searchable(temp_db: HubDatabase) -> None:
    ...
    skill = manager.get_by_name("csharp")
    ...
    results = search.search("csharp dotnet csproj nullable async aspnet efcore xunit", top_k=3)
    assert results
    assert results[0].skill_name == "csharp"

...
WORKFLOW_PATH = REPO_ROOT / "src/gobby/install/shared/workflows/review.yaml"
...
def test_holistic_reviewer_wired() -> None:
    ...
    assert _has_spawn_agent_step(workflow, "holistic-reviewer")
    assert "task_id" in workflow.get("inputs", {})
    assert "approve" in workflow_text.lower()
    assert "reject" in workflow_text.lower()
    assert "escalate" in workflow_text.lower()

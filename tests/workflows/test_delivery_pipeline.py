...
class TestInjectionOutcomeCapture:
    ...
    def test_injected_delivery_queues_usefulness_judgment(self, db: HubDatabase) -> None:
        ...
        payload = {
            ...
            "producer": "daemon_memory_recall",
            "origin_turn_seq": 4,
            "recall_request_id": "recall-queue",
            "project_id": PROJECT_ID,
            "memories": [{"id": "m-q", "content": "queued fact", "type": "fact"}],
        }

        result = engine._format_memory_recall_delivery(payload, PLATFORM_SESSION_ID, _variables())

        assert result is not None
        assert recorded  # outcomes recorded → queue written
        pending = _vars(db, PLATFORM_SESSION_ID)[PENDING_USEFULNESS_VARIABLE]
        assert pending == [
            {
                "recall_request_id": "recall-queue",
                "memory_ids": ["m-q"],
    ...
    def test_no_usefulness_queue_without_recorder(self, db: HubDatabase) -> None:
        from gobby.memory.usefulness import PENDING_USEFULNESS_VARIABLE

        engine = RuleEngine(db)  # no injection_outcome_recorder wired
        payload = {
            "type": "memory_recall",
            "producer": "daemon_memory_recall",
            "origin_turn_seq": 4,
            "recall_request_id": "recall-off",
            "memories": [{"id": "m-off", "content": "fact"}],
        }

        result = engine._format_memory_recall_delivery(payload, PLATFORM_SESSION_ID, _variables())

        assert result is not None
        assert PENDING_USEFULNESS_VARIABLE not in _vars(db, PLATFORM_SESSION_ID)

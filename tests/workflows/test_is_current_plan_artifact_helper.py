...
def test_exact_match_with_absolute_file_path(tmp_path: Path) -> None:
...
def test_neighboring_plan_file_does_not_match(tmp_path: Path) -> None:
    project_path = str(tmp_path)
    artifact_rel = ".gobby/plans/task-42-plan.md"
...
def test_missing_artifact_path_returns_false(tmp_path: Path) -> None:
    file_path = str(tmp_path / ".gobby" / "plans" / "task-42-plan.md")
...
def test_path_normalization_handles_dot_segments(tmp_path: Path) -> None:
    project_path = str(tmp_path)

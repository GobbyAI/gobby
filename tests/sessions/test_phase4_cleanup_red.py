...
def test_hooks_source_no_longer_uses_session_storage_alias(repo_root) -> None:
    """Hooks should stop carrying the legacy _session_storage name."""
    hooks_root = repo_root / "src" / "gobby" / "hooks"
...
def test_hook_manager_keeps_exactly_one_session_manager_attribute(repo_root) -> None:
    """HookManager should keep only _session_manager after cleanup."""
    source_path = repo_root / "src" / "gobby" / "hooks" / "hook_manager.py"
...
def test_session_manager_service_tests_use_canonical_attribute_access(repo_root) -> None:
    """Session-manager tests should stop using shim imports and dict-style assertions."""
    source_path = repo_root / "tests" / "sessions" / "test_sessions_manager.py"
    assert source_path.exists()
    source = source_path.read_text(encoding="utf-8")
